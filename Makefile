...
functional-test-xdc-coverage: prepare-coverage-test
	...
	@printf $(COLOR) "VOID ..."
	@$(GOTESTSUM) --debug --junitfile $(NEW_REPORT) -- \
		-v $(FUNCTIONAL_TEST_XDC_ROOT) -timeout=$(TEST_TIMEOUT) $(TEST_TAG_FLAG) -persistenceType=$(PERSISTENCE_TYPE) -persistenceDriver=$(PERSISTENCE_DRIVER) $(FUNCTIONAL_TEST_COVERPKG) -coverprofile=$(NEW_COVER_PROFILE)
