...
# Nil-safety analysis. Scoped to chasm/lib/scheduler for now; override NILAWAY_PKGS
# and NILAWAY_INCLUDE_PKGS to widen coverage as more packages are made nil-clean.
# -include-pkgs restricts the expensive inference to our own packages; without it
# nilaway analyzes the entire transitive dependency graph and OOMs CI runners.
.PHONY: lint-nilaway
NILAWAY_PKGS ?= ./chasm/lib/scheduler/...
NILAWAY_INCLUDE_PKGS ?= $(MODULE_ROOT)/chasm/lib/scheduler
lint-nilaway: $(NILAWAY)
	@printf $(COLOR) "Running nilaway on scheduler package..."
	@$(NILAWAY) \
		-include-pkgs $(NILAWAY_INCLUDE_PKGS) \
		-include-errors-in-files $(ROOT)/chasm/lib/scheduler \
		-exclude-test-files \
