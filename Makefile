...
integration-test-coverage: prepare-coverage-test
	...
	go run ./cmd/tools/test-runner2 test --max-attempts=$(MAX_TEST_ATTEMPTS) --run-timeout=5m --junitfile=$(NEW_REPORT) --log-dir=$(TEST_OUTPUT_ROOT) --group-by=file -- \
...
functional-test-coverage: prepare-coverage-test
	...
	go run ./cmd/tools/test-runner2 test --max-attempts=$(MAX_TEST_ATTEMPTS) --run-timeout=5m --junitfile=$(NEW_REPORT) --log-dir=$(TEST_OUTPUT_ROOT) --group-by=suite -- \
