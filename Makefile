...
$(GOLANGCI_LINT): $(LOCALBIN) $(shell find cmd/tools/lint-experimental-api tools/lint-experimental-api -name "*.go" 2>/dev/null)
	@printf $(COLOR) "Build custom golangci-lint..."
	@go build \
		-ldflags="-s -w -X main.version=$(GOLANGCI_LINT_VERSION)" \
		-o $(ROOT)/$(GOLANGCI_LINT) ./cmd/tools/lint-experimental-api
