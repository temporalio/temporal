...
func (c *clientImpl) ForceRescheduleActivity(
	ctx context.Context,
	request *historyservice.ForceRescheduleActivityRequest,
	opts ...grpc.CallOption,
) (*historyservice.ForceRescheduleActivityResponse, error) {
	shardID := c.shardIDFromWorkflowID(request.GetNamespaceId(), request.GetWorkflowId())
	var response *historyservice.ForceRescheduleActivityResponse
	op := func(ctx context.Context, client historyservice.HistoryServiceClient) error {
		var err error
		ctx, cancel := c.createContext(ctx)
		defer cancel()
		response, err = client.ForceRescheduleActivity(ctx, request, opts...)
		return err
	}
	if err := c.executeWithRedirect(ctx, shardID, op); err != nil {
		return nil, err
