...
func (c *clientImpl) ForceWorkflowTaskTimeout(
	ctx context.Context,
	request *historyservice.ForceWorkflowTaskTimeoutRequest,
	opts ...grpc.CallOption,
) (*historyservice.ForceWorkflowTaskTimeoutResponse, error) {
	shardID := c.shardIDFromWorkflowID(request.GetNamespaceId(), request.GetWorkflowExecution().GetWorkflowId())
	var response *historyservice.ForceWorkflowTaskTimeoutResponse
	op := func(ctx context.Context, client historyservice.HistoryServiceClient) error {
		var err error
		ctx, cancel := c.createContext(ctx)
		defer cancel()
		response, err = client.ForceWorkflowTaskTimeout(ctx, request, opts...)
		return err
	}
	if err := c.executeWithRedirect(ctx, shardID, op); err != nil {
		return nil, err
