...
// Marshal an object of type VersionReactivationEligibility to the protobuf v3 wire format
func (val *VersionReactivationEligibility) Marshal() ([]byte, error) {
	return proto.Marshal(val)
}

// Unmarshal an object of type VersionReactivationEligibility from the protobuf v3 wire format
func (val *VersionReactivationEligibility) Unmarshal(buf []byte) error {
	return proto.Unmarshal(buf, val)
}

// Size returns the size of the object, in bytes, once serialized
func (val *VersionReactivationEligibility) Size() int {
	return proto.Size(val)
}

// Equal returns whether two VersionReactivationEligibility values are equivalent by recursively
// comparing the message's fields.
// For more information see the documentation for
// https://pkg.go.dev/google.golang.org/protobuf/proto#Equal
func (this *VersionReactivationEligibility) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	var that1 *VersionReactivationEligibility
	switch t := that.(type) {
	case *VersionReactivationEligibility:
		that1 = t
	case VersionReactivationEligibility:
		that1 = &t
	default:
