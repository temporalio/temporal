...
CREATE TABLE task_queue_user_data (
  ...
  data            VARBINARY(MAX) NOT NULL, -- temporal.server.api.persistence.v1.TaskQueueUserData
...
CREATE TABLE history_scheduled_tasks (
  shard_id INT NOT NULL,
  category_id INT NOT NULL,
  visibility_timestamp DATETIME2(6) NOT NULL,
  task_id BIGINT NOT NULL,
  --
  data VARBINARY(MAX) NOT NULL,
  data_encoding VARCHAR(16) NOT NULL,
...
CREATE TABLE chasm_node_maps (
  ...
  -- SQL Server limits clustered primary keys to 900 bytes and chasm_path alone
  -- can be 1536 bytes, so the primary key uses a persisted SHA2-256 hash of
  -- chasm_path instead of the raw path. chasm_path stays a regular column and
  -- predicates on it remain valid.
  chasm_path_hash AS CAST(HASHBYTES('SHA2_256', chasm_path) AS BINARY(32)) PERSISTED NOT NULL,
--
  metadata VARBINARY(MAX) NOT NULL,
  metadata_encoding VARCHAR(16),
  data VARBINARY(MAX),
  data_encoding VARCHAR(16),
  PRIMARY KEY (shard_id, namespace_id, workflow_id, run_id, chasm_path_hash)
);
...
CREATE TABLE cluster_membership (
    ...
    session_start        DATETIME2(6) DEFAULT '1970-01-02 00:00:01',
    last_heartbeat       DATETIME2(6) DEFAULT '1970-01-02 00:00:01',
    record_expiry        DATETIME2(6) DEFAULT '1970-01-02 00:00:01',
    PRIMARY KEY (membership_partition, host_id)
);

CREATE INDEX cm_idx_rolehost ON cluster_membership (role, host_id);
...
CREATE TABLE queues (
    ...
    metadata_payload VARBINARY(MAX) NOT NULL,
...
CREATE TABLE nexus_endpoints (
    id            BINARY(16) NOT NULL,
    data          VARBINARY(MAX) NOT NULL, -- temporal.server.api.persistence.v1.NexusEndpoint
    data_encoding VARCHAR(16) NOT NULL,    -- Encoding type used for serialization, in practice this should always be proto3
    version       BIGINT NOT NULL,         -- Version of this row, used for optimistic concurrency
    PRIMARY KEY (id)
);
