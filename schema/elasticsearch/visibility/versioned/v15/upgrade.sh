...
echo "=== Step 1. Add TemporalPriorityKey and TemporalFairnessKey builtin search attributes ==="
...
if [ -z "${AUTO_CONFIRM}" ]; then
    read -p "Add TemporalPriorityKey and TemporalFairnessKey builtin search attributes to the index ${ES_VIS_INDEX_V1}? (N/y)" -n 1 -r
    echo
else
    REPLY="y"
fi
