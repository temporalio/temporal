...
echo "=== Step 1. Add TemporalScheduleNextActionTime builtin search attribute ==="
...
if [ -z "${AUTO_CONFIRM}" ]; then
    read -p "Add TemporalScheduleNextActionTime builtin search attribute to the index ${ES_VIS_INDEX_V1}? (N/y)" -n 1 -r
    echo
else
    REPLY="y"
fi
