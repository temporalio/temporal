...
func TestRequire_FailureScenarios(t *testing.T) {
	...
	t.Run("reports all attempt errors on timeout", func(t *testing.T) {
		...
		require.Contains(t, tb.fatals(), "not satisfied after")
		errs := tb.errors()
		require.Contains(t, errs, "await stats: polls=2 failed_attempts=2")
		require.Contains(t, errs, "attempt_duration min=")
		require.Contains(t, errs, "sleep_duration min=")
		require.Contains(t, errs, "slowest attempts:")
		require.Contains(t, errs, "context at timeout:")
