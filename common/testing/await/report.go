...
func (r timeoutReport) reportTimeout(
	...
	var sections []string
	sections = append(sections, formatAwaitStats(r.stats, parentErr, awaitErr, deadlineRemaining, r.attempts))
	if s := formatFinalAttemptContext(r.failures); s != "" {
		sections = append(sections, s)
	}
	if s := formatAttemptErrors(r.failures); s != "" {
		sections = append(sections, s)
	}
	...
	tb.Fatalf("%s\n\n%s: %s\ndetails:\n  attempts         = %d\n  attempt timeouts = %d",
		diagnostics, funcName, message, r.attempts, r.attemptTimeouts)
}
