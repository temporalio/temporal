...
func (s *Scenario) Verify(p propAccessor) {
	s.t.Helper()
	if p.getVal() != true {
...
func (s *Scenario) Eventually(p propAccessor, opts ...WaiterOption) {
	s.t.Helper()
...
func (s *Scenario) Run(testFn func(*Scenario)) {
	s.t.Logf("starting exploration run")
...
func (s *Scenario) Maybe(name string, fn func()) {
	s.t.Helper()
