...
func fullyPopulatedReplication(phase ReplicationPhase) ReplicationLifecyclePayload {
	return ReplicationLifecyclePayload{
		...
		HydratedVT:               &VersionedTransitionEntry{FailoverVersion: 5, TransitionCount: 7},
		ShippedFirstEventID:      19,
		ShippedFirstEventVersion: 5,
		ShippedLastEventID:       30,
		ShippedLastEventVersion:  11,
		Attempt:                  2,
...
func TestReplicationLifecycleFieldSetLocked(t *testing.T) {
	...
	want := map[ReplicationPhase]map[string]any{
		ReplicationSent: mergeFields(base, map[string]any{
			...
			"hydrated_versioned_transition":        `{"failover_version":5,"transition_count":7}`,
			"shipped_first_event_id":               int64(19),
			"shipped_first_event_version":          int64(5),
			"shipped_last_event_id":                int64(30),
			"shipped_last_event_version":           int64(11),
		}),
