...
func TestFindDeploymentVersionForWorkflowID(t *testing.T) {
	tests := []struct {
		...
		// useRampingVersionInitialTask=true: always routes to ramping version regardless of ramp percentage
		{
			...
			useRampingVersionInitialTask: true,
			want:                         v2,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got, _ := FindTargetDeploymentVersionAndRevisionNumberForWorkflowID(tt.current.GetVersion(), 0, tt.ramping.GetVersion(), tt.ramping.GetRampPercentage(), 0, "my-wf-id", tt.useRampingVersionInitialTask); !got.Equal(tt.want) {
