...
func (s *taskStore) CreateTasks(
	...
	// Update metadata along with tasks, similar to Cassandra v2 behavior.
	// This ensures metadata (including ApproximateBacklogCount) is persisted with each task write.
	queueFilter := bson.M{"_id": queueID, "range_id": request.RangeID}
	updateQueue := bson.M{"$set": bson.M{
		"data":          request.TaskQueueInfo.Data,
		"data_encoding": request.TaskQueueInfo.EncodingType.String(),
		"updated_at":    time.Now().UTC(),
	}}
	...
	insertedIDs := make([]string, 0, len(request.Tasks))
	for _, task := range request.Tasks {
		...
		if _, err := tasksColl.InsertOne(ctx, doc); err != nil {
			if mongo.IsDuplicateKeyError(err) {
				return nil, &persistence.ConditionFailedError{Msg: "task already exists"}
			}
			return nil, serviceerror.NewUnavailablef("CreateTasks insert failed: %v", err)
		}
		insertedIDs = append(insertedIDs, docID)
	}

	res, err := s.db.Collection(collectionTaskQueues).UpdateOne(ctx, queueFilter, updateQueue)
	if err != nil {
		return nil, serviceerror.NewUnavailablef("CreateTasks update queue failed: %v", err)
	}
	if res.MatchedCount == 0 {
		if len(insertedIDs) > 0 {
			_, _ = tasksColl.DeleteMany(ctx, bson.M{"_id": bson.M{"$in": insertedIDs}})
		}
		return nil, &persistence.ConditionFailedError{Msg: "task queue range mismatch"}
	}

	// Metadata (including ApproximateBacklogCount) is updated along with tasks,
	// similar to Cassandra behavior. This ensures draining backlog managers
	// see correct counts when reading from persistence during migration.
	return &persistence.CreateTasksResponse{UpdatedMetadata: true}, nil
}
