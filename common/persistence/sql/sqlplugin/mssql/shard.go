...
const (
 ...
 WHERE shard_id = ?`
	...
	lockShardQry     = `SELECT range_id FROM shards WITH (UPDLOCK, ROWLOCK) WHERE shard_id = ?`
	readLockShardQry = `SELECT range_id FROM shards WITH (REPEATABLEREAD, ROWLOCK) WHERE shard_id = ?`
)
