...
const (
 ...
 WHERE shard_id = ? AND namespace_id = ? AND workflow_id = ? AND run_id = ?`
	...
	// T-SQL has no FOR UPDATE / FOR SHARE suffix; lock hints go on the table
	// in the FROM clause, so the write/read lock queries are spelled out in
	// full instead of sharing a base constant.
	writeLockExecutionQuery = `SELECT db_record_version, next_event_id FROM executions WITH (UPDLOCK, ROWLOCK)
 WHERE shard_id = ? AND namespace_id = ? AND workflow_id = ? AND run_id = ?`
	readLockExecutionQuery = `SELECT db_record_version, next_event_id FROM executions WITH (REPEATABLEREAD, ROWLOCK)
 WHERE shard_id = ? AND namespace_id = ? AND workflow_id = ? AND run_id = ?`
...
FROM current_executions ce WITH (UPDLOCK, ROWLOCK)
INNER JOIN executions e WITH (UPDLOCK, ROWLOCK) ON e.shard_id = ce.shard_id AND e.namespace_id = ce.namespace_id AND e.workflow_id = ce.workflow_id AND e.run_id = ce.run_id
...
FROM current_chasm_executions ce WITH (UPDLOCK, ROWLOCK)
INNER JOIN executions e WITH (UPDLOCK, ROWLOCK) ON e.shard_id = ce.shard_id AND e.namespace_id = ce.namespace_id AND e.workflow_id = ce.business_id AND e.run_id = ce.run_id
WHERE ce.shard_id = ? AND ce.namespace_id = ? AND ce.business_id = ? AND ce.archetype_id = ?`

	lockCurrentExecutionQuery = `SELECT
shard_id, namespace_id, workflow_id, run_id, create_request_id, state, status, start_time, last_write_version, data, data_encoding
FROM current_executions WITH (UPDLOCK, ROWLOCK) WHERE shard_id = ? AND namespace_id = ? AND workflow_id = ?`
	lockCurrentChasmExecutionQuery = `SELECT
shard_id, namespace_id, business_id as workflow_id, run_id, create_request_id, state, status, start_time, last_write_version, data, data_encoding
FROM current_chasm_executions WITH (UPDLOCK, ROWLOCK) WHERE shard_id = ? AND namespace_id = ? AND business_id = ? AND archetype_id = ?`
...
`
 ...
 VALUES(:shard_id, :category_id, :task_id, :data, :data_encoding)`
	...
	getHistoryImmediateTasksQuery = `SELECT TOP (?) task_id, data, data_encoding
 FROM history_immediate_tasks WHERE shard_id = ? AND category_id = ? AND task_id >= ? AND task_id < ? ORDER BY task_id`
  ...
  VALUES (:shard_id, :category_id, :visibility_timestamp, :task_id, :data, :data_encoding)`
	...
	getHistoryScheduledTasksQuery = `SELECT TOP (?) visibility_timestamp, task_id, data, data_encoding FROM history_scheduled_tasks
  WHERE shard_id = ?
  AND category_id = ?
 ...
 VALUES(:shard_id, :task_id, :data, :data_encoding)`
	...
	getTransferTasksQuery = `SELECT TOP (?) task_id, data, data_encoding
 FROM transfer_tasks WHERE shard_id = ? AND task_id >= ? AND task_id < ? ORDER BY task_id`
  ...
  VALUES (:shard_id, :visibility_timestamp, :task_id, :data, :data_encoding)`
	...
	getTimerTasksQuery = `SELECT TOP (?) visibility_timestamp, task_id, data, data_encoding FROM timer_tasks
  WHERE shard_id = ?
  AND ((visibility_timestamp >= ? AND task_id >= ?) OR visibility_timestamp > ?)
  AND visibility_timestamp < ?
  ORDER BY visibility_timestamp,task_id`
  ...
  VALUES(:shard_id, :task_id, :data, :data_encoding)`
	...
	getReplicationTasksQuery = `SELECT TOP (?) task_id, data, data_encoding FROM replication_tasks WHERE
shard_id = ? AND task_id >= ? AND task_id < ? ORDER BY task_id`
	...
	getReplicationTasksDLQQuery = `SELECT TOP (?) task_id, data, data_encoding FROM replication_tasks_dlq WHERE
...
ORDER BY task_id`
 ...
 VALUES(:shard_id, :task_id, :data, :data_encoding)`
	...
	getVisibilityTasksQuery = `SELECT TOP (?) task_id, data, data_encoding
 FROM visibility_tasks WHERE shard_id = ? AND task_id >= ? AND task_id < ? ORDER BY task_id`
	...
	// buffered_events.id is an IDENTITY column in the mssql schema; inserts
	// must not set it.
...
VALUES (:shard_id, :namespace_id, :workflow_id, :run_id, :data, :data_encoding)`
	...
	getBufferedEventsQuery    = `SELECT data, data_encoding FROM buffered_events WHERE shard_id = ? AND namespace_id = ? AND workflow_id = ? AND run_id = ? ORDER BY id`
...
//
// NOTE: sqlx expands the row slice into a single multi-row INSERT and SQL
// Server caps one batch at 2100 parameters, so very large task batches would
// exceed the cap. Chunking oversized batches is a known follow-up; this
// applies to every multi-row task insert in this file.
...
// (see InsertIntoHistoryImmediateTasks for the 2100-parameter batch cap note)
func (mdb *db) InsertIntoHistoryScheduledTasks(
	ctx context.Context,
	rows []sqlplugin.HistoryScheduledTasksRow,
) (sql.Result, error) {
	for i := range rows {
		rows[i].VisibilityTimestamp = mdb.converter.ToMSSQLDateTime(rows[i].VisibilityTimestamp)
	}
...
func (mdb *db) RangeSelectFromHistoryScheduledTasks(
	...
	filter.InclusiveMinVisibilityTimestamp = mdb.converter.ToMSSQLDateTime(filter.InclusiveMinVisibilityTimestamp)
	filter.ExclusiveMaxVisibilityTimestamp = mdb.converter.ToMSSQLDateTime(filter.ExclusiveMaxVisibilityTimestamp)
	if err := mdb.SelectContext(ctx,
		&rows,
	...
	for i := range rows {
		rows[i].VisibilityTimestamp = mdb.converter.FromMSSQLDateTime(rows[i].VisibilityTimestamp)
	}
	return rows, nil
}
...
func (mdb *db) DeleteFromHistoryScheduledTasks(
	...
	filter.VisibilityTimestamp = mdb.converter.ToMSSQLDateTime(filter.VisibilityTimestamp)
	return mdb.ExecContext(ctx,
