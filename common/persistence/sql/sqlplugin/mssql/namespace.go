...
const (
 ...
 WHERE partition_id=54321 AND id = ?`
	...
	// TOP (?) replaces the trailing LIMIT of the mysql/postgresql queries, so
	// the page-size argument is bound FIRST at these call sites.
	listNamespacesQuery      = `SELECT TOP (?) id, name, is_global, data, data_encoding, notification_version FROM namespaces WHERE partition_id=? ORDER BY id`
	listNamespacesRangeQuery = `SELECT TOP (?) id, name, is_global, data, data_encoding, notification_version FROM namespaces WHERE partition_id=? AND id > ? ORDER BY id`
	...
	lockNamespaceMetadataQuery   = `SELECT notification_version FROM namespace_metadata WITH (UPDLOCK, ROWLOCK) WHERE partition_id=?`
	updateNamespaceMetadataQuery = `UPDATE namespace_metadata SET notification_version = ? WHERE notification_version = ? AND partition_id=?`
)
