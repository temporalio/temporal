...
const (
	...
	templateGetMessagesQuery         = `SELECT TOP (?) message_id, message_payload, message_encoding FROM queue WHERE queue_type = ? and message_id > ? and message_id <= ? ORDER BY message_id ASC`
	...
	templateGetLastMessageIDQuery = `SELECT TOP (1) message_id FROM queue WITH (UPDLOCK, ROWLOCK) WHERE queue_type = ? ORDER BY message_id DESC`
	...
	templateLockQueueMetadataQuery   = `SELECT data, data_encoding, version from queue_metadata WITH (UPDLOCK, ROWLOCK) WHERE queue_type = ?`
)
