...
const (
	...
	// TOP (?) replaces LIMIT ?; the limit binds first and an explicit
	// ORDER BY keeps the task_queue_name > ? pagination deterministic.
	listTaskQueueUserDataQry = `SELECT TOP (?) task_queue_name, data, data_encoding, version FROM task_queue_user_data ` +
		`WHERE namespace_id = ? AND task_queue_name > ? ORDER BY task_queue_name ASC`
...
func (mdb *db) ListTaskQueueUserDataEntries(ctx context.Context, request *sqlplugin.ListTaskQueueUserDataEntriesRequest) ([]sqlplugin.TaskQueueUserDataEntry, error) {
	...
	err := mdb.SelectContext(ctx, &rows, listTaskQueueUserDataQry, request.Limit, request.NamespaceID, request.LastTaskQueueName)
	return rows, err
}
