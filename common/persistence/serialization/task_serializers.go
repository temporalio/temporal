...
func serializeTransferTask(
	...
	case *tasks.ActivityCancelControlTask:
		transferTask = transferActivityCancelControlTaskToProto(task)
	default:
...
func deserializeTransferTask(
	...
	case enumsspb.TASK_TYPE_TRANSFER_ACTIVITY_CANCEL_CONTROL:
		task = transferActivityCancelControlTaskFromProto(transferTask)
	default:
...
func transferActivityCancelControlTaskToProto(task *tasks.ActivityCancelControlTask) *persistencespb.TransferTaskInfo {
	return &persistencespb.TransferTaskInfo{
		NamespaceId:    task.WorkflowKey.NamespaceID,
		WorkflowId:     task.WorkflowKey.WorkflowID,
		RunId:          task.WorkflowKey.RunID,
		...
		VisibilityTime: timestamppb.New(task.VisibilityTimestamp),
		TaskDetails: &persistencespb.TransferTaskInfo_ActivityCancelControlTaskDetails_{
			ActivityCancelControlTaskDetails: &persistencespb.TransferTaskInfo_ActivityCancelControlTaskDetails{
...
func transferActivityCancelControlTaskFromProto(task *persistencespb.TransferTaskInfo) tasks.Task {
	details := task.GetActivityCancelControlTaskDetails()
	return &tasks.ActivityCancelControlTask{
		WorkflowKey: definition.NewWorkflowKey(
			task.NamespaceId,
			task.WorkflowId,
			task.RunId,
