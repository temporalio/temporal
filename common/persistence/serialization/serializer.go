...
var DefaultDecoder Decoder = NewSerializerWithEncoding(enumspb.ENCODING_TYPE_UNSPECIFIED)
...
// NewSerializer returns a Serializer that picks the encoding type based on an environment variable.
// If none is set, it defaults to the Proto3 codec. See EncodingTypeFromEnv for details.
func NewSerializer() Serializer {
	return NewSerializerWithEncoding(EncodingTypeFromEnv())
}

// NewSerializerWithEncoding returns a Serializer that uses the provided encoding type.
func NewSerializerWithEncoding(encodingType enumspb.EncodingType) Serializer {
	return &serializerImpl{encodingType: encodingType}
}
...
func (t *serializerImpl) DeserializeStrippedEvents(data *commonpb.DataBlob) ([]*historyspb.StrippedHistoryEvent, error) {
	...
	case enumspb.ENCODING_TYPE_JSON:
		// Not applying performance optimization for JSON as it is only used in a debugging context.
		err = Decode(data, events)
	default:
		return nil, NewUnknownEncodingTypeError(data.EncodingType.String(), enumspb.ENCODING_TYPE_PROTO3, enumspb.ENCODING_TYPE_JSON)
	}
	if err != nil {
