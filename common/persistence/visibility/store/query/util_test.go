...
func TestSAColName(t *testing.T) {
	col := NewSAColName("Alias", "Field", enumspb.INDEXED_VALUE_TYPE_BOOL)
	require.Equal(t, "Field", sqlparser.String(col))

	// system search attribute have special mapping
	col = NewSAColName("NamespaceId", "NamespaceId", enumspb.INDEXED_VALUE_TYPE_KEYWORD)
...
func TestNewFuncExpr(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			name:   "one col arg",
			fnName: "FN_NAME",
			exprs:  []sqlparser.Expr{NamespaceIDSaColName},
			want:   "FN_NAME(namespace_id)",
		},
		{
			name:   "multiple args",
			fnName: "FN_NAME",
			exprs:  []sqlparser.Expr{NamespaceIDSaColName, sqlparser.NewIntVal([]byte("123"))},
