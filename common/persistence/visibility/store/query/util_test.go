...
func TestSAColName(t *testing.T) {
	col := NewSAColumnName("Alias", "Field", enumspb.INDEXED_VALUE_TYPE_BOOL)
	require.Equal(t, "Field", sqlparser.String(col))

	// system search attribute have special mapping
	col = NewSAColumnName("NamespaceId", "NamespaceId", enumspb.INDEXED_VALUE_TYPE_KEYWORD)
...
func TestNewFuncExpr(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			name:   "one col arg",
			fnName: "FN_NAME",
			exprs:  []sqlparser.Expr{NamespaceIDSAColumnName},
			want:   "FN_NAME(namespace_id)",
		},
		{
			name:   "multiple args",
			fnName: "FN_NAME",
			exprs:  []sqlparser.Expr{NamespaceIDSAColumnName, sqlparser.NewIntVal([]byte("123"))},
		...
		},
		{
			...
			want: &sqlparser.OrExpr{
				Left: &sqlparser.OrExpr{
					...
					Right: &sqlparser.OrExpr{
						Left:  sqlparser.NewStrVal([]byte("3")),
