	...
	testCases := []struct {
		...
		},
		{
			...
			inExpr: &sqlparser.AndExpr{
				...
				Right: &sqlparser.ComparisonExpr{
					Operator: sqlparser.EqualStr,
					Left:     NamespaceDivisionSaColName,
			...
			setupMock: func() {
				...
				s.storeQCMock.EXPECT().
					...
					Return(e1, nil)
				s.storeQCMock.EXPECT().
					ConvertKeywordComparisonExpr(sqlparser.EqualStr, NamespaceDivisionSaColName, "bar").
					Return(e2, nil)
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			if tc.mockNamespaceDivisionExpr {
				s.storeQCMock.EXPECT().
					ConvertIsExpr(sqlparser.IsNullStr, NamespaceDivisionSaColName).
...
func (s *queryConverterSuite) TestConvertColName() {
	...
	testCases := []struct {
		...
		},
		{
			...
			out: NamespaceDivisionSaColName,
		},
		{
