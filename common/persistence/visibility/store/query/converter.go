...
type (
	StoreQueryConverter[ExprT any] interface {
		...
		ConvertComparisonExpr(operator string, col *SAColumnName, value any) (ExprT, error)

		ConvertKeywordComparisonExpr(operator string, col *SAColumnName, value any) (ExprT, error)

		ConvertKeywordListComparisonExpr(operator string, col *SAColumnName, value any) (ExprT, error)

		ConvertTextComparisonExpr(operator string, col *SAColumnName, value any) (ExprT, error)

		ConvertRangeExpr(operator string, col *SAColumnName, from, to any) (ExprT, error)

		ConvertIsExpr(operator string, col *SAColumnName) (ExprT, error)
	}
...
func (c *QueryConverter[ExprT]) convertSelectStmt(
	...
	if sel.Where.Expr != nil {
		// // Wrap the WHERE clause in parenthesis to make sure the added conditions below (namespace and
		// // namespace division) won't mess up with the user's query.
		// if _, ok := sel.Where.Expr.(*sqlparser.ParenExpr); !ok {
		// 	sel.Where.Expr = &sqlparser.ParenExpr{Expr: sel.Where.Expr}
		// }
...
func (c *QueryConverter[ExprT]) convertColName(in sqlparser.Expr) (*SAColumnName, error) {
	...
	if searchattribute.IsMappable(saAlias) {
		...
		if saAlias == searchattribute.ScheduleID && saFieldName == saAlias {
			if _, isCustom := c.saTypeMap.Custom()[saFieldName]; !isCustom {
	...
	colName := NewSAColumnName(saAlias, saFieldName, saType)
...
func (c *QueryConverter[ExprT]) parseSQLVal(
	...
	default:
		return c.validateValueTypeV2(saName, saType, value)
	}
}

func (c *QueryConverter[ExprT]) validateValueTypeV2(
	saName string,
	saType enumspb.IndexedValueType,
	value any,
) (any, error) {
