...
func TestSupported(t *testing.T) {
	for sql, expectedJson := range selectCaseMap {
		...
		if err != nil {
			t.Error("the generated dsl json unmarshal error!", sql)
		}
		fmt.Println(expectedJson)
		fmt.Println(string(actualJson))
		fmt.Println("-----------")

		// if !reflect.DeepEqual(expectedMap, source) {
		// 	t.Error("the generated dsl is not equal to expected", expectedJson)
		// }
	}
}
