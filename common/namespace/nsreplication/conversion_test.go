...
// TestShouldReplicateNamespace pins the single replicate/skip gate shared by both
// transports (queue + CHASM). In particular it locks in that a DELETED namespace
// is never replicated regardless of globalness/cluster count — the divergence that
// previously only existed on the queue path.
...
// TestNamespaceDetailToTaskAttributes pins the single detail->wire converter shared
// by both transports. Because both HandleTransmissionTask and the CHASM peer task
// now build their payload here, this guards against the "field replicated by one
// transport but dropped by the other" failure mode: any replicated field added to
// NamespaceTaskAttributes must be threaded through this function or this test fails.
