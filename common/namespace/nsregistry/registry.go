...
func (r *registry) RegisterStateChangeCallback(key any, cb namespace.StateChangeCallbackFn) {
	...
	r.stateChangeCallbacks.Store(
		key,
		namespace.StateChangeCallbackFn(func(ns *namespace.Namespace, deletedFromDb bool) {
...
// Each retry attempt creates a fresh context to ensure clean resource cleanup.
func (r *registry) startWatch(ctx context.Context, initialWatch bool) (watchStartResult, error) {
	// Track the current attempt's cancel function so we can cancel it on retry
	var currentWatchCancel context.CancelFunc

	result, err := backoff.ThrottleRetryContextWithReturn(
		ctx,
		func(ctx context.Context) (watchStartResult, error) {
			// Cancel previous attempt's context to clean up any partial watch state
			if currentWatchCancel != nil {
				currentWatchCancel()
			}

			// Create fresh watch context for this attempt
			watchCtx, watchCancel := context.WithCancel(ctx)
			currentWatchCancel = watchCancel

			eventCh, err := r.persistence.WatchNamespaces(watchCtx)
			if err != nil {
				if !errors.Is(err, persistence.ErrWatchNotSupported) {
					r.logger.Error("Error starting namespace watch", tag.Error(err))
				}
				return watchStartResult{}, err
			}
	...
	if err != nil {
		// Cancel the last failed attempt's context
		if currentWatchCancel != nil {
			currentWatchCancel()
		}
		return watchStartResult{}, err
