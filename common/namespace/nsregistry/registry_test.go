...
// TestRefreshAndReadthroughRace exercises the data race between refreshNamespaces
// (which reads the map after releasing nsMapsLock) and the readthrough path (which
// writes to the map under nsMapsLock).
//
// The race occurred after refreshNamespaces swaps newIDToNamespace into r.idToNamespace
// and releases the lock and then reads the map to record a metric. This test fails
// without the fix when run with -race.
func TestRefreshAndReadthroughRace(t *testing.T) {
	ctrl := gomock.NewController(t)

	mockPersistence := nsregistry.NewMockPersistence(ctrl)
	mockPersistence.EXPECT().WatchNamespaces(gomock.Any()).
		Return(nil, persistence.ErrWatchNotSupported).AnyTimes()

	factory := namespace.NewDefaultReplicationResolverFactory()

	// Namespace A: returned by ListNamespaces (the bulk refresh path).
	idA := namespace.NewID()
	nsRecordA := &persistence.GetNamespaceResponse{
	...
	// Namespace B: returned by GetNamespace (the readthrough path).
	// Not included in ListNamespaces, so each refresh cycle evicts it from the
	// cache, forcing the next GetNamespace("ns-readthrough") to read through again.
	idB := namespace.NewID()
	nsRecordB := &persistence.GetNamespaceResponse{
		Namespace: &persistencespb.NamespaceDetail{
			Info: &persistencespb.NamespaceInfo{
				Id:   idB.String(),
				Name: "ns-readthrough",
	...
	// ListNamespaces always returns only A — B will be evicted each refresh cycle.
	mockPersistence.EXPECT().ListNamespaces(gomock.Any(), gomock.Any()).
		Return(&persistence.ListNamespacesResponse{
			Namespaces: []*persistence.GetNamespaceResponse{nsRecordA},
		}, nil).AnyTimes()

	// GetNamespace returns B on readthrough cache miss.
	mockPersistence.EXPECT().GetNamespace(gomock.Any(), gomock.Any()).
		Return(nsRecordB, nil).AnyTimes()

	// Short refresh interval to maximize overlap between refresh and readthrough.
	reg := nsregistry.NewRegistry(
		mockPersistence,
		false,
	...
	// Hammer GetNamespace("ns-readthrough") from many goroutines. Each call that
	// misses the cache (after a refresh evicts B) triggers the readthrough path,
	// which writes to r.idToNamespace under nsMapsLock — racing with
	// refreshNamespaces reading len(newIDToNamespace) without the lock.
	var wg sync.WaitGroup
	for range 10 {
		wg.Go(func() {
			for range 200 {
				ns, err := reg.GetNamespace(namespace.Name("ns-readthrough"))
				require.NoError(t, err)
				require.Equal(t, namespace.Name("ns-readthrough"), ns.Name())
			}
		})
	}
	wg.Wait()
