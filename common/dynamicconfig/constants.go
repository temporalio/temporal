...
leaves the membership ring, giving in-flight long-polls time to drain before the connection is closed.`,
	...
	EmitReplicationLifecycleEvents = NewGlobalBoolSetting(
		...
		false,
		`EmitReplicationLifecycleEvents controls whether the history service emits ReplicationLifecycle and related ParentChildLifecycle wide events. Cluster-level; default off.`,
	)
