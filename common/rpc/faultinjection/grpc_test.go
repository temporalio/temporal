...
func TestRPCFaultGenerator_RegistersNamespaceIDAndNameAliases(t *testing.T) {
	...
	requests := []struct {
		...
		{name: "namespace ID", request: &matchingservice.AddWorkflowTaskRequest{NamespaceId: "namespace-id"}},
		{name: "namespace name", request: &workflowservice.UpdateNamespaceRequest{Namespace: "namespace-name"}},
	}
	phases := []struct {
		name     string
		matched  bool
		...
		{name: "registered", matched: true, response: "response"},
		{name: "unregistered"},
	}
	...
	for _, phase := range phases {
		...
		for _, test := range requests {
			t.Run(phase.name+"/"+test.name, func(t *testing.T) {
				...
				require.NoError(t, err)
				require.Equal(t, phase.matched, matched)
				require.Equal(t, phase.response, response)
			})
		}
	}
}
