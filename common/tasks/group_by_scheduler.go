...
func (g *groupScheduler) release() {
	...
	if g.activeTasks == 0 && !g.closed {
		g.idleTimerID++
		idleTimerID := g.idleTimerID
		g.idleTimer = time.AfterFunc(g.idleTimeout, func() { g.onIdle(idleTimerID) })
	}
	g.mu.Unlock()
}
...
func (g *groupScheduler) closeIfIdle(idleTimerID uint64) bool {
	g.mu.Lock()
	defer g.mu.Unlock()
	if g.activeTasks != 0 || g.closed || idleTimerID != g.idleTimerID {
		return false
	}
	g.closed = true
	g.idleTimerID++
...
func (s *GroupByScheduler[K, T]) getOrCreateScheduler(key K) *groupScheduler {
	s.mu.RLock()
	if !s.stopped.Load() {
		if group, ok := s.schedulers[key]; ok {
			group.acquire()
			s.mu.RUnlock()
			return group
		}
	...
	if !ok {
		...
		group.onIdle = func(idleTimerID uint64) { s.removeIfIdle(key, group, idleTimerID) }
...
func (s *GroupByScheduler[K, T]) removeIfIdle(key K, group *groupScheduler, idleTimerID uint64) {
	s.mu.Lock()
	if current, ok := s.schedulers[key]; !ok || current != group || !group.closeIfIdle(idleTimerID) {
		s.mu.Unlock()
		return
	}
	delete(s.schedulers, key)
