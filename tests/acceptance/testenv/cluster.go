...
func (c *Cluster) SetGlobalDynamicConfig(
	globalSetting dynamicconfig.GenericSetting,
	value any,
) {
	c.tbase.GetTestCluster().Host().DcClient().OverrideSetting(globalSetting, value)
}
...
func (c *Cluster) CloseShard(workflow *model.WorkflowExecution) {
	c.tbase.AdminClient().CloseShard(testcore.NewContext(), &adminservice.CloseShardRequest{
		ShardId: common.WorkflowIDToHistoryShard(
			string(workflow.GetNamespace().GetID()),
			string(workflow.GetID()),
			c.tbase.GetTestClusterConfig().HistoryConfig.NumHistoryShards,
		),
	})
}
