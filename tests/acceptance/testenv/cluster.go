...
var actionIdKey = "stamp-action-id"
...
func newCluster(s *stamp.Scenario, mdlEnv *stamp.ModelEnv, configs []action.ClusterConfig) *Cluster {
	...
	c := stamp.Act(mdlEnv.Root(), action.StartCluster{Name: "local", Configs: configs})
...
func (c *Cluster) OnAction(
	...
	case *workflowservice.SetWorkerDeploymentCurrentVersionRequest:
		_, err := issueWorkflowRPC(ctx, c.physical.FrontendClient(), t, params.ActID)
		return err
