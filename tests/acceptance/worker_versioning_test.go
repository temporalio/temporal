	...
	t.Run("Start child workflow", func(t *testing.T) {
		ts.Run(t, func(s *Scenario) {
			...
			s.Await(&dvrA.IsCurrent, OnRetry(func() {
				_, _ = ActTry(c, SetWorkerDeploymentCurrentVersion{Version: dvrA})
			}))
			...
			s.Await(&dvrB.IsCurrent, OnRetry(func() {
				_, _ = ActTry(c, SetWorkerDeploymentCurrentVersion{Version: dvrB})
			}))
			wftB.Cancel()

			// Start child workflow
			childTaskQueue := GenChoice[*model.TaskQueue]("child task queue",
				tq,                                     // TODO: add labels
			...
			wft := Act(wkr, RespondWorkflowTaskCompleted{
				...
				Version:            dvrA,
				VersioningBehavior: ANY_VERSIONING_BEHAVIOR,
			})
