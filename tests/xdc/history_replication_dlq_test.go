...
func (s *historyReplicationDLQSuite) TestWorkflowReplicationTaskFailure() {
	...
	s.clusters[1].InjectHook(
		s.T(),
		testhooks.NewHook(testhooks.NamespaceReplicationTaskAfterExecute, func(task any) {
			s.namespaceReplicationTaskExecutors.tasks <- task.(*replicationspb.NamespaceTaskAttributes)
		}),
		namespace.Name(ns),
	)
	...
	s.clusters[1].InjectHook(
		s.T(),
		testhooks.NewHook(testhooks.HistoryReplicationDLQWrite, func(request any) {
	...
	s.clusters[1].InjectHook(
		s.T(),
		testhooks.NewHook(testhooks.HistoryReplicationTaskBeforeExecute, func(task any) error {
			return s.beforeReplicationTaskExecute(task.(*replicationspb.ReplicationTask))
		}),
		namespaceID,
	)
	s.clusters[1].InjectHook(
		s.T(),
		testhooks.NewHook(testhooks.HistoryReplicationTaskAfterExecute, func(task any) {
			s.afterReplicationTaskExecute(task.(*replicationspb.ReplicationTask))
		}),
		namespaceID,
	)
...
func (s *historyReplicationDLQSuite) beforeReplicationTaskExecute(replicationTask *replicationspb.ReplicationTask) error {
