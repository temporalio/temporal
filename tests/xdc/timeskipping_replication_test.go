...
// installs a TimeSkippingFastForwardTimerTask whose visibility is scheduled wall-clock time.
...
//  3. State replicates to standby. Standby's own TimeSkippingFastForwardTimerTask was generated
...
func (s *timeSkippingReplicationSuite) TestStandbyTimeSkippingTimerTaskAcksOnReachedFastForward() {
	...
	// This is driven by the TimeSkippingFastForwardTimerTask firing on the active.
	await.Require(ctx, s.T(), func(t *await.T) {
		info := s.getExecutionInfoFromCluster(ctx, 0, nsID, wfID, runID).GetTimeSkippingInfo()
		require.NotNil(t, info)
		require.False(t, info.GetConfig().GetEnabled(),
	...
	// TimeSkippingFastForwardTimerTask was generated by refreshTasksForTimeSkipping during
	...
	s.True(ff.GetHasReached(),
		"standby must observe HasReached=true; absence indicates the standby's TimeSkippingFastForwardTimerTask didn't replicate the fast-forward transition correctly")
}
