...
func TestQueryWorkflow(t *testing.T) {
	t.Run("Sticky", func(t *testing.T) {
		...
		queryResult, err := sdkClient.QueryWorkflow(ctx, id, "", "test", "test")
		s.NoError(err)
	...
	t.Run("Consistent_PiggybackQuery", func(t *testing.T) {
		s := testcore.NewEnv(t)
		taskQueue := s.Tv().TaskQueue().Name
			...
			for {
				...
				if msg == "pause" {
					...
					_ = workflow.ExecuteLocalActivity(ctx, func() {
						time.Sleep(time.Second * 3) //nolint:forbidigo
					}).Get(ctx, nil)
				}
			}
		}
		...
		err = sdkClient.SignalWorkflow(ctx, id, "", "test", "pause")
		s.NoError(err)

		err = sdkClient.SignalWorkflow(ctx, id, "", "test", "abc")
		s.NoError(err)

		queryResult, err := sdkClient.QueryWorkflow(ctx, id, "", "test")
		s.NoError(err)
	...
	t.Run("QueryWhileBackoff", func(t *testing.T) {
		s := testcore.NewEnv(t)
		tv := s.Tv()
		...
		for tci, tc := range testCases {
			t.Run(tc.testName, func(t *testing.T) {
				...
				queryResp, err := sdkClient.QueryWorkflow(ctx, tv.WorkflowID(), workflowRun.GetRunID(), tv.QueryType())
	...
	t.Run("QueryBeforeStart", func(t *testing.T) {
		s := testcore.NewEnv(t)
		taskQueue := s.Tv().TaskQueue().Name
		...
		// Don't start a worker yet - we want the workflow started but no tasks processed
		id := s.Tv().WorkflowID()
		workflowOptions := sdkclient.StartWorkflowOptions{
			ID:                 id,
			TaskQueue:          taskQueue,
		...
		queryWorker := sdkworker.New(sdkClient, taskQueue, sdkworker.Options{})
	...
	t.Run("QueryFailedWorkflowTask", func(t *testing.T) {
		s := testcore.NewEnv(t)
		taskQueue := s.Tv().TaskQueue().Name
		...
		workflowFn := func(ctx workflow.Context) (string, error) {
			...
			if err != nil {
				t.Fatalf("SetQueryHandler failed: %s", err.Error())
			}
		...
		_, err = sdkClient.QueryWorkflow(ctx, id, "", testname)
		s.Error(err)
		s.IsType(&serviceerror.WorkflowNotReady{}, err)
	})

	t.Run("ClosedWithoutWorkflowTaskStarted", func(t *testing.T) {
		s := testcore.NewEnv(t)
		taskQueue := s.Tv().TaskQueue().Name
		...
		err = sdkClient.TerminateWorkflow(ctx, id, "", "terminating to make sure query fails")
		s.NoError(err)
	...
	t.Run("FailurePropagated", func(t *testing.T) {
		s := testcore.NewEnv(t)
		taskQueue := s.Tv().TaskQueue().Name
		...
		workflowRun, err := sdkClient.ExecuteWorkflow(ctx, client.StartWorkflowOptions{TaskQueue: taskQueue}, "workflow")
		s.NoError(err)
