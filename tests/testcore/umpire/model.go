...
func (c *Composite) Rules() *RuleSet {
	sets := make([]*RuleSet, 0, len(c.components))
	for _, component := range c.components {
		if provider, ok := component.(ruleProvider); ok {
			sets = append(sets, provider.Rules())
		}
	}
...
// mergeRuleSets folds sets into one fresh RuleSet, applying RuleSet.Merge's
// first-seen, name-deduped semantics. nil sets are ignored.
...
// stepTSetter is implemented by anything that needs the current rapid step's
// *T pushed to it before each action — most notably *Interceptor, which fans
// the step out to every strategy (fault injection, observation, mutation, …).
type stepTSetter interface {
	SetStepT(*T)
}
...
func (m *Model[M]) setT(t *T) {
	...
	if setter, ok := any(m.self).(stepTSetter); ok {
		setter.SetStepT(t)
	}
}

func (m *Model[M]) clearStepT() {
	if setter, ok := any(m.self).(stepTSetter); ok {
		setter.SetStepT(nil)
	}
}
