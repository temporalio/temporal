...
// with the same seed.
//
// EntityStore is safe for concurrent use. This matters because the rapid
// action goroutine reads the store while server-side observer strategies
// write to it from gRPC handler goroutines; the mutex keeps those accesses
// memory-safe. Note that it does not make a read-modify-write across two
// calls atomic — callers that need that (e.g. check-then-Add) must tolerate
// the store changing under them, which is inherent to observing a live
// server.
