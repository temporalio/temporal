...
// Now returns the umpire's clock time at the moment CheckRules was invoked.
// Liveness rules compare it against Record.At to fail an overdue pending
// condition mid-run (final == false) instead of waiting for the final check.
// A zero value means no clock was set (e.g. a hand-built context in a test).
...
// Rule is the sealed interface implemented by SafetyRule and LivenessRule.
// The methods are unexported, so only this package can supply a rule type;
// that lets AddRule and the RuleSet operations dispatch on a single
// interface instead of switching on `any` at runtime, turning a whole class
// of wiring mistakes into compile errors.
type Rule interface {
	ruleName() string
	rulePhase() rulePhase
	ruleCoverage() []CoveragePoint
	ruleExamples() func(*parallelsuite.T)
	ruleIsEmpty() bool
	// run invokes the rule's Check (if any). The final flag is ignored by
	// safety rules and passed through to liveness rules.
	run(ctx *RuleContext, history []*Record, final bool)
}

// rulePhase orders rule evaluation: all safety rules are checked before any
// liveness rule (see CheckRules).
type rulePhase int

const (
	phaseSafety rulePhase = iota
	phaseLiveness
)
...
func (r SafetyRule) ruleName() string                     { return r.Name }
func (r SafetyRule) rulePhase() rulePhase                 { return phaseSafety }
func (r SafetyRule) ruleCoverage() []CoveragePoint        { return r.Coverage }
func (r SafetyRule) ruleExamples() func(*parallelsuite.T) { return r.Examples }
func (r SafetyRule) ruleIsEmpty() bool                    { return r.Check == nil && r.Examples == nil }

func (r SafetyRule) run(ctx *RuleContext, history []*Record, _ bool) {
	if r.Check != nil {
		r.Check(ctx, history)
	}
}
...
func (r LivenessRule) ruleName() string                     { return r.Name }
func (r LivenessRule) rulePhase() rulePhase                 { return phaseLiveness }
func (r LivenessRule) ruleCoverage() []CoveragePoint        { return r.Coverage }
func (r LivenessRule) ruleExamples() func(*parallelsuite.T) { return r.Examples }
func (r LivenessRule) ruleIsEmpty() bool                    { return r.Check == nil && r.Examples == nil }

func (r LivenessRule) run(ctx *RuleContext, history []*Record, final bool) {
	if r.Check != nil {
		r.Check(ctx, history, final)
	}
}
