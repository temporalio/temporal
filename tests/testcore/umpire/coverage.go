...
// coverageInstanceKey derives a map key from an arbitrary coverage instance
// key. It uses Go-syntax formatting so callers can pass any value (an ID
// string, a struct of identifying fields, …) without constraining it to
// comparable. The contract is that two instance keys are the same coverage
// instance iff they format identically; pass values whose %#v rendering is
// stable and distinguishing (avoid pointers and unexported-field structs).
