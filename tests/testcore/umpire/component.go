...
// running scenario. The extraction helpers (Behaviors, Strategies, Rules,
// RPCs, Faults) silently keep the first-seen entry on a conflict, so an
// inconsistent set runs with whatever the merge happens to produce. Build the
// set through NewComponents — it runs Validate up front — rather than
// constructing the slice literal directly, so those conflicts surface as an
// error instead of silently-wrong behavior.
type Components []*Component

// NewComponents validates the given components and returns them as a
// Components value. It is the recommended entry point: a non-nil error means
// the set is inconsistent (see Validate) and must not be used, which makes
// the subsequent extraction helpers safe to treat as infallible.
func NewComponents(components ...*Component) (Components, error) {
...
func (cs Components) Rules() *RuleSet {
	sets := make([]*RuleSet, 0, len(cs))
	for _, c := range cs {
		sets = append(sets, c.Rules)
	}
