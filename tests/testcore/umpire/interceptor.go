...
// stepTSetter contract used by Model.setT).
...
// SetStepT pushes the current rapid step into every strategy. Implements
// stepTSetter so Model.setT picks it up via embedding.
func (i *Interceptor) SetStepT(t *T) {
	for _, s := range i.strategies {
		if s.SetT != nil {
			s.SetT(t)
		}
