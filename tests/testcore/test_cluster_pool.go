...
func (p *clusterPool) get(
	t *testing.T,
	dedicated bool,
	dynamicConfig map[dynamicconfig.Key]any,
	fxOptionsByService map[primitives.ServiceName][]fx.Option,
) *FunctionalTestBase {
	if dedicated || len(dynamicConfig) > 0 || len(fxOptionsByService) > 0 {
		return p.getDedicated(t, dynamicConfig, fxOptionsByService)
	}
...
func (p *clusterPool) getShared(t *testing.T) *FunctionalTestBase {
	return p.shared.get(t, func() *FunctionalTestBase {
		return p.createCluster(t, nil, nil, true)
	})
}

func (p *clusterPool) getDedicated(
	t *testing.T,
	dynamicConfig map[dynamicconfig.Key]any,
	fxOptionsByService map[primitives.ServiceName][]fx.Option,
) *FunctionalTestBase {
	if len(dynamicConfig) > 0 || len(fxOptionsByService) > 0 {
		// Custom dynamic config or fx options requires a fresh cluster (can't reuse).
		p.dedicated.acquireSlot(t)
		cluster := p.createCluster(t, dynamicConfig, fxOptionsByService, false)
	...
	// If no custom dynamic config or fx options are provided, reuse an existing cluster.
	return p.dedicated.get(t, func() *FunctionalTestBase {
		return p.createCluster(t, nil, nil, false)
	})
}

func (p *clusterPool) createCluster(
	t *testing.T,
	dynamicConfig map[dynamicconfig.Key]any,
	fxOptionsByService map[primitives.ServiceName][]fx.Option,
	shared bool,
	...
	for serviceName, fxOpts := range fxOptionsByService {
		opts = append(opts, WithFxOptionsForService(serviceName, fxOpts...))
	}
