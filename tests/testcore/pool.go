...
type sharedBasePool struct {
	...
	creating int // Number of bases currently being created
}
...
func (cp *sharedBasePool) acquire(t *testing.T, dynamicConfig map[dynamicconfig.Key]any) *FunctionalTestBase {
	...
	for {
		...
		// If pool isn't full (including bases being created), create a new base.
		// We track creating count to prevent multiple goroutines from racing
		// to create more bases than maxPoolSize.
		if len(cp.bases)+cp.creating < maxPoolSize {
			cp.creating++
			cp.mu.Unlock()
