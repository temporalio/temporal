...
func (s *standaloneActivityTestSuite) TestPauseActivityExecution() {
	...
	t.Run("WorkflowActivity", func(t *testing.T) {
		...
		workflowFn := s.makeWorkflowFuncForNewAPITests(activityFn, &temporal.RetryPolicy{
		...
		wfID := testcore.RandomizeStr(t.Name())
		...
		// Unblock the running activity instance so the workflow can eventually finish.
...
func (s *standaloneActivityTestSuite) TestUnpauseActivityExecution() {
	...
	t.Run("WorkflowActivity", func(t *testing.T) {
		...
		// Wait for activity to start, then pause it via the old API.
		s.WaitForChannel(ctx, activityStartedCh)
		...
		// Unblock the first activity run so it fails and the activity becomes paused.
		close(activityBlockCh)
...
func (s *standaloneActivityTestSuite) TestResetActivityExecution() {
	...
	t.Run("WorkflowActivity", func(t *testing.T) {
		...
		// Wait for activity to fail and accumulate attempts.
		s.EventuallyWithT(func(t *assert.CollectT) {
			desc, err := s.SdkClient().DescribeWorkflowExecution(ctx, workflowRun.GetID(), workflowRun.GetRunID())
			require.NoError(t, err)
			require.Len(t, desc.PendingActivities, 1)
		...
		// Verify attempt count was reset to 1.
		s.EventuallyWithT(func(t *assert.CollectT) {
			desc, err := s.SdkClient().DescribeWorkflowExecution(ctx, workflowRun.GetID(), workflowRun.GetRunID())
			require.NoError(t, err)
			require.Len(t, desc.PendingActivities, 1)
...
func (s *standaloneActivityTestSuite) TestUpdateActivityExecutionOptions() {
	...
	t.Run("WorkflowActivity", func(t *testing.T) {
		...
		// Wait for activity to fail (first attempt).
		s.EventuallyWithT(func(t *assert.CollectT) {
			desc, err := s.SdkClient().DescribeWorkflowExecution(ctx, workflowRun.GetID(), workflowRun.GetRunID())
			require.NoError(t, err)
			require.Len(t, desc.PendingActivities, 1)
		...
		// Shorten retry interval via the new UpdateActivityExecutionOptions API.
		...
		// Activity should retry quickly and eventually start a second time.
		s.EventuallyWithT(func(t *assert.CollectT) {
