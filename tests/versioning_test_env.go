...
func (env *VersioningTestEnv) doPollWftAndHandle(
	...
	go func() {
		_, _ = f() // errors are surfaced via the test context timeout in Rcv
		close(async)
	}()
	return nil, nil
}
...
func (env *VersioningTestEnv) doPollActivityAndHandle(
	...
	if async == nil {
		...
		go func() {
			_ = f() // errors are surfaced via the test context timeout in Rcv
			close(async)
		}()
	}
}
