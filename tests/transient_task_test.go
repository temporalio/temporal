...
func TestTransientTask(t *testing.T) {
	t.Run("TransientWorkflowTaskTimeout", func(t *testing.T) {
		s := testcore.NewEnv(t)
	...
	t.Run("TransientWorkflowTaskHistorySize", func(t *testing.T) {
		s := testcore.NewEnv(t)
		...
		wtHandler := func(task *workflowservice.PollWorkflowTaskQueueResponse) ([]*commandpb.Command, error) {
			...
			require.Equal(t, event.GetEventType(), enumspb.EVENT_TYPE_WORKFLOW_TASK_STARTED)
			...
			switch stage {
			case 1:
				require.Less(t, attrs.HistorySizeBytes, int64(1024*1024))
				require.False(t, attrs.SuggestContinueAsNew)
			...
			case 2:
				require.Greater(t, attrs.HistorySizeBytes, int64(1024*1024))
				require.False(t, attrs.SuggestContinueAsNew)
			...
			case 3:
				require.Greater(t, attrs.HistorySizeBytes, int64(2048*1024))
				require.True(t, attrs.SuggestContinueAsNew)
			...
			case 4:
				...
				require.GreaterOrEqual(t, attrs.HistorySizeBytes, failedTaskSawSize)
				require.Less(t, attrs.HistorySizeBytes, failedTaskSawSize+10000)
				require.False(t, attrs.SuggestContinueAsNew)
				sawFields = append(sawFields, fields{size: attrs.HistorySizeBytes, suggest: attrs.SuggestContinueAsNew})
				return nil, nil
			...
			case 5:
				...
				require.Greater(t, attrs.HistorySizeBytes, prevSize)
				require.Less(t, attrs.HistorySizeBytes, prevSize+10000)
				require.False(t, attrs.SuggestContinueAsNew) // now false
 ...
 25 WorkflowExecutionCompleted`, sawFieldsFlat...), allEvents)
	...
	t.Run("NoTransientWorkflowTaskAfterFlushBufferedEvents", func(t *testing.T) {
		s := testcore.NewEnv(t)
