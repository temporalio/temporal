...
func newTaskQueueStatsContext(
	...
	opts := []testcore.TestOption{
		testcore.WithWorkerService("task queue stats"),
...
func TestTaskQueueStats_Pri_Suite(t *testing.T) {
	parallelsuite.RunLegacySequential(t, &TaskQueueStatsSuite{}, true) //nolint:staticcheck // SA1019: suite runs sequentially to limit worker-service cluster churn.
}
...
func (s *TaskQueueStatsSuite) TestVersioningSuite(usePriMatcher bool) {
	for _, behavior := range testcore.AllMatchingBehaviors() {
		s.T().Run(behavior.Name()+"Suite", func(t *testing.T) { //nolint:testifylint // nested parallelsuite.Run needs raw *testing.T
			parallelsuite.RunLegacySequential(t, &TaskQueueStatsVersionSuite{}, usePriMatcher, behavior) //nolint:staticcheck // SA1019: suite runs sequentially to limit worker-service cluster churn.
		})
	}
}
