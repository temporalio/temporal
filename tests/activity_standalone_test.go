...
// A mutation retry pinned to a closed run must not affect a replacement run with the same activity ID.
func (s *standaloneActivityTestSuite) TestRunQualifiedMutationRetry() {
	env := s.newTestEnv()

	testCases := []struct {
		name        string
		closesRun   bool
		newMutation func(t *testing.T, activityID string, runID string) func(context.Context) error
	}{
		{
			name:      "Cancel",
			closesRun: true,
			newMutation: func(t *testing.T, activityID string, runID string) func(context.Context) error {
				...
				return func(ctx context.Context) error {
					_, err := env.FrontendClient().RequestCancelActivityExecution(ctx, req)
					return err
				}
			},
		},
		{
			...
			newMutation: func(t *testing.T, activityID string, runID string) func(context.Context) error {
				...
				return func(ctx context.Context) error {
					_, err := env.FrontendClient().TerminateActivityExecution(ctx, req)
					return err
				}
			},
		},
		{
			...
			newMutation: func(t *testing.T, activityID string, runID string) func(context.Context) error {
				...
				return func(ctx context.Context) error {
					_, err := env.FrontendClient().PauseActivityExecution(ctx, req)
					return err
				}
			},
		},
	...
	for _, tc := range testCases {
		s.Run(tc.name, func(s *standaloneActivityTestSuite) {
			...
			firstStart := env.startAndValidateActivity(s.Context(), t, activityID, taskQueue)
			mutateFirstRun := tc.newMutation(t, activityID, firstStart.GetRunId())
			require.NoError(t, mutateFirstRun(s.Context()))

			if !tc.closesRun {
				_, err := env.FrontendClient().TerminateActivityExecution(s.Context(), &workflowservice.TerminateActivityExecutionRequest{
					Namespace:  env.Namespace().String(),
					ActivityId: activityID,
					RunId:      firstStart.GetRunId(),
					Identity:   "terminator",
					RequestId:  testcore.RandomizeStr(t.Name() + "-close"),
					Reason:     "Close first run",
				})
				require.NoError(t, err)
			}

			replacementStart, err := env.FrontendClient().StartActivityExecution(s.Context(), &workflowservice.StartActivityExecutionRequest{
				Namespace:           env.Namespace().String(),
				ActivityId:          activityID,
				ActivityType:        env.Tv().ActivityType(),
				Identity:            env.Tv().WorkerIdentity(),
				...
				RequestId:           testcore.RandomizeStr(t.Name() + "-replacement"),
			})
			require.NoError(t, err)
			require.NotEqual(t, firstStart.GetRunId(), replacementStart.GetRunId())

			require.NoError(t, mutateFirstRun(s.Context()))

			replacement, err := env.FrontendClient().DescribeActivityExecution(s.Context(), &workflowservice.DescribeActivityExecutionRequest{
				Namespace:  env.Namespace().String(),
				ActivityId: activityID,
				RunId:      replacementStart.GetRunId(),
			})
			require.NoError(t, err)
			require.Equal(t, enumspb.ACTIVITY_EXECUTION_STATUS_RUNNING, replacement.GetInfo().GetStatus())
			require.Equal(t, enumspb.PENDING_ACTIVITY_STATE_SCHEDULED, replacement.GetInfo().GetRunState())
		})
	}
}
