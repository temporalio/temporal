...
// TestRetryableFailureTruncation verifies that an oversized failure is truncated before it is
// retained for the next attempt, while the terminal failure is reported back to the caller in full.
func (s *standaloneActivityTestSuite) TestRetryableFailureTruncation() {
	env := s.newTestEnv()
	t := s.T()

	const failureSizeLimit = 1024
	cleanup := env.GetTestCluster().OverrideDynamicConfig(
		t,
		dynamicconfig.MutableStateActivityFailureSizeLimitError,
		[]dynamicconfig.ConstrainedValue{{
			Constraints: dynamicconfig.Constraints{Namespace: env.Namespace().String()},
			Value:       failureSizeLimit,
		}},
	)
	defer cleanup()

	oversizedFailure := func() *failurepb.Failure {
		activityFailure := &failurepb.Failure{
			...
			FailureInfo: &failurepb.Failure_ApplicationFailureInfo{ApplicationFailureInfo: &failurepb.ApplicationFailureInfo{
				...
				NonRetryable: false,
				Details:      payloads.EncodeBytes(make([]byte, failureSizeLimit*2)),
			}},
		}
		require.Greater(t, activityFailure.Size(), failureSizeLimit)
	...
	// Attempt 1: fail retryably with a failure that exceeds the limit.
	...
	// The failure retained for the retry is replaced by a truncated server failure.
	describeResp, err := env.FrontendClient().DescribeActivityExecution(s.Context(), &workflowservice.DescribeActivityExecutionRequest{
		Namespace:          env.Namespace().String(),
		ActivityId:         activityID,
		RunId:              startResp.RunId,
	...
	require.NoError(t, err)
	retainedFailure := describeResp.GetInfo().GetLastFailure()
	require.NotNil(t, retainedFailure)
	require.LessOrEqual(t, retainedFailure.Size(), failureSizeLimit)
	...
	require.Equal(t, "activity failure with large details", retainedFailure.GetCause().GetMessage())
	...
	// Attempt 2 is the last one, so its failure is terminal and must be reported back in full.
	pollResp2, err := env.pollActivityTaskQueue(s.Context(), taskQueue)
	require.NoError(t, err)
	require.EqualValues(t, 2, pollResp2.Attempt)
	terminalFailure := oversizedFailure()
	_, err = env.FrontendClient().RespondActivityTaskFailed(s.Context(), &workflowservice.RespondActivityTaskFailedRequest{
		Namespace: env.Namespace().String(),
	...
	require.NoError(t, err)
	protorequire.ProtoEqual(t, terminalFailure, pollActivityResp.GetOutcome().GetFailure())
}
