...
func Compile(declaration Declaration) (*Protocol, error) {
	...
	for i, fact := range declaration.Facts {
		...
		if err != nil {
			return nil, fmt.Errorf("protocolv2: fact %d: %w", i, err)
		}
		if _, exists := factTypes[factType]; exists {
			return nil, fmt.Errorf("protocolv2: duplicate fact type %s", factType)
		}
		factTypes[factType] = struct{}{}
		if fact.Name() != factName {
			return nil, fmt.Errorf("protocolv2: fact name %q does not match concrete type %q", fact.Name(), factName)
		}
	}

	for _, entity := range declaration.Entities {
		if _, exists := protocol.entities[entity.Type]; exists {
			return nil, fmt.Errorf("protocolv2: duplicate entity type %q", entity.Type)
		}
		if entity.New == nil {
			return nil, fmt.Errorf("protocolv2: entity %q has nil factory", entity.Type)
		}
		probe, err := callEntityFactory(entity.New)
		if err != nil {
			return nil, fmt.Errorf("protocolv2: entity %q: %w", entity.Type, err)
		}
		if probe.Type() != entity.Type {
			return nil, fmt.Errorf("protocolv2: entity factory returns type %q but declares type %q", probe.Type(), entity.Type)
		}
		...
		if string(entity.Type) != concreteName {
			return nil, fmt.Errorf("protocolv2: entity %q does not match concrete type %q", entity.Type, concreteName)
		}
		for _, subscription := range entity.Facts {
			subscriptionType, _, err := declaredFactType(subscription)
			if err != nil {
				return nil, fmt.Errorf("protocolv2: entity %q subscription: %w", entity.Type, err)
			}
			if _, registered := factTypes[subscriptionType]; !registered {
				return nil, fmt.Errorf("protocolv2: entity %q subscription %s is absent from the fact set", entity.Type, subscriptionType)
			}
		}
		...
		if lifecycled, ok := probe.(umpire.Lifecycled); ok {
			...
			if lifecycle == nil {
				return nil, fmt.Errorf("protocolv2: entity %q has nil lifecycle", entity.Type)
			}
			if err := lifecycle.Validate(); err != nil {
				return nil, fmt.Errorf("protocolv2: entity %q lifecycle: %w", entity.Type, err)
			}
		}
...
func (p *Protocol) addAction(entityType umpire.EntityType, lifecycle *umpire.Lifecycle, binding ActionBinding) error {
	...
	if key.Entity != entityType {
		return fmt.Errorf("protocolv2: entity %q contains action entity %q", entityType, key.Entity)
	}
	if lifecycle == nil {
		return fmt.Errorf("protocolv2: action %v belongs to entity %q, which is not lifecycled", key, entityType)
	}
	if !isConcreteHosting(key.Hosting) {
		return fmt.Errorf("protocolv2: action %v requires concrete hosting: Standalone or Embedded", key)
	}
	if !lifecycleHasEdge(lifecycle, key.From, key.Event) {
		return fmt.Errorf("protocolv2: action %v references unknown edge", key)
	}
	if _, exists := p.actions[key]; exists {
		return fmt.Errorf("protocolv2: duplicate action key %v", key)
	}
	if edgeHosting := lifecycle.EdgeHosting(key.From, key.Event); edgeHosting != umpire.AnyHosting && edgeHosting != key.Hosting {
		return fmt.Errorf("protocolv2: action %v has %s edge hosting", key, edgeHosting)
	}
	if actionHosting := binding.Action.Hosting; actionHosting != umpire.AnyHosting && actionHosting != key.Hosting {
		return fmt.Errorf("protocolv2: action hosting %s does not match key %v", actionHosting, key)
	}
	if !actionHasEffect(binding.Action, key.Entity, key.Event) {
		return fmt.Errorf("protocolv2: action %v has no matching effect", key)
	}
...
func (p *Protocol) addGap(entityType umpire.EntityType, lifecycle *umpire.Lifecycle, gap ActionGap) error {
	key := gap.Key
	if key.Entity != entityType {
		return fmt.Errorf("protocolv2: entity %q contains gap entity %q", entityType, key.Entity)
	}
	if lifecycle == nil {
		return fmt.Errorf("protocolv2: gap %v belongs to entity %q, which is not lifecycled", key, entityType)
	}
	if !isConcreteHosting(key.Hosting) {
		return fmt.Errorf("protocolv2: gap %v requires concrete hosting: Standalone or Embedded", key)
	}
	if !lifecycleHasEdge(lifecycle, key.From, key.Event) {
		return fmt.Errorf("protocolv2: gap %v references unknown edge", key)
	}
	if strings.TrimSpace(gap.Reason) == "" {
		return fmt.Errorf("protocolv2: gap reason is empty for %v", key)
	}
	if _, exists := p.actions[key]; exists {
		return fmt.Errorf("protocolv2: gap %v overlaps action", key)
	}
	if _, exists := p.gaps[key]; exists {
		return fmt.Errorf("protocolv2: duplicate gap key %v", key)
	}
