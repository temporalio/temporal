...
func (p *Protocol) PlanTo(
	...
	if _, ok := p.entities[entityType]; !ok {
		return nil, fmt.Errorf("protocolv2: unknown entity %q", entityType)
	}
	lifecycle, ok := p.Lifecycle(entityType)
	if !ok {
		return nil, fmt.Errorf("protocolv2: entity %q is not lifecycled", entityType)
	}
	plan, err := umpire.PlanTo(lifecycle, target, mode, constraints, options...)
	if err != nil {
		return nil, fmt.Errorf("protocolv2: plan %q to %q: %w", entityType, target, err)
	}
	return plan, nil
}
...
func (p *Protocol) PlanEdge(
	...
	if _, ok := p.entities[entityType]; !ok {
		return nil, fmt.Errorf(
			"protocolv2: unknown entity for edge %s",
			actionContext(entityType, from, event, hosting),
		)
	}
	if !isConcreteHosting(hosting) {
		return nil, fmt.Errorf(
			"protocolv2: edge %s requires concrete hosting: Standalone or Embedded",
			actionContext(entityType, from, event, hosting),
		)
	}
	lifecycle, ok := p.Lifecycle(entityType)
	if !ok {
		return nil, fmt.Errorf(
			"protocolv2: entity is not lifecycled for edge %s",
			actionContext(entityType, from, event, hosting),
		)
	}
	if !lifecycleHasEdge(lifecycle, from, event) {
		return nil, fmt.Errorf("protocolv2: unknown edge %s", actionContext(entityType, from, event, hosting))
	}
	if edgeHosting := lifecycle.EdgeHosting(from, event); edgeHosting != umpire.AnyHosting && edgeHosting != hosting {
		return nil, fmt.Errorf(
			"protocolv2: edge %s is %s-only",
			actionContext(entityType, from, event, hosting),
			edgeHosting,
		)
	}
	...
	for _, routeEvent := range events {
		...
		if reason, gap := p.gaps[key]; gap {
			return nil, fmt.Errorf("protocolv2: no action for %s: %s", actionContext(entityType, state, routeEvent, hosting), reason)
		}
		action, found := p.Action(key)
		if !found {
			return nil, fmt.Errorf("protocolv2: no action for %s", actionContext(entityType, state, routeEvent, hosting))
		}
		actions = append(actions, action)
		destination, found := lifecycleEdgeDestination(lifecycle, state, routeEvent)
		if !found {
			return nil, fmt.Errorf(
				"protocolv2: route contains unknown edge %s",
