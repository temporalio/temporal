...
func TestDefaultMatchesV1EntityAndLifecycleCatalogs(t *testing.T) {
	...
	for _, entity := range model.DefaultEntities() {
		...
		if lifecycled {
			...
			v1Lifecycle, ok := planner.DefaultModels().Lifecycle(string(entityType))
			require.True(t, ok)
			require.Equal(t, lifecycleCatalog(v1Lifecycle), lifecycleCatalog(gotLifecycle))
		}
	}
...
func TestDefaultMatchesV1ModelStateFactSet(t *testing.T) {
	protocol, err := Default()
	require.NoError(t, err)
