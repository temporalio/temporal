...
func (s *ParentClosePolicySuite) TestParentClosePolicyTerminate() {
	...
	tl := "parent-close-policy-terminate-taskqueue"
	identity := "worker1"
...
func (s *ParentClosePolicySuite) TestParentClosePolicyRequestCancel() {
	...
	tl := "parent-close-policy-cancel-taskqueue"
	identity := "worker1"
	...
	combinedHandler := func(task *workflowservice.PollWorkflowTaskQueueResponse) ([]*commandpb.Command, error) {
		if task.WorkflowExecution.WorkflowId == parentID {
	...
	poller := &testcore.TaskPoller{
		...
		TaskQueue:           taskQueue,
		Identity:            identity,
		WorkflowTaskHandler: combinedHandler,
		Logger:              s.Logger,
		T:                   s.T(),
	}
...
func (s *ParentClosePolicySuite) TestParentClosePolicyAbandon() {
	...
	tl := "parent-close-policy-abandon-taskqueue"
	identity := "worker1"
...
func (s *ParentClosePolicySuite) TestParentClosePolicyMultipleChildren() {
	...
	tl := "parent-close-policy-multi-taskqueue"
	identity := "worker1"
	...
	poller := &testcore.TaskPoller{
		...
		TaskQueue:           taskQueue,
		Identity:            identity,
		WorkflowTaskHandler: childHandler,
		Logger:              s.Logger,
		T:                   s.T(),
	}
...
func (s *ParentClosePolicySuite) TestParentClosePolicyChildAlreadyCompleted() {
	...
	tl := "parent-close-policy-completed-taskqueue"
	identity := "worker1"
