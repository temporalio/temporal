...
func (s *WorkerDeploymentSuite) pollDeploymentReturnErr(ctx context.Context, tv *testvars.TestVars) error {
	_, err := s.FrontendClient().PollWorkflowTaskQueue(ctx, &workflowservice.PollWorkflowTaskQueueRequest{
		Namespace:         s.Namespace().String(),
		TaskQueue:         tv.TaskQueue(),
		Identity:          "random",
...
func (s *WorkerDeploymentSuite) TestNamespaceDeployments_AutoCreateRateLimiter_RejectsBurst() {
	...
	s.OverrideDynamicConfig(dynamicconfig.MatchingMaxDeployments, 10000)
	...
	for i := range numNames {
		go func(i int) {
			results <- s.pollDeploymentReturnErr(ctx, tv.WithDeploymentSeriesNumber(i).WithTaskQueueNumber(i))
		}(i)
	}
