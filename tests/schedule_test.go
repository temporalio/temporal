...
func runSharedScheduleTests(t *testing.T, newContext contextFactory) {
	...
	t.Run("TestSystemSearchAttributesNotStoredAsCustom", func(t *testing.T) { t.Parallel(); testSystemSearchAttributesNotStoredAsCustom(t, newContext) })
}
...
func testSystemSearchAttributesNotStoredAsCustom(t *testing.T, newContext contextFactory) {
	...
	assertNoReservedLeak := func(sa *commonpb.SearchAttributes, label string) {
		...
		require.Equal(t, csaValue.Data, fields[csaKeyword].GetData(), "%s: custom SA must be stored", label)
		for k := range fields {
			require.Falsef(t, sadefs.IsReserved(k), "%s: reserved SA %q leaked into custom search attributes", label, k)
		}
	}
	...
	require.Eventually(t, func() bool {
		...
		fields := describeResp.GetSearchAttributes().GetIndexedFields()
		if string(csaValue.Data) != string(fields[csaKeyword].GetData()) {
			return false
		}
