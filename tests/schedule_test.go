...
func newEnvWithIdleTime(t *testing.T, chasmEnabled bool, idleTime time.Duration, extra ...testcore.TestOption) *testcore.TestEnv {
	...
	opts := append([]testcore.TestOption{testcore.WithDynamicConfig(chasmscheduler.CurrentTweakables, tweakables)}, extra...)
	return newScheduleEnv(t, chasmEnabled, opts...)
}
...
func (s *ScheduleCHASMSuite) TestBufferOverrunDropsActions() {
	...
	env := newScheduleEnv(s.T(), true, testcore.WithDynamicConfig(chasmscheduler.CurrentTweakables, tweakables))
	...
	s.AwaitTruef(func() bool {
		...
		return err == nil && desc.GetInfo().GetBufferDropped() > 0 && len(desc.GetInfo().GetRunningWorkflows()) >= 1
	}, awaitTimeout, pollInterval, "expected BufferDropped > 0 with a gated workflow holding the buffer full")
	s.Positive(runs.Load(), "the gated workflow should have started")
	completeRunningWorkflows(s.Context(), s.T(), env, sid)
}

func (s *ScheduleCHASMSuite) TestLargeScheduleID() {
	env := newScheduleEnv(s.T(), true)
	const workflowIDColumnLimit = 255
	sid := strings.Repeat("a", workflowIDColumnLimit-len(scheduler.WorkflowIDPrefix))
	...
	s.AwaitTruef(func() bool { return runs.Load() > 0 }, awaitTimeout, pollInterval,
		"schedule ID of length %d should start a workflow", len(sid))
}

func (s *ScheduleCHASMSuite) TestPauseOnFailureCancelDoesNotPause() {
	s.pauseOnFailureCancelOrTerminate(false)
}

func (s *ScheduleCHASMSuite) TestPauseOnFailureTerminateDoesNotPause() {
	s.pauseOnFailureCancelOrTerminate(true)
}

func (s *ScheduleCHASMSuite) pauseOnFailureCancelOrTerminate(terminate bool) {
	env := newScheduleEnv(s.T(), true)
	...
	s.AwaitTruef(func() bool {
		...
		for _, action := range desc.GetInfo().GetRecentActions() {
			if action.GetStartWorkflowStatus() == enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING && action.GetStartWorkflowResult().GetRunId() != "" {
	...
	if terminate {
		...
		_, err := env.FrontendClient().TerminateWorkflowExecution(s.Context(), &workflowservice.TerminateWorkflowExecutionRequest{
			Namespace: env.Namespace().String(), WorkflowExecution: execution, Reason: s.T().Name(), Identity: "test",
		})
		s.NoError(err)
	} else {
		_, err := env.FrontendClient().RequestCancelWorkflowExecution(s.Context(), &workflowservice.RequestCancelWorkflowExecutionRequest{
			Namespace: env.Namespace().String(), WorkflowExecution: execution, Identity: "test", RequestId: uuid.NewString(),
		})
		s.NoError(err)
	}
	s.Awaitf(func(s *ScheduleCHASMSuite) {
		...
		s.NoError(err)
		for _, action := range desc.GetInfo().GetRecentActions() {
			if action.GetStartWorkflowResult().GetRunId() == execution.GetRunId() && action.GetStartWorkflowStatus() == wantStatus {
				s.False(desc.GetSchedule().GetState().GetPaused())
				return
			}
		}
		s.Fail("terminal action has not been recorded")
	}, awaitTimeout, pollInterval, "canceled and terminated workflows must not pause a PauseOnFailure schedule by default")
}
...
func (s *ScheduleSuite) TestListSchedulesRecentActionsCapped(chasmEnabled bool) {
	...
	registerCountingWorkflow(env, wt, &runs)
	createSchedule(s.Context(), s.T(), env, sid, &schedulepb.Schedule{
		Spec: intervalSpec(fastInterval), Action: startWorkflowAction(env, wid, wt),
	})
	...
	s.AwaitTruef(func() bool {
		...
		return err == nil && len(desc.GetInfo().GetRecentActions()) > memoCap
	}, awaitTimeout, pollInterval, "DescribeSchedule should report more than %d recent actions", memoCap)

	var entry *schedulepb.ScheduleListEntry
	s.AwaitTruef(func() bool {
		...
		for _, candidate := range resp.GetSchedules() {
			if candidate.GetScheduleId() == sid {
				entry = candidate
				return len(candidate.GetInfo().GetRecentActions()) >= memoCap
			}
		}
		return false
	}, awaitTimeout, pollInterval, "ListSchedules should contain the schedule")
	s.Len(entry.GetInfo().GetRecentActions(), memoCap)
}
...
func (s *ScheduleSuite) TestScheduleContinuesAfterWorkflowRetryFailure(chasmEnabled bool) {
	...
	env := newScheduleEnv(s.T(), chasmEnabled,
		testcore.WithDynamicConfig(callback.EncodeInternalTokenWithEnvelope, true),
	)
...
func (s *ScheduleSuite) TestListSchedulesFilterByScheduleId(chasmEnabled bool) {
	...
	for _, tc := range tests {
		s.Run(tc.name, func(s *ScheduleSuite) {
			s.Await(func(s *ScheduleSuite) {
...
func (s *ScheduleSuite) TestSchedule_InternalTaskQueue(chasmEnabled bool) {
	...
	s.Run("CreateSchedule_PerNSWorkerTaskQueue", func(s *ScheduleSuite) {
		...
		s.Require().Contains(err.Error(), errorMessageKeyword)
	})

	// Test UpdateSchedule with internal task queue
	s.Run("UpdateSchedule_PerNSWorkerTaskQueue", func(s *ScheduleSuite) {
...
func (s *ScheduleCHASMSuite) resetWithAdditionalCallback(enableCHASMCallbacks bool) {
	...
	completion := await.Rcv(s.T(), ch.requestCh)
...
func (s *ScheduleCHASMSuite) TestScheduleCreationRolloutPercent() {
	...
	// Passing false to newScheduleEnv provides the worker without forcing CHASM routing.
	env := newScheduleEnv(s.T(), false,
	...
	s.Awaitf(func(s *ScheduleCHASMSuite) {
		s.NoError(describeOnCHASM(s.Context(), chasmSID))
	}, 15*time.Second, 250*time.Millisecond,
		"schedule %q bucketed into CHASM should be describable via the CHASM handler", chasmSID)

	var notFoundErr *serviceerror.NotFound
	s.ErrorAs(describeOnCHASM(s.Context(), v1SID), &notFoundErr,
...
// TestScheduleClosesFromIdle runs the idle-close matrix as subtests.
func (s *ScheduleCHASMSuite) TestScheduleClosesFromIdle() {
	...
	for _, c := range cases {
		s.Run(c.name, func(s *ScheduleCHASMSuite) {
			// Drive a schedule whose spec exhausts itself, then assert (1) the expected
			// number of runs fire, (2) FutureActionTimes drains to zero, and (3) the
			// schedule closes after IdleTime.
			env := newEnvWithIdleTime(s.T(), true, shortIdleTime)
			...
			s.Awaitf(func(s *ScheduleCHASMSuite) {
				resp, err := env.FrontendClient().DescribeSchedule(s.Context(), &workflowservice.DescribeScheduleRequest{
					Namespace:  env.Namespace().String(),
					ScheduleId: sid,
				})
				s.NoError(err)
				s.Empty(resp.Info.FutureActionTimes)
...
func (s *ScheduleCHASMSuite) TestMirroredIncludeExcludeSpec() {
	...
	s.Error(lmErr, "ListMatchingTimes should error for a mirrored include/exclude spec")
}
...
func (s *ScheduleCHASMSuite) TestMirroredIncludeExcludeSpecOnUpdate() {
	...
	everySecond := &schedulepb.CalendarSpec{Second: "*", Minute: "*", Hour: "*"}
	updateCtx, cancel := context.WithTimeout(s.Context(), 10*time.Second)
	defer cancel()
	...
	s.NoError(err, "UpdateSchedule to a mirrored include/exclude spec should not hang")

	// Once the mirrored spec is applied, ListMatchingTimes must surface the compute limit.
	s.Awaitf(func(s *ScheduleCHASMSuite) {
		_, lmErr := env.FrontendClient().ListScheduleMatchingTimes(s.Context(), &workflowservice.ListScheduleMatchingTimesRequest{
			Namespace:  env.Namespace().String(),
			ScheduleId: sid,
			StartTime:  timestamppb.New(time.Now().UTC()),
			EndTime:    timestamppb.New(time.Now().UTC().Add(time.Hour)),
...
func (s *ScheduleCHASMSuite) TestScheduleFarFutureActionTimes() {
	...
	s.Awaitf(func(s *ScheduleCHASMSuite) {
		...
		s.NoError(err)
		s.GreaterOrEqual(len(resp.GetInfo().GetFutureActionTimes()), 10)
	...
	for i := 1; i < len(future); i++ {
		s.Equal(interval, future[i].AsTime().Sub(future[i-1].AsTime()),
			"consecutive future action times should be exactly one interval apart")
	}
	s.Greater(future[len(future)-1].AsTime().Sub(time.Now().UTC()), warn,
	...
	s.NoError(err)
	s.GreaterOrEqual(len(resp.GetStartTime()), 10,
...
func (s *ScheduleCHASMSuite) TestScheduleManyCalendars() {
	...
	describe := func(s *ScheduleCHASMSuite) *schedulepb.ScheduleInfo {
		resp, err := env.FrontendClient().DescribeSchedule(s.Context(), &workflowservice.DescribeScheduleRequest{
			Namespace:  env.Namespace().String(),
			ScheduleId: sid,
		})
	...
	s.Awaitf(func(s *ScheduleCHASMSuite) {
		info := describe(s)
		s.NotEmpty(info.GetRecentActions())
		s.NotEmpty(info.GetFutureActionTimes())
	...
	s.Awaitf(func(s *ScheduleCHASMSuite) {
		...
		s.True(newerRecent && advancedFuture)
	...
	s.NoError(err)
	s.Empty(resp.GetStartTime(), "the excluded minute must have no matches")
}
...
func (s *ScheduleCHASMSuite) TestScheduleCountsVisibility() {
	...
	s.AwaitTruef(func() bool {
		return matchesQuery(s.Context(), fmt.Sprintf("%s >= 1", chasmscheduler.ScheduleRunningWorkflowCountName))
	}, 30*time.Second, 500*time.Millisecond,
		"schedule must be queryable by ScheduleRunningWorkflowCount >= 1")

	s.AwaitTruef(func() bool {
		return matchesQuery(s.Context(), fmt.Sprintf("%s >= 1", chasmscheduler.ScheduleBufferedStartsCountName))
	}, 30*time.Second, 500*time.Millisecond,
