...
func TestScheduleV1(t *testing.T) {
	...
	t.Run("TestActionDelayMetrics", func(t *testing.T) { t.Parallel(); testActionDelayMetrics(t, newContext) })
...
func testActionDelayMetrics(t *testing.T, newContext contextFactory) {
	...
	require.Equal(t, actions[1].GetActualTime().AsTime().Sub(closeTime(actions[0])), recordings[0].Value)
	require.Equal(t, actions[2].GetActualTime().AsTime().Sub(closeTime(actions[1])), recordings[1].Value)
}
