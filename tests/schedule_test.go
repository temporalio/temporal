...
func (s *ScheduleFunctionalSuite) TestBasics() {
	...
	s.Nil(describeResp.SearchAttributes.IndexedFields[defs.BinaryChecksums])
	s.Nil(describeResp.SearchAttributes.IndexedFields[defs.BuildIds])
	s.Nil(describeResp.SearchAttributes.IndexedFields[defs.TemporalNamespaceDivision])
	...
	s.Equal(schSAIntValue.Data, describeResp.SearchAttributes.IndexedFields[csaInt].Data)
	s.Equal(schSABoolValue.Data, describeResp.SearchAttributes.IndexedFields[csaBool].Data)
	s.Nil(visibilityResponse.SearchAttributes.IndexedFields[defs.BinaryChecksums])
	s.Nil(visibilityResponse.SearchAttributes.IndexedFields[defs.BuildIds])
	s.Nil(visibilityResponse.SearchAttributes.IndexedFields[defs.TemporalNamespaceDivision])
	...
	s.Equal(payload.EncodeString(sid).Data, ex0.SearchAttributes.IndexedFields[defs.TemporalScheduledById].Data)
	var ex0StartTime time.Time
	s.NoError(payload.Decode(ex0.SearchAttributes.IndexedFields[defs.TemporalScheduledStartTime], &ex0StartTime))
	...
	wfResp, err = s.FrontendClient().ListWorkflowExecutions(testcore.NewContext(), &workflowservice.ListWorkflowExecutionsRequest{
		Namespace: s.Namespace().String(),
		PageSize:  5,
		Query:     defs.QueryWithAnyNamespaceDivision(`ExecutionStatus = "Running"`),
	})
	s.NoError(err)
	count := 0
	for _, ex := range wfResp.Executions {
	...
	wfResp, err = s.FrontendClient().ListWorkflowExecutions(testcore.NewContext(), &workflowservice.ListWorkflowExecutionsRequest{
		Namespace: s.Namespace().String(),
		PageSize:  5,
		Query:     fmt.Sprintf("%s = '%s'", defs.TemporalNamespaceDivision, scheduler.NamespaceDivision),
	})
	s.NoError(err)
