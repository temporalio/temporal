...
func testScheduleVersionCeiling(t *testing.T, newContext contextFactory) {
	...
	// The persisted "tweakables" markers must record the clamped version. This attributes the
	// clamp to the workflow layer (the memo assertion above is also satisfiable by the
	// frontend's creation-time clamp alone) and pins the actual server-persisted wire shape:
	// json/plain payloads with Version=7, which an older peer decodes on replay. The first
	// marker is recorded before any action, so the action-count gate above already guarantees
	// it is persisted; a single fetch suffices.
	obs, err := versionguard.Observe(s.GetHistory(s.Namespace().String(),
		&commonpb.WorkflowExecution{WorkflowId: scheduler.WorkflowIDPrefix + sid}))
	require.NoError(t, err)
	require.NotEmpty(t, obs.TweakableVersions, "workflow history must record tweakables markers")
	for i, v := range obs.TweakableVersions {
		require.Equal(t, int64(7), v, "tweakables marker %d must record the clamped version", i)
		require.Equal(t, "json/plain", obs.TweakableEncodings[i], "tweakables marker %d must be json/plain", i)
	}
}

// tweakablesVersionsFromHistory returns the recorded TweakablePolicies.Version from every
// "tweakables" marker, via versionguard.Observe (the same projection the gate guard uses).
func tweakablesVersionsFromHistory(events []*historypb.HistoryEvent) []int64 {
	obs, err := versionguard.Observe(events)
	if err != nil {
		return nil
	}
	return obs.TweakableVersions
}
...
func testScheduleVersionCeilingAppearsMidRun(t *testing.T, newContext contextFactory) {
	...
	trigger()
	require.Eventually(t, containsVersion(int64(scheduler.CurrentTweakablePolicies.Version)), 15*time.Second, 500*time.Millisecond,
...
func testScheduleVersionCeilingMemoSize(t *testing.T, newContext contextFactory) {
	...
	// Large enough that the untrimmed list-info memo exceeds the lowered 2KB limit while
