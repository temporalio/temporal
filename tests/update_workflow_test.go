	...
	t.Run("StickySpeculativeWorkflowTask_AcceptComplete", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				//nolint:testifylint // Assertions in goroutine are intentional for concurrent sticky task polling
				go func() {
...
`
  ...
  4 WorkflowTaskCompleted`, s.GetHistory(s.Namespace().String(), tv.WorkflowExecution()))
	...
	t.Run("ContinueAsNew_Suggestion", func(t *testing.T) {
		...
		s := NewUpdateTestEnv(t, map[dynamicconfig.Key]any{
	...
	t.Run("UpdateWithStart", func(t *testing.T) {
		...
		sendUpdateWithStart := func(s *UpdateTestEnv, ctx context.Context, startReq *workflowservice.StartWorkflowExecutionRequest, updateReq *workflowservice.UpdateWorkflowExecutionRequest) chan multiopsResponseErr {
			...
			//nolint:testifylint // Assertions in goroutine are intentional for concurrent multi-operation execution
			go func() {
		...
		t.Run("workflow start conflict", func(t *testing.T) {
			t.Run("workflow id conflict policy fail: use-existing", func(t *testing.T) {
				// Use dedicated cluster because InjectHook modifies cluster-wide state.
				s := NewUpdateTestEnv(t, map[dynamicconfig.Key]any{
