...
VARIABLES
    ...
    \* @type: Set(<<Str, Str>>);
    relation_activity_delivery_task,
    \* @type: Set(<<Str, Str>>);
    relation_activity_obligation,
    \* @type: Set(<<Str, Str>>);
...
vars == <<exists_Activity, state_Activity, exists_DeliveryAttempt, state_DeliveryAttempt, exists_DeliveryQueue, state_DeliveryQueue, exists_DeliveryTask, state_DeliveryTask, exists_Poller, state_Poller, exists_WorkObligation, state_WorkObligation, relation_activity_delivery_task, relation_activity_obligation, relation_delivery_accepted_start, relation_delivery_attempt_poller, relation_delivery_attempt_task, relation_delivery_task_obligation, relation_delivery_task_queue>>
...
TypeOK ==
    ...
    /\ relation_activity_delivery_task \in SUBSET (ActivityIDs \X DeliveryTaskIDs)
    /\ relation_activity_obligation \in SUBSET (ActivityIDs \X WorkObligationIDs)
...
Cardinality_activity_delivery_task ==
    /\ \A tuple \in relation_activity_delivery_task: tuple[1] \in exists_Activity /\ tuple[2] \in exists_DeliveryTask
    /\ \A source \in ActivityIDs: Cardinality({target \in DeliveryTaskIDs: <<source, target>> \in relation_activity_delivery_task}) <= 1
    /\ \A target \in DeliveryTaskIDs: Cardinality({source \in ActivityIDs: <<source, target>> \in relation_activity_delivery_task}) <= 1

Cardinality_activity_obligation ==
    /\ \A tuple \in relation_activity_obligation: tuple[1] \in exists_Activity /\ tuple[2] \in exists_WorkObligation
    /\ \A source \in ActivityIDs: Cardinality({target \in WorkObligationIDs: <<source, target>> \in relation_activity_obligation}) <= 1
    /\ \A target \in WorkObligationIDs: Cardinality({source \in ActivityIDs: <<source, target>> \in relation_activity_obligation}) <= 1
...
Activity_backing_off_cancel_AnyHosting(entity) ==
    ...
    /\ UNCHANGED <<exists_Activity, exists_DeliveryAttempt, state_DeliveryAttempt, exists_DeliveryQueue, state_DeliveryQueue, exists_DeliveryTask, state_DeliveryTask, exists_Poller, state_Poller, exists_WorkObligation, state_WorkObligation, relation_activity_delivery_task, relation_activity_obligation, relation_delivery_accepted_start, relation_delivery_attempt_poller, relation_delivery_attempt_task, relation_delivery_task_obligation, relation_delivery_task_queue>>
...
Activity_delivery_authorizeEnabled(entity, obligation, task, attempt) ==
    /\ entity \in ActivityIDs
    /\ entity \in exists_Activity
    ...
    /\ (state_Activity[entity] = "scheduled" /\ <<entity, obligation>> \in relation_activity_obligation /\ <<entity, task>> \in relation_activity_delivery_task /\ (state_WorkObligation[obligation] = "valid" /\ state_DeliveryTask[task] = "reserved" /\ state_DeliveryAttempt[attempt] = "reserved" /\ <<task, obligation>> \in relation_delivery_task_obligation /\ <<attempt, task>> \in relation_delivery_attempt_task))

Activity_delivery_authorize(entity, obligation, task, attempt) ==
    /\ Activity_delivery_authorizeEnabled(entity, obligation, task, attempt)
    ...
    /\ UNCHANGED <<exists_Activity, exists_DeliveryAttempt, exists_DeliveryQueue, state_DeliveryQueue, exists_DeliveryTask, exists_Poller, state_Poller, exists_WorkObligation, relation_activity_delivery_task, relation_activity_obligation, relation_delivery_attempt_poller, relation_delivery_attempt_task, relation_delivery_task_obligation, relation_delivery_task_queue>>

Activity_delivery_persistEnabled(entity, obligation, task, queue) ==
    /\ entity \in ActivityIDs
...
Activity_delivery_persist(entity, obligation, task, queue) ==
    /\ Activity_delivery_persistEnabled(entity, obligation, task, queue)
    ...
    /\ relation_activity_delivery_task' = relation_activity_delivery_task \union {<<entity, task>>}
    /\ relation_activity_obligation' = relation_activity_obligation \union {<<entity, obligation>>}
...
Activity_delivery_rejectEnabled(entity, obligation, task, attempt) ==
    /\ entity \in ActivityIDs
    /\ entity \in exists_Activity
    /\ obligation \in WorkObligationIDs
    /\ obligation \in exists_WorkObligation
...
Activity_delivery_reject(entity, obligation, task, attempt) ==
    /\ Activity_delivery_rejectEnabled(entity, obligation, task, attempt)
    ...
    /\ UNCHANGED <<exists_Activity, state_Activity, exists_DeliveryAttempt, exists_DeliveryQueue, state_DeliveryQueue, exists_DeliveryTask, exists_Poller, state_Poller, exists_WorkObligation, state_WorkObligation, relation_activity_delivery_task, relation_activity_obligation, relation_delivery_accepted_start, relation_delivery_attempt_poller, relation_delivery_attempt_task, relation_delivery_task_obligation, relation_delivery_task_queue>>

Activity_delivery_resolve_persistedEnabled(obligation, task) ==
    /\ obligation \in WorkObligationIDs
    /\ obligation \in exists_WorkObligation
    /\ task \in DeliveryTaskIDs
    /\ task \in exists_DeliveryTask
...
Activity_delivery_resolve_persisted(obligation, task) ==
    /\ Activity_delivery_resolve_persistedEnabled(obligation, task)
    /\ state_WorkObligation' = [state_WorkObligation EXCEPT ![obligation] = "valid"]
    /\ UNCHANGED <<exists_Activity, state_Activity, exists_DeliveryAttempt, state_DeliveryAttempt, exists_DeliveryQueue, state_DeliveryQueue, exists_DeliveryTask, state_DeliveryTask, exists_Poller, state_Poller, exists_WorkObligation, relation_activity_delivery_task, relation_activity_obligation, relation_delivery_accepted_start, relation_delivery_attempt_poller, relation_delivery_attempt_task, relation_delivery_task_obligation, relation_delivery_task_queue>>

Activity_delivery_retryEnabled(entity, previous_obligation, previous_task, obligation, task, queue) ==
    /\ entity \in ActivityIDs
    /\ entity \in exists_Activity
    /\ previous_obligation \in WorkObligationIDs
    /\ previous_obligation \in exists_WorkObligation
    /\ previous_task \in DeliveryTaskIDs
    /\ previous_task \in exists_DeliveryTask
    /\ obligation \in WorkObligationIDs
    /\ obligation \notin exists_WorkObligation
    /\ task \in DeliveryTaskIDs
    /\ task \notin exists_DeliveryTask
    /\ queue \in DeliveryQueueIDs
    /\ queue \in exists_DeliveryQueue
    /\ (state_Activity[entity] = "backing_off" /\ <<entity, previous_obligation>> \in relation_activity_obligation /\ <<entity, previous_task>> \in relation_activity_delivery_task)

Activity_delivery_retry(entity, previous_obligation, previous_task, obligation, task, queue) ==
    /\ Activity_delivery_retryEnabled(entity, previous_obligation, previous_task, obligation, task, queue)
    /\ state_Activity' = [state_Activity EXCEPT ![entity] = "scheduled"]
    /\ exists_DeliveryTask' = exists_DeliveryTask \union {task}
    /\ state_DeliveryTask' = [state_DeliveryTask EXCEPT ![task] = "pending"]
    /\ exists_WorkObligation' = exists_WorkObligation \union {obligation}
    /\ state_WorkObligation' = [state_WorkObligation EXCEPT ![obligation] = "valid"]
    /\ relation_activity_delivery_task' = (relation_activity_delivery_task) \ {<<entity, previous_task>>} \union {<<entity, task>>}
    /\ relation_activity_obligation' = (relation_activity_obligation) \ {<<entity, previous_obligation>>} \union {<<entity, obligation>>}
    /\ relation_delivery_task_obligation' = relation_delivery_task_obligation \union {<<task, obligation>>}
    /\ relation_delivery_task_queue' = relation_delivery_task_queue \union {<<task, queue>>}
    /\ UNCHANGED <<exists_Activity, exists_DeliveryAttempt, state_DeliveryAttempt, exists_DeliveryQueue, state_DeliveryQueue, exists_Poller, state_Poller, relation_delivery_accepted_start, relation_delivery_attempt_poller, relation_delivery_attempt_task>>
...
Delivery_persist_ambiguous(obligation, task, queue) ==
    ...
    /\ relation_delivery_task_obligation' = relation_delivery_task_obligation \union {<<task, obligation>>}
    /\ relation_delivery_task_queue' = relation_delivery_task_queue \union {<<task, queue>>}
    /\ UNCHANGED <<exists_Activity, state_Activity, exists_DeliveryAttempt, state_DeliveryAttempt, exists_DeliveryQueue, state_DeliveryQueue, exists_Poller, state_Poller, relation_activity_delivery_task, relation_activity_obligation, relation_delivery_accepted_start, relation_delivery_attempt_poller, relation_delivery_attempt_task>>
...
Delivery_reserve(task, attempt, poller) ==
    ...
    /\ UNCHANGED <<exists_Activity, state_Activity, exists_DeliveryQueue, state_DeliveryQueue, exists_DeliveryTask, exists_Poller, state_Poller, exists_WorkObligation, state_WorkObligation, relation_activity_delivery_task, relation_activity_obligation, relation_delivery_accepted_start, relation_delivery_task_obligation, relation_delivery_task_queue>>
...
Next ==
    ...
    \/ \E entity \in ActivityIDs, obligation \in WorkObligationIDs, task \in DeliveryTaskIDs, attempt \in DeliveryAttemptIDs: Activity_delivery_authorize(entity, obligation, task, attempt)
    \/ \E entity \in ActivityIDs, obligation \in WorkObligationIDs, task \in DeliveryTaskIDs, queue \in DeliveryQueueIDs: Activity_delivery_persist(entity, obligation, task, queue)
    \/ \E entity \in ActivityIDs, obligation \in WorkObligationIDs, task \in DeliveryTaskIDs, attempt \in DeliveryAttemptIDs: Activity_delivery_reject(entity, obligation, task, attempt)
    \/ \E obligation \in WorkObligationIDs, task \in DeliveryTaskIDs: Activity_delivery_resolve_persisted(obligation, task)
    \/ \E entity \in ActivityIDs, previous_obligation \in WorkObligationIDs, previous_task \in DeliveryTaskIDs, obligation \in WorkObligationIDs, task \in DeliveryTaskIDs, queue \in DeliveryQueueIDs: Activity_delivery_retry(entity, previous_obligation, previous_task, obligation, task, queue)
...
CanStep ==
    ...
    \/ \E entity \in ActivityIDs, obligation \in WorkObligationIDs, task \in DeliveryTaskIDs, attempt \in DeliveryAttemptIDs: Activity_delivery_authorizeEnabled(entity, obligation, task, attempt)
    \/ \E entity \in ActivityIDs, obligation \in WorkObligationIDs, task \in DeliveryTaskIDs, queue \in DeliveryQueueIDs: Activity_delivery_persistEnabled(entity, obligation, task, queue)
    \/ \E entity \in ActivityIDs, obligation \in WorkObligationIDs, task \in DeliveryTaskIDs, attempt \in DeliveryAttemptIDs: Activity_delivery_rejectEnabled(entity, obligation, task, attempt)
    \/ \E obligation \in WorkObligationIDs, task \in DeliveryTaskIDs: Activity_delivery_resolve_persistedEnabled(obligation, task)
    \/ \E entity \in ActivityIDs, previous_obligation \in WorkObligationIDs, previous_task \in DeliveryTaskIDs, obligation \in WorkObligationIDs, task \in DeliveryTaskIDs, queue \in DeliveryQueueIDs: Activity_delivery_retryEnabled(entity, previous_obligation, previous_task, obligation, task, queue)
...
activity_delivery_accepted_start_correspondence ==
    \A activity \in ActivityIDs: activity \in exists_Activity => ((state_Activity[activity] = "started" => \E obligation \in WorkObligationIDs: obligation \in exists_WorkObligation /\ ((<<activity, obligation>> \in relation_activity_obligation /\ state_WorkObligation[obligation] = "accepted" /\ \E attempt \in DeliveryAttemptIDs: attempt \in exists_DeliveryAttempt /\ (<<obligation, attempt>> \in relation_delivery_accepted_start)))))

activity_delivery_intent_correspondence ==
    \A activity \in ActivityIDs: activity \in exists_Activity => (((state_Activity[activity] = "scheduled" \/ state_Activity[activity] = "started" \/ state_Activity[activity] = "backing_off") => (\E obligation \in WorkObligationIDs: obligation \in exists_WorkObligation /\ (<<activity, obligation>> \in relation_activity_obligation) /\ \E deliveryTask \in DeliveryTaskIDs: deliveryTask \in exists_DeliveryTask /\ (<<activity, deliveryTask>> \in relation_activity_delivery_task))))
