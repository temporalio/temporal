...
func TestProtocolPlanSettlingEdgesUsesCompiledActionsAndRetainsGaps(t *testing.T) {
	declaration := plannerDeclaration()
	entity := &declaration.Entities[0]
	key := entity.Actions[3].Key
	entity.Actions = entity.Actions[:3]
	...
	planned, err := protocol.PlanSettlingEdges("CompilerLifecycleEntity", umpire.Embedded)
	require.NoError(t, err)
	require.Empty(t, planned.Plans)
	require.Equal(t, []ActionGap{{Key: key, Reason: "needs a real timer"}}, planned.Omitted)
}

func TestProtocolSampleSettlingPlanIsReproducible(t *testing.T) {
	protocol, err := Compile(plannerDeclaration())
	require.NoError(t, err)

	first, err := protocol.SampleSettlingPlan("CompilerLifecycleEntity", umpire.Standalone, 42)
	require.NoError(t, err)
	second, err := protocol.SampleSettlingPlan("CompilerLifecycleEntity", umpire.Standalone, 42)
	require.NoError(t, err)
	require.Equal(t, first, second)
	require.Equal(t, []string{"start-standalone", "finish-standalone"}, actionNames(first.Actions))
}
