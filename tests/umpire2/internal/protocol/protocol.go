...
type RelationMutation = umpire.RelationMutation

// RelationDeriver translates one observed fact into zero or more relation mutations.
type RelationDeriver = umpire.RelationDeriver
...
// RuntimeDeclaration projects the compiled protocol into the generic runtime declaration.
func (p *Protocol) RuntimeDeclaration(rules []umpire.RuntimeRuleDeclaration) umpire.RuntimeDeclaration {
	entities := make([]umpire.RuntimeEntityDeclaration, 0, len(p.entityOrder))
	for _, entityType := range p.entityOrder {
		entities = append(entities, umpire.RuntimeEntityDeclaration{Type: entityType, New: p.entities[entityType].new})
	}
	return umpire.RuntimeDeclaration{
		Facts:            append([]umpire.Fact(nil), p.facts...),
		Entities:         entities,
		Relations:        append([]umpire.RelationSchema(nil), p.relations...),
		RelationDerivers: append([]umpire.RelationDeriver(nil), p.derivers...),
		Rules:            append([]umpire.RuntimeRuleDeclaration(nil), rules...),
	}
}
