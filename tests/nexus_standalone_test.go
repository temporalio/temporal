...
func (s *NexusStandaloneTestSuite) TestDescribeStandaloneNexusOperation() {
	...
	s.Run("LongPollStateChange", func(s *NexusStandaloneTestSuite) {
		...
		s.NoError(await.RequireReceive(s.T(), terminateErrCh))

		// Verify the longpoll result.
		result := await.RequireReceive(s.T(), describeResultCh)
		s.NoError(result.err)
...
func (s *NexusStandaloneTestSuite) TestStandaloneNexusOperationPoll() {
	s.Run("WaitStageStarted", func(s *NexusStandaloneTestSuite) {
		...
		await.RequireReceive(s.T(), pollStartedCh)
		...
		result := await.RequireReceive(s.T(), pollResultCh)
		s.NoError(result.err)
...
func (s *NexusStandaloneTestSuite) TestAsyncCompletionIgnoresExecutionTransitionInCallbackToken() {
	...
	s.NoError(err)
	callback := await.RequireReceive(s.T(), callbackCh)
