...
func TestWorkflowFailures(t *testing.T) {
  ...
  3 WorkflowExecutionTimedOut`, historyEvents)
	...
	t.Run("WorkflowTaskFailed", func(t *testing.T) {
		...
		wtHandler := func(task *workflowservice.PollWorkflowTaskQueueResponse) ([]*commandpb.Command, error) {
			...
			if sendSignal {
				require.NoError(t, s.SendSignal(s.Namespace().String(), workflowExecution, "signalC", nil, identity))
				require.NoError(t, s.SendSignal(s.Namespace().String(), workflowExecution, "signalD", nil, identity))
				require.NoError(t, s.SendSignal(s.Namespace().String(), workflowExecution, "signalE", nil, identity))
				sendSignal = false
			}
			...
			require.Equal(t, enumspb.EVENT_TYPE_WORKFLOW_TASK_STARTED, lastWorkflowTaskEvent.GetEventType())
		...
		require.NoError(t, err, "failed to send signal to execution")
		...
		require.Equal(t, 12, signalCount)
		...
		require.Equal(t, 16, signalCount)
 ...
 28 WorkflowExecutionCompleted //wfCompletedEvent`, events)
		...
		require.Equal(t, lastWorkflowTaskTime, lastWorkflowTaskStartedEvent.GetEventTime().AsTime())
		wfCompletedEvent := events[27]
		require.True(t, wfCompletedEvent.GetEventTime().AsTime().Sub(lastWorkflowTaskTime) >= time.Second)
	})

	t.Run("RespondWorkflowTaskCompleted_ReturnsErrorIfInvalidArgument", func(t *testing.T) {
		s := testcore.NewEnv(t)
		...
		require.Equal(t, "BadRecordMarkerAttributes: MarkerName is not set on RecordMarkerCommand.", err.Error())
