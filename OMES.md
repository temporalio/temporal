...
declarative description."** That is exactly the workload substrate the Player wants
([`PLAYER.md`](./PLAYER.md) observation #2: *"Reuse Omes kitchensink workflows as the
...
- **The one generalization it needs to become reactive.** As-is, `Execute` is coarse — "do one
  ...
  explorer / Player, guarding over the Umpire model) needs to choose the *next single action*
...
## Why this matters to Umpire / Player / SAA

Omes solves three problems the active side (Player) and the model side (SAA) would otherwise
have to re-solve:
...
1. **A declarative, replayable workload.** `TestInput` is data: hand-write it for a targeted
   ...
   `PLAYER.md` is *"ordered/guarded []Pitch + input parameters"* — structurally the same idea
...
3. **Client-only RPC actions already exist.** `DoStandaloneActivity` /
   ...
   poll) and with the Player's black-box Pitches.
...
`PLAYER.md` gestures at: a cheap **declarative background plane** (Kitchen Sink `TestInput`,
...
- **Replay the realized trace, not the plan.** Reactive decisions depend on observed timing, so
  ...
  identical config) and `PLAYER.md`'s *"separate run from eval."*
...
| **Maps to** | SAA fuzzer / `PLAYER.md` P4; the Omes fuzzer *with an oracle attached* | SAA explorer (BFS/random-walk) realized through the any-language worker; `PLAYER.md` P2/P3 |
...
1. **Adopt `TestInput` as the SDK realizer's target format**, not a new grammar. A Player
...
3. **Copy the replayability discipline.** Saved binary programs (not just seeds) + a curated
   ...
   Game, re-check offline"* constraint in `PLAYER.md`, already proven in Omes.
4. **Reuse the `project` gRPC harness shape** for driving native SDK code the Player doesn't
...
- **Load-shaped, not exploration-shaped.** `GenericExecutor` is a steady-rate concurrency
  ...
  guard-over-the-model driving (`PLAYER.md`) and the `Reachable`-derived traversal
