...
func TestIntegration(t *testing.T) {
	...
	t.Run("failure: deep subtest (3-level nesting)", func(t *testing.T) {
		...
		// TestDeepSuite has two groups (GroupA, GroupB), each with identically
		// named Pass and Fail subtests. Both Fail subtests fail on attempt 1.
		// Each failure triggers its own retry with a correctly scoped
		// -test.run and -test.skip. The overlapping names ("Pass", "Fail")
		// ensure skip patterns are properly scoped per group.
