...
func TestIntegration(t *testing.T) {
	...
	t.Run("failure: data race", func(t *testing.T) {
		...
		assertLogFiles(t, res, // TestRace crashes, no retry (max-attempts=1)
			file("TestRace",
				"TESTRUNNER LOG",
				"Attempt:     1",
				"DATA RACE",
...
func assertLogFiles(t *testing.T, res integResult, opts ...logFileOpt) {
	t.Helper()
	files := readTestLogFiles(res.logDir)
...
func assertJUnit(t *testing.T, res integResult, opts ...junitOpt) {
	t.Helper()
...
func assertConsole(t *testing.T, res integResult, opts ...consoleOpt) {
	t.Helper()
