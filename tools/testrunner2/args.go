...
const (
	GroupByPackage GroupMode = "package" // all tests in a package run together (for unit tests)
	GroupByFile    GroupMode = "file"    // all tests in a file run together (for integration tests) - default
	GroupBySuite   GroupMode = "suite"   // each TestXxx function runs separately (for functional tests)
...
func parseConfig(command string, args []string, cfg *config) ([]string, error) {
	...
	for _, arg := range args {
		...
		if after, ok := strings.CutPrefix(arg, groupByFlag); ok {
			...
			case GroupByPackage, GroupByFile, GroupBySuite, GroupByNone:
				cfg.groupBy = GroupMode(after)
			default:
				return nil, fmt.Errorf("invalid argument %s: must be 'package', 'file', 'suite', or 'none'", groupByFlag)
			}
	...
	case testCommand:
		...
		if cfg.groupBy == "" {
			return nil, fmt.Errorf("missing required argument %q: use 'package' for unit tests, 'file' for integration tests, 'suite' for functional tests, 'none' for simple unit tests", groupByFlag)
		}
