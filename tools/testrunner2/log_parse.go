...
func tryParseTimeout(lines []string, i int, line string) ([]alert, int, bool) {
	...
	filtered := filterParentTests(tests)
...
// filterParentTests removes parent tests when their subtests are present.
// E.g., given [TestFoo1, TestFoo1/SubTest1, TestFoo2], returns [TestFoo1/SubTest1, TestFoo2].
func filterParentTests(tests []string) []string {
	// Build a set of all tests for quick lookup
	testSet := make(map[string]struct{}, len(tests))
	for _, t := range tests {
		testSet[t] = struct{}{}
	}
	...
	var filtered []string
	for _, t := range tests {
		// Check if this test is a parent of any other test in the list
		isParent := false
		prefix := t + "/"
...
var testifyErrorSections = []string{"Error Trace:", "Error:", "Test:", "Messages:"}

// extractErrorTrace parses test failure output and extracts just the testify
// error sections (Error Trace, Error, Test, Messages), filtering out log lines.
