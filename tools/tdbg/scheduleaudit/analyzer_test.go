...
func TestStartedWorkflows(t *testing.T) {
	t.Run("still running is active after start", func(t *testing.T) {
		...
		require.NotNil(t, sw.blockingAt(mustParseTime("2026-05-19T19:00:00Z")))
	})
	...
	t.Run("closed before query time is inactive", func(t *testing.T) {
		...
		require.Nil(t, sw.blockingAt(mustParseTime("2026-05-19T19:00:00Z")))
	})
...
func TestOverlapClassOf(t *testing.T) {
	cases := []struct {
		...
		{enumspb.SCHEDULE_OVERLAP_POLICY_CANCEL_OTHER, replacesOnOverlap},
		{enumspb.SCHEDULE_OVERLAP_POLICY_TERMINATE_OTHER, replacesOnOverlap},
...
func TestClassify(t *testing.T) {
	scheduled := []ScheduledTime{
		{Nominal: mustParseTime("2026-05-19T18:00:00Z"), Jittered: mustParseTime("2026-05-19T18:00:00Z")},
		{Nominal: mustParseTime("2026-05-19T19:00:00Z"), Jittered: mustParseTime("2026-05-19T19:00:00Z")},
	}
	...
	t.Run("buffering policy + running blocker -> pending", func(t *testing.T) {
		...
		require.Equal(t, categoryPending, r.Missed[mustParseTime("2026-05-19T19:00:00Z")])
		require.Zero(t, r.Count(categorySkipOverlap))
	})

	t.Run("buffering policy treats an earlier delayed start as a logical blocker", func(t *testing.T) {
		fires := []ScheduledTime{
			{Nominal: mustParseTime("2026-05-19T19:00:00Z"), Jittered: mustParseTime("2026-05-19T19:00:00Z")},
			{Nominal: mustParseTime("2026-05-19T20:00:00Z"), Jittered: mustParseTime("2026-05-19T20:00:00Z")},
		}
		delayed := groupExecutions([]Execution{{
			WorkflowID: "delayed", NominalTime: fires[0].Nominal, StartTime: mustParseTime("2026-05-19T20:30:00Z"),
			Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING,
		}})
		r := &Result{}
		classify(r, fires, delayed, delayed, enumspb.SCHEDULE_OVERLAP_POLICY_BUFFER_ALL)
		require.Equal(t, 1, r.Matched)
		require.Equal(t, categoryPending, r.Missed[fires[1].Nominal])
	})

	t.Run("buffer one replays delayed selection before later fires", func(t *testing.T) {
		fires := []ScheduledTime{
			{Nominal: mustParseTime("2026-05-19T19:00:00Z"), Jittered: mustParseTime("2026-05-19T19:00:00Z")},
			{Nominal: mustParseTime("2026-05-19T20:00:00Z"), Jittered: mustParseTime("2026-05-19T20:00:00Z")},
			{Nominal: mustParseTime("2026-05-19T21:00:00Z"), Jittered: mustParseTime("2026-05-19T21:00:00Z")},
		}
		delayed := groupExecutions([]Execution{{
			WorkflowID: "delayed", NominalTime: fires[0].Nominal, StartTime: mustParseTime("2026-05-19T20:30:00Z"),
			Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING,
		...
		r := &Result{}
		classify(r, fires, delayed, delayed, enumspb.SCHEDULE_OVERLAP_POLICY_BUFFER_ONE)
		require.Equal(t, categoryPending, r.Missed[fires[1].Nominal])
		require.Equal(t, categorySkipOverlap, r.Missed[fires[2].Nominal])
	})

	t.Run("overlap uses jittered fire time", func(t *testing.T) {
		closeTime := mustParseTime("2026-05-19T19:15:00Z")
		sw := groupExecutions([]Execution{{
			WorkflowID: "blocker", StartTime: mustParseTime("2026-05-19T19:05:00Z"), CloseTime: &closeTime,
			Status: enumspb.WORKFLOW_EXECUTION_STATUS_COMPLETED, NominalTime: mustParseTime("2026-05-19T18:00:00Z"),
		}})
		r := &Result{}
		classify(r, []ScheduledTime{{
			Nominal: mustParseTime("2026-05-19T19:00:00Z"), Jittered: mustParseTime("2026-05-19T19:10:00Z"),
		}}, startedWorkflows{}, sw, enumspb.SCHEDULE_OVERLAP_POLICY_SKIP)
		require.Equal(t, categorySkipOverlap, r.Missed[mustParseTime("2026-05-19T19:00:00Z")])
	})

	t.Run("buffer one keeps first overlap and skips later overlap", func(t *testing.T) {
		sw := groupExecutions([]Execution{{
			WorkflowID: "blocker", StartTime: mustParseTime("2026-05-19T18:00:00Z"),
			Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING, NominalTime: mustParseTime("2026-05-19T18:00:00Z"),
		}})
		fires := []ScheduledTime{
			{Nominal: mustParseTime("2026-05-19T19:00:00Z"), Jittered: mustParseTime("2026-05-19T19:00:00Z")},
			{Nominal: mustParseTime("2026-05-19T20:00:00Z"), Jittered: mustParseTime("2026-05-19T20:00:00Z")},
		...
		r := &Result{}
		classify(r, fires, startedWorkflows{}, sw, enumspb.SCHEDULE_OVERLAP_POLICY_BUFFER_ONE)
		require.Equal(t, categoryPending, r.Missed[fires[0].Nominal])
		require.Equal(t, categorySkipOverlap, r.Missed[fires[1].Nominal])
	})

	t.Run("buffer one tracks an observed buffered start", func(t *testing.T) {
		closeTime := mustParseTime("2026-05-19T21:00:00Z")
		blocker := Execution{
			WorkflowID: "blocker", StartTime: mustParseTime("2026-05-19T18:00:00Z"), CloseTime: &closeTime,
			Status: enumspb.WORKFLOW_EXECUTION_STATUS_COMPLETED, NominalTime: mustParseTime("2026-05-19T18:00:00Z"),
		}
		buffered := Execution{
			WorkflowID: "buffered", StartTime: closeTime,
			Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING, NominalTime: mustParseTime("2026-05-19T19:00:00Z"),
		}
		fires := []ScheduledTime{
			{Nominal: buffered.NominalTime, Jittered: buffered.NominalTime},
			{Nominal: mustParseTime("2026-05-19T20:00:00Z"), Jittered: mustParseTime("2026-05-19T20:00:00Z")},
		}
		r := &Result{}
	...
	t.Run("buffer one at window boundary is inconclusive", func(t *testing.T) {
		closeTime := mustParseTime("2026-05-19T21:00:00Z")
		sw := groupExecutions([]Execution{{
			WorkflowID: "blocker", StartTime: mustParseTime("2026-05-19T17:00:00Z"), CloseTime: &closeTime,
			Status: enumspb.WORKFLOW_EXECUTION_STATUS_COMPLETED, NominalTime: mustParseTime("2026-05-19T17:00:00Z"),
		}})
		fire := ScheduledTime{Nominal: mustParseTime("2026-05-19T19:00:00Z"), Jittered: mustParseTime("2026-05-19T19:00:00Z")}
		r := &Result{WindowStart: mustParseTime("2026-05-19T18:00:00Z")}
		classify(r, []ScheduledTime{fire}, startedWorkflows{}, sw, enumspb.SCHEDULE_OVERLAP_POLICY_BUFFER_ONE)
		require.Equal(t, categoryInconclusiveOverlap, r.Missed[fire.Nominal])
	})

	t.Run("duplicate predecessor ordering is inconclusive", func(t *testing.T) {
		predecessor := mustParseTime("2026-05-19T18:00:00Z")
		active := groupExecutions([]Execution{
			{WorkflowID: "a", NominalTime: predecessor, StartTime: predecessor, Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING},
			{WorkflowID: "b", NominalTime: predecessor, StartTime: predecessor, Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING},
		})
		fire := ScheduledTime{Nominal: mustParseTime("2026-05-19T19:00:00Z"), Jittered: mustParseTime("2026-05-19T19:00:00Z")}
		r := &Result{}
		classify(r, []ScheduledTime{fire}, startedWorkflows{}, active, enumspb.SCHEDULE_OVERLAP_POLICY_BUFFER_ALL)
		require.Equal(t, categoryInconclusiveOverlap, r.Missed[fire.Nominal])
	})

	t.Run("duplicate predecessor ordering does not hide an observed replacement", func(t *testing.T) {
		predecessor := mustParseTime("2026-05-19T18:00:00Z")
		fire := ScheduledTime{Nominal: mustParseTime("2026-05-19T19:00:00Z"), Jittered: mustParseTime("2026-05-19T19:00:00Z")}
		observed := Execution{
			WorkflowID: "observed", NominalTime: fire.Nominal, StartTime: fire.Jittered,
			Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING,
		}
		active := groupExecutions([]Execution{
			{WorkflowID: "a", NominalTime: predecessor, StartTime: predecessor, Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING},
		...
		r := &Result{}
		classify(r, []ScheduledTime{fire}, groupExecutions([]Execution{observed}), active, enumspb.SCHEDULE_OVERLAP_POLICY_CANCEL_OTHER)
		require.Equal(t, 1, r.Matched)
		require.Empty(t, r.Missed)
	})
	...
	for _, policy := range []enumspb.ScheduleOverlapPolicy{
		...
		t.Run(overlapPolicyName(policy)+" supersedes all but the latest overlap", func(t *testing.T) {
			closeTime := mustParseTime("2026-05-19T21:00:00Z")
			sw := groupExecutions([]Execution{{
				WorkflowID: "blocker", StartTime: mustParseTime("2026-05-19T18:00:00Z"), CloseTime: &closeTime,
				Status: enumspb.WORKFLOW_EXECUTION_STATUS_COMPLETED, NominalTime: mustParseTime("2026-05-19T18:00:00Z"),
			...
			r := &Result{}
			classify(r, fires, startedWorkflows{}, sw, policy)
			require.Equal(t, categorySkipOverlap, r.Missed[fires[0].Nominal])
			require.Equal(t, categoryRealMiss, r.Missed[fires[1].Nominal])
		})
	}

	t.Run("cancel policy keeps only the latest overlap pending", func(t *testing.T) {
		sw := groupExecutions([]Execution{{
			WorkflowID: "blocker", StartTime: mustParseTime("2026-05-19T18:00:00Z"),
			Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING, NominalTime: mustParseTime("2026-05-19T18:00:00Z"),
		}})
		...
		r := &Result{}
		classify(r, fires, startedWorkflows{}, sw, enumspb.SCHEDULE_OVERLAP_POLICY_CANCEL_OTHER)
		require.Equal(t, categorySkipOverlap, r.Missed[fires[0].Nominal])
		require.Equal(t, categoryPending, r.Missed[fires[1].Nominal])
	})
}

func TestMarkAwaitingStarts(t *testing.T) {
	asOf := mustParseTime("2026-05-19T20:00:00Z")
	old := ScheduledTime{Nominal: mustParseTime("2026-05-19T19:58:00Z"), Jittered: mustParseTime("2026-05-19T19:58:00Z")}
	recent := ScheduledTime{Nominal: mustParseTime("2026-05-19T19:59:30Z"), Jittered: mustParseTime("2026-05-19T19:59:30Z")}
	skipped := ScheduledTime{Nominal: mustParseTime("2026-05-19T19:59:45Z"), Jittered: mustParseTime("2026-05-19T19:59:45Z")}
	r := &Result{
		AsOf:           asOf,
		DelayThreshold: time.Minute,
		Missed: map[time.Time]string{
			old.Nominal:     categoryRealMiss,
			recent.Nominal:  categoryRealMiss,
			skipped.Nominal: categorySkipOverlap,
		},
	}

	markAwaitingStarts(r, []ScheduledTime{old, recent, skipped})
	require.Equal(t, categoryRealMiss, r.Missed[old.Nominal])
	require.Equal(t, categoryAwaitingStart, r.Missed[recent.Nominal])
	require.Equal(t, categorySkipOverlap, r.Missed[skipped.Nominal])
}
...
type fakeScheduleLoader struct {
	...
	namespaceID      string
	contextAfterList error
}

func (f *fakeScheduleLoader) ListScheduleIDs(ctx context.Context, _ string, yield func(id string) error) error {
	for _, e := range f.entries {
		if err := yield(e.ID); err != nil {
			return err
		}
	}
	f.contextAfterList = ctx.Err()
	return nil
}
...
type fakeExecutionLoader struct {
	...
	failSchedule string
	calls        [][]string
	queryStarts  []time.Time
}

func (f *fakeExecutionLoader) ListExecutions(_ context.Context, _ string, scheduleIDs []string, queryStart, _ time.Time) (map[string][]Execution, error) {
	f.mu.Lock()
	f.calls = append(f.calls, append([]string(nil), scheduleIDs...))
	f.queryStarts = append(f.queryStarts, queryStart)
	f.mu.Unlock()
	for _, scheduleID := range scheduleIDs {
		if scheduleID != f.failSchedule {
			continue
		}
		return nil, status.Error(codes.Internal, "visibility failed")
	}
	out := make(map[string][]Execution, len(scheduleIDs))
	for _, scheduleID := range scheduleIDs {
		out[scheduleID] = f.byScheduleID[scheduleID]
	}
	return out, nil
}
...
func TestAuditor(t *testing.T) {
	...
	t.Run("concurrency: whole namespace streams every schedule", func(t *testing.T) {
		...
		require.Equal(t, StatsSnapshot{Listed: 100, Audited: 100}, stats.Snapshot())
		require.Len(t, exec.calls, 4)
		for _, ids := range exec.calls {
			require.Len(t, ids, executionBatchSize)
		}
	})

	t.Run("failed schedule does not cancel namespace listing", func(t *testing.T) {
		entries := make([]ScheduleEntry, executionBatchSize+1)
		for i := range entries {
			entries[i] = ScheduleEntry{ID: fmt.Sprintf("s%02d", i), Spec: hourlyAllHoursSpec(), WorkflowType: "W"}
		}
		entries[0].ID = "bad"
		loader := &fakeScheduleLoader{entries: entries}
		executions := &fakeExecutionLoader{failSchedule: "bad"}
		a := &Auditor{
			WindowStart: mustParseTime("2026-05-19T18:00:00Z"),
			WindowEnd:   mustParseTime("2026-05-19T20:00:00Z"),
			Schedules:   loader,
		...
		results, err := runAudit(a, Target{Namespace: "ns"})
		require.ErrorContains(t, err, "visibility failed")
		require.NoError(t, loader.contextAfterList)
		require.Len(t, results, 1)
		require.Equal(t, entries[len(entries)-1].ID, results[0].ScheduleID)
		require.Len(t, executions.calls, 2)
	})
	...
	t.Run("schedule-id targets share one namespace pipeline and visibility batches", func(t *testing.T) {
		entries := make([]ScheduleEntry, executionBatchSize+5)
		targets := make([]Target, len(entries))
		for i := range entries {
			id := fmt.Sprintf("s%02d", i)
			entries[i] = ScheduleEntry{ID: id, Spec: hourlyAllHoursSpec(), WorkflowType: "W"}
			targets[i] = Target{Namespace: "ns", ScheduleID: id}
		}
		executions := &fakeExecutionLoader{byScheduleID: map[string][]Execution{}}
		a := &Auditor{
			WindowStart: mustParseTime("2026-05-19T18:00:00Z"),
			WindowEnd:   mustParseTime("2026-05-19T20:00:00Z"),
			Concurrency: 8,
			Schedules:   &fakeScheduleLoader{entries: entries},
			Executions:  executions,
			Progress:    io.Discard,
		}

		results, err := runAudit(a, targets...)
		require.NoError(t, err)
		require.Len(t, results, len(entries))
		require.Len(t, executions.calls, 2)
		require.ElementsMatch(t, []int{5, executionBatchSize}, []int{len(executions.calls[0]), len(executions.calls[1])})
	})
	...
	t.Run("paused mid-window included is inconclusive", func(t *testing.T) {
		...
		require.Len(t, results, 1)
		require.True(t, results[0].Paused)
		require.Equal(t, auditStatusInconclusiveScheduleChanged, results[0].AuditStatus)
	...
	t.Run("window entirely past retention yields nothing after clamping", func(t *testing.T) {
		...
		require.Empty(t, a.Executions.(*fakeExecutionLoader).calls, "no-fire schedules should not query visibility")
	})
	...
	t.Run("modified during window -> inconclusive", func(t *testing.T) {
		...
		require.Equal(t, auditStatusInconclusiveScheduleChanged, r.AuditStatus)
	})

	t.Run("modified schedule with no fires under current spec is still reported", func(t *testing.T) {
		loader := &fakeScheduleLoader{entries: []ScheduleEntry{{
			ID: "s1", WorkflowType: "W",
			Spec: &schedulepb.ScheduleSpec{
				Interval: []*schedulepb.IntervalSpec{{Interval: durationpb.New(time.Hour)}},
				EndTime:  timestamppb.New(mustParseTime("2026-05-19T17:00:00Z")),
			},
			UpdateTime: mustParseTime("2026-05-19T19:30:00Z"),
		}}}
		a := &Auditor{
	...
	t.Run("high frequency schedule is skipped without aborting namespace audit", func(t *testing.T) {
		loader := &fakeScheduleLoader{entries: []ScheduleEntry{{
			ID: "fast", WorkflowType: "W",
			Spec: &schedulepb.ScheduleSpec{Interval: []*schedulepb.IntervalSpec{{Interval: durationpb.New(time.Second)}}},
		}}}
		var progress strings.Builder
		a := &Auditor{
			WindowStart: mustParseTime("2026-05-18T00:00:00Z"),
			WindowEnd:   mustParseTime("2026-05-19T04:00:02Z"),
			Schedules:   loader,
			Executions:  &fakeExecutionLoader{byScheduleID: map[string][]Execution{}},
			Progress:    &progress,
			Stats:       NewStats(),
		...
		require.NoError(t, err)
		require.Empty(t, results)
		require.Contains(t, progress.String(), "skipping schedule with more than 100k fires")
		require.Equal(t, 1, a.Stats.Snapshot().Skipped)
	})
...
func TestScheduledTimesJitter(t *testing.T) {
	...
	var shifted ScheduledTime
	for _, st := range got {
		if st.Jittered.Sub(st.Nominal) > time.Second {
			shifted = st
			break
		}
	}
	require.False(t, shifted.Nominal.IsZero())
	narrow, err := scheduledTimes(spec, jitterSeed("ns-id", "s1"), shifted.Nominal.Add(time.Second), shifted.Jittered)
	require.NoError(t, err)
	require.Contains(t, narrow, shifted, "a nominal before the window is included when its jittered fire is in-window")

	beforeFire, err := scheduledTimes(spec, jitterSeed("ns-id", "s1"), shifted.Nominal.Add(time.Second), shifted.Jittered.Add(-time.Millisecond))
	require.NoError(t, err)
	require.NotContains(t, beforeFire, shifted, "a fire is not expected before its jittered time")
...
func TestActionDelays(t *testing.T) {
	...
	require.Equal(t, categoryOnTime, byWF["wfA"].Category)
	require.Equal(t, categoryOnTime, byWF["wfLong"].Category)
	...
	require.Equal(t, categoryLate, byWF["wfB"].Category)
	...
	require.Equal(t, categoryDelayedOverlap, c.Category)
}

func TestActionDelaysAllowAllIgnoresConcurrentWorkflow(t *testing.T) {
	closeTime := mustParseTime("2026-05-19T22:00:00Z")
	entries := []Execution{
		{WorkflowID: "already-running", StartTime: mustParseTime("2026-05-19T18:00:00Z"), CloseTime: &closeTime, Status: enumspb.WORKFLOW_EXECUTION_STATUS_COMPLETED, NominalTime: mustParseTime("2026-05-19T18:00:00Z")},
		{WorkflowID: "concurrent", StartTime: mustParseTime("2026-05-19T20:05:00Z"), Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING, NominalTime: mustParseTime("2026-05-19T20:00:00Z")},
	}
	all := groupExecutions(entries)
	delays := buildDelays(
		groupExecutions(entries[1:]),
		all,
		map[time.Time]time.Time{mustParseTime("2026-05-19T20:00:00Z"): mustParseTime("2026-05-19T20:00:00Z")},
		enumspb.SCHEDULE_OVERLAP_POLICY_ALLOW_ALL,
		30*time.Minute,
	)
	require.Len(t, delays, 1)
	require.Zero(t, delays[0].OverlapWait)
	require.Equal(t, 5*time.Minute, delays[0].DispatchDelay)
	require.Equal(t, categoryOnTime, delays[0].Category)
}

func TestActionDelaysUseDefaultLateThreshold(t *testing.T) {
	nominal := mustParseTime("2026-05-19T20:00:00Z")
	entry := Execution{
		WorkflowID: "late", NominalTime: nominal, StartTime: nominal.Add(2 * time.Minute),
		Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING,
	}
	workflows := groupExecutions([]Execution{entry})
	delays := buildDelays(workflows, workflows, map[time.Time]time.Time{nominal: nominal}, enumspb.SCHEDULE_OVERLAP_POLICY_ALLOW_ALL, 0)
	require.Len(t, delays, 1)
	require.Equal(t, categoryLate, delays[0].Category)
}

func TestActionDelaysFollowBufferAllQueue(t *testing.T) {
	closeA := mustParseTime("2026-05-19T22:00:00Z")
	closeB := mustParseTime("2026-05-19T23:00:00Z")
	entries := []Execution{
		{WorkflowID: "a", NominalTime: mustParseTime("2026-05-19T18:00:00Z"), StartTime: mustParseTime("2026-05-19T18:00:00Z"), CloseTime: &closeA, Status: enumspb.WORKFLOW_EXECUTION_STATUS_COMPLETED},
		{WorkflowID: "b", NominalTime: mustParseTime("2026-05-19T19:00:00Z"), StartTime: closeA, CloseTime: &closeB, Status: enumspb.WORKFLOW_EXECUTION_STATUS_COMPLETED},
		{WorkflowID: "c", NominalTime: mustParseTime("2026-05-19T20:00:00Z"), StartTime: closeB, Status: enumspb.WORKFLOW_EXECUTION_STATUS_RUNNING},
	}
	all := groupExecutions(entries)
	delays := buildDelays(
		groupExecutions(entries[2:]), all,
		map[time.Time]time.Time{entries[2].NominalTime: entries[2].NominalTime},
		enumspb.SCHEDULE_OVERLAP_POLICY_BUFFER_ALL, 30*time.Minute,
	)
	require.Len(t, delays, 1)
	require.Equal(t, closeB, delays[0].Desired)
	require.Equal(t, 3*time.Hour, delays[0].OverlapWait)
	require.Zero(t, delays[0].DispatchDelay)
	require.Equal(t, categoryDelayedOverlap, delays[0].Category)
}
