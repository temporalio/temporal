...
The minimum server, SDK, and CLI 
...
Worker Versioning needs to be enabled in the server. To enable Worker Versioning, the following dynamic config fields must be set to `true`.
...
### Walkthrough: Blue-Green Deploy in a Dev Cluster
#### 1. Start a dev server with versioning enabled
```bash
...
#### 2. Define your first versioned worker
In the worker options, pass in the Build ID your worker will poll on, _and_ opt in to use the
Build ID for versioning. The best practice is to provide a new Build ID generated by your build pipeline whenever a new 
build of your code is made. Below is an example written in Go.
```go
w := worker.New(
...
#### 4. Send all new tasks to your versioned worker
[Add an assignment rule](#update-rules-using-cli) targeting your Build ID to send 100% of new
tasks to your versioned worker. The default percentage ramp is 100, so we don't need to specify
the `--percentage` flag.
```bash
temporal task-queue versioning insert-assignment-rule --task-queue my-tq --build-id $BUILD_ID
```
Once this rule is created, all tasks from new workflow executions will be sent to the versioned
worker that you started above. You can check the progress of workflows like this:
```bash
temporal -n $NS workflow list --query "BuildIds = 'assigned:$BUILD_ID'"
```

#### 5. Change your workflow and build/deploy your next versioned worker
Make some code change to `MyWorkflow`, set `BUILD_ID` to a new value, so you can build a new versioned worker

#### 6. Send 10% of new tasks to your new versioned worker

#### 7. Monitor workflows running on the new version and confirm success

#### 8. Commit the new version

#### 9. Wait until the previous version is no longer reachable

#### 10. Decommission the previous versioned worker
