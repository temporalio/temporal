...
stage and not recommended for production usage. Future breaking changes may be made in deemed necessary.
...
## When should use Worker Versioning
...
## Getting Started
Here is a guide for typical usage of Worker Versioning.
### 0. Enable Worker Versioning in Temporal Server or Cloud
Worker Versioning needs to be enabled from server
...
Temporal Cloud users should open ticket against the support team to get added to the early release.

### 1. Opt In and Set Worker Build ID
Using Worker options to opt in Worker Versioning and pass Build ID. This is supported
   in all SDKs.

Below is an example written in Go. The best practice is to provide a new Build ID generated 
by your build pipeline whenever a new build of your code is made. In the following example 
we assume the build pipeline generates a Build ID value and set it in the `BUILD_ID` env var
to make it available to the worker application.
```go
...
```
### 2. Deploy 
Deploy your worker with new Build ID while keeping the old Build ID still running.
### 3. Update Versioning Rules 
[Use CLI to update Worker Versioning Rules](#update-rules-using-cli)
