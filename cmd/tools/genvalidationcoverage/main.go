...
func main() {
	...
	fatalIfErr(err)
	if len(messages) == 0 {
		fatalf("-message or -messages-file is required")
	}
	...
	fatalIfErr(os.WriteFile(*outFlag, src, 0o644))
}
...
func splitMessage(message string) (string, string, error) {
		...
		for _, file := range pkg.Files {
			for _, decl := range file.Decls {
				...
				for _, spec := range genDecl.Specs {
					...
					return protoStructFields(fset, file, structType)
				}
			}
		}
	}
...
func render(allData []templateData) ([]byte, error) {
	...
	var b bytes.Buffer
	fmt.Fprintf(&b, "// Code generated by genvalidationcoverage. DO NOT EDIT.\n\n")
	...
	fmt.Fprintf(&b, "\t\"go.temporal.io/server/common/validation\"\n")
	fmt.Fprintf(&b, ")\n\n")
	for _, data := range allData {
