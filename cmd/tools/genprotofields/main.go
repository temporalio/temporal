...
func main() {
	...
	imports := newImports()
	imports.add("go.temporal.io/server/api/protohelpers/protofield")
	...
	writeFile(*validateOutFlag, "validate_gen.go", renderValidate(validatePackage, msgs))
}
...
// renderValidate emits the validate package: a validator struct per message
// (one Rule field per proto field) plus a Validate method that checks every
// field, enforcing exhaustiveness via the same field model as the handles.
func renderValidate(pkg string, msgs []*messageModel) []byte {
	imports := newImports()
	for _, m := range msgs {
		imports.add(m.PkgPath)
	}
	...
	for _, m := range msgs {
		...
		fmt.Fprintf(&b, "\n// %s is an exhaustive validator for %s. Every field must be\n", m.GoName, typeExpr)
		fmt.Fprintln(&b, "// assigned a Rule; an unset field fails validation.")
		fmt.Fprintf(&b, "type %s struct {\n", m.GoName)
		for _, f := range m.Fields {
		...
		fmt.Fprintf(&b, "func (v %s) Validate(actual %s) error {\n", m.GoName, typeExpr)
		fmt.Fprintf(&b, "\te := newEval(%q)\n", m.GoName)
		for _, f := range m.Fields {
			fmt.Fprintf(&b, "\te.field(%q, v.%s, actual.Get%s())\n", f.ProtoName, f.GoName, f.GoName)
		}
		fmt.Fprintln(&b, "\treturn e.err()")
		fmt.Fprintln(&b, "}")
	}
	return b.Bytes()
}
