...
func (e taskExecutor) recordOperationCanceled(op Operation, namespaceName, workflowType string, closeTime time.Time) {
	e.recordOperationOutcome(op, namespaceName, workflowType, nexusoperationpb.OPERATION_STATUS_CANCELED, closeTime, chasmnexus.NexusOperationCancelCount.With)
}
