# Pitcher — how a developer writes a test

The developer-facing guide to the active side. For the broader architecture read
[`PITCHER.md`](./PITCHER.md); for the model these tests plan over read
[`UMPIRE_ABOUT.md`](./UMPIRE_ABOUT.md) and [`SAAMODEL.md`](./SAAMODEL.md).

## The one idea: describe states, not steps

Umpire already lets you *judge* by declaring invariants over entity **states**. The Pitcher is
the mirror image: you *drive* by declaring the **states you want reached**, and the Pitcher
computes how to get there. You never hand-write the sequence of RPCs — you name a target and it
plans a route through the model graph (the same `umpire.Lifecycle` graph, via `Reachable`/`Cells`).

```
Umpire:  observed state ── judged by ──▶ invariants   (declare what must hold)
Pitcher: target state  ── planned by ──▶ routes       (declare what to reach)
```
...
You don't fetch a model by hand. `pitcher.DefaultModels()` is the catalog of every entity model,
and you name a target **fully-qualified by entity** — `("WorkflowUpdate", "completed")`. Both the
catalog and the Umpire's fact-routing derive from one declaration, `model.DefaultEntities()`, so
the drive side and the judge side can never disagree about which entities exist ("one model, two
consumers" — `PITCHER.md`); adding an entity is a one-line change in one place. This matters because state names are shared across
entities: both `Workflow` and `WorkflowUpdate` have a `completed` state, so `"completed"` alone is
ambiguous. Qualifying by entity resolves it, validates the state against that model, and is the
addressing scheme cross-entity targets will extend.

```go
models := pitcher.DefaultModels()

wf, _  := models.PlanTo("Workflow", "completed", pitcher.Shortest, pitcher.Constraints{})
// wf.Routes  == [[start complete]]
upd, _ := models.PlanTo("WorkflowUpdate", "completed", pitcher.Shortest, pitcher.Constraints{})
// upd.Routes == [[admit complete]]
```

(The lower-level `pitcher.PlanTo(lc, …)` that takes a raw `*umpire.Lifecycle` still exists — the
catalog is a thin, generic-free façade over it — but tests should use the catalog.)

## Plan before run (the core property)

Every Pitcher run produces an inspectable **`Plan`** before it touches the server. A `Plan` is a
...
Name the target state; the Pitcher finds the route. This is a *known regression*: the developer
...
func TestUpdateReachesCompleted(t *testing.T) {
	...
	plan, err := models.PlanTo("WorkflowUpdate", "completed", pitcher.Shortest, pitcher.Constraints{})
	require.NoError(t, err)
	require.Equal(t, [][]string{{"admit", "complete"}}, plan.Routes)

	require.NoError(t, plan.Run(ctx, driver)) // Umpire judges the resulting traffic
}
```
...
|---|---|---|
| `pitcher.Shortest` (default) | one canonical shortest route; deterministic | fast, reproducible regressions |
| `pitcher.AllRoutes` | every simple route to the target | route-dependent bugs ("does it matter *how* we got accepted?") |
| `pitcher.Random` + `WithSeed(n)` | one route chosen by the seed; reproducible | cheap variation without exhaustive cost |

```go
// Exercise every distinct way an update can reach "completed".
plan, err := models.PlanTo("WorkflowUpdate", "completed", pitcher.AllRoutes, pitcher.Constraints{})
require.NoError(t, err)
...
Instead of a target, give **constraints** that carve a sub-graph, and let the Pitcher roam it.
...
func TestExploreUpdateHappyPath(t *testing.T) {
	...
	plan, err := models.Explore("WorkflowUpdate", pitcher.Constraints{
		DenyEvents: []string{"reject", "abort"}, // stay on the success paths
		MaxDepth:   5,
	}, pitcher.WithSeed(1))
	require.NoError(t, err)

	require.NoError(t, plan.Run(ctx, driver)) // Umpire judges what the walk produces
}
```
...
## The Driver — turning a plan into real traffic

A `Plan`'s routes are *abstract model events* (`"admit"`, `"accept"`, …). A `Driver` realizes each
one as real traffic against the SUT; it is the single seam between the pure planner and a live
server.
...
A real Temporal driver is constructed for one target entity and maps events onto RPCs / worker
...
require.NoError(t, plan.Run(ctx, d)) // (2) drive it
env.Umpire().CheckNamespace(ctx, ns) // (3) Umpire judges — no hand-written assertions
```
...
2. **Negative / reachability assertions** — assert a state is *unreachable* under constraints.
   ...
   ```go
   _, err := models.PlanTo("WorkflowUpdate", "accepted", pitcher.Shortest,
       pitcher.Constraints{DenyEvents: []string{"accept"}})
...
3. **Coverage-goal tests** — "exercise *every* cell/edge in sub-graph Z at least once." The
   `Cells()` denominator from `SAAMODEL.md` #2 becomes the goal; the Pitcher plans a covering set
...
- **Built and tested today** (`tests/umpire/pitcher`, no server needed): the `DefaultModels()`
...
- **The next seam**: a concrete Temporal `Driver` (events → RPCs / worker polls), wired to the
  same client and namespace the Umpire scopes, so `Run` drives a real cluster and the Umpire judges.
