...
func (s *RegistryTestSuite) TestRegistry_RegisterComponents_Success() {
	...
	archetypeID := chasm.GenerateTypeID("TestLibrary.Component1")
	displayName, ok := r.ArchetypeDisplayName(archetypeID)
	s.Require().True(ok)
	s.Require().Equal("Component1", displayName)
	fqn, ok := r.ArchetypeFqnByID(archetypeID)
	s.Require().True(ok)
	s.Require().Equal("TestLibrary.Component1", fqn)

	missingRC, ok := r.Component("TestLibrary.Component2")
	require.False(s.T(), ok)
	require.Nil(s.T(), missingRC)
	_, ok = r.ArchetypeFqnByID(chasm.GenerateTypeID("TestLibrary.Component2"))
	s.Require().False(ok)
