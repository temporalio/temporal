...
// TestSkipPersistenceIfClean_WithNondeterministicEncoding verifies that a
// component registered with WithNondeterministicEncoding() is always added to
// UpdatedNodes when accessed via MutableContext, even when its data is unchanged.
func (s *nodeSuite) TestSkipPersistenceIfClean_WithNondeterministicEncoding() {
	s.nodeBackend.HandleNextTransitionCount = func() int64 { return 1 }
	s.nodeBackend.HandleGetCurrentVersion = func() int64 { return 1 }

	// First transaction: create and persist.
	rootNode := NewEmptyTree(s.registry, s.timeSource, s.nodeBackend, s.nodePathEncoder, s.logger, s.metricsHandler)
	rootComponent := &TestNondeterministicEncodingComponent{
		Data: &persistencespb.WorkflowExecutionState{RunId: "nde-run-id"},
	}
	...
	// Second transaction: load and touch via MutableContext without mutation.
	s.nodeBackend.HandleNextTransitionCount = func() int64 { return 2 }
	s.nodeBackend.HandleGetCurrentVersion = func() int64 { return 1 }
	...
	// WithNondeterministicEncoding opts out of skip-if-clean: node must always be persisted.
	s.Contains(secondMutation.UpdatedNodes, "", "component with WithNondeterministicEncoding must always be in UpdatedNodes")
	s.Equal(
		int64(2),
		secondMutation.UpdatedNodes[""].GetMetadata().GetLastUpdateVersionedTransition().GetTransitionCount(),
		"LastUpdateVersionedTransition must be bumped even for unchanged data when opt-out is set",
	)
}
