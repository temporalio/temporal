...
var (
	...
	protoCodec    = serialization.NewProto3Codec()
)
...
func (n *Node) serializeComponentNode() error {
	for field := range n.valueFields() {
		...
		if !field.val.IsNil() {
			var err error
			if blob, err = protoCodec.Encode(field.val.Interface().(proto.Message)); err != nil {
				return err
			}
		}
...
func (n *Node) serializeDataNode() error {
	...
	if protoValue != nil {
		var err error
		if blob, err = protoCodec.Encode(protoValue); err != nil {
			return err
		}
	}
	n.serializedNode.Data = blob
...
func unmarshalProto(
	...
	if err := protoCodec.Decode(dataBlob, value.Interface().(proto.Message)); err != nil {
...
func serializeTask(
	...
	if ok {
		return protoCodec.Encode(protoValue)
	}
	...
	for i := 0; i < taskGoType.NumField(); i++ {
		...
		var err error
		blob, err = protoCodec.Encode(fieldV.Interface().(proto.Message))
		if err != nil {
			return nil, err
		}
	}
