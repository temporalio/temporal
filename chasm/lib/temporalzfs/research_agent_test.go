...
// Run:
//
//	go test ./chasm/lib/temporalzfs/ -run TestResearchAgent -v
//
...
`)
	...
	researchDir, err := h.Mkdir(ctx, &temporalzfspb.MkdirRequest{
		NamespaceId:   nsID,
		FilesystemId:  fsID,
	...
	qcDir, err := h.Mkdir(ctx, &temporalzfspb.MkdirRequest{
		NamespaceId:   nsID,
		FilesystemId:  fsID,
	...
	sourcesFile, err := h.CreateFile(ctx, &temporalzfspb.CreateFileRequest{
		NamespaceId:   nsID,
		FilesystemId:  fsID,
	...
	writeResp, err := h.WriteChunks(ctx, &temporalzfspb.WriteChunksRequest{
		NamespaceId:  nsID,
		FilesystemId: fsID,
	...
	// Verify read back.
	readResp, err := h.ReadChunks(ctx, &temporalzfspb.ReadChunksRequest{
		NamespaceId:  nsID,
		FilesystemId: fsID,
		InodeId:      sourcesInodeID,
		Offset:       0,
	...
	_, err = h.Truncate(ctx, &temporalzfspb.TruncateRequest{
		NamespaceId:  nsID,
		FilesystemId: fsID,
		InodeId:      sourcesInodeID,
		NewSize:      0,
	...
	require.NoError(t, err)
	_, err = h.WriteChunks(ctx, &temporalzfspb.WriteChunksRequest{
		NamespaceId:  nsID,
		FilesystemId: fsID,
		InodeId:      sourcesInodeID,
		Offset:       0,
	...
	// Create analysis.md.
	analysisFile, err := h.CreateFile(ctx, &temporalzfspb.CreateFileRequest{
		NamespaceId:   nsID,
		FilesystemId:  fsID,
		ParentInodeId: qcInodeID,
		Name:          "analysis.md",
	...
	dirResp, err := h.ReadDir(ctx, &temporalzfspb.ReadDirRequest{
		NamespaceId:  nsID,
		FilesystemId: fsID,
		InodeId:      qcInodeID,
	})
	...
	reportFile, err := h.CreateFile(ctx, &temporalzfspb.CreateFileRequest{
		NamespaceId:   nsID,
		FilesystemId:  fsID,
		ParentInodeId: qcInodeID,
		Name:          "report.md",
	...
	finalDir, err := h.ReadDir(ctx, &temporalzfspb.ReadDirRequest{
		NamespaceId:  nsID,
		FilesystemId: fsID,
		InodeId:      qcInodeID,
	})
	...
	reportAttr, err := h.Getattr(ctx, &temporalzfspb.GetattrRequest{
		NamespaceId:  nsID,
		FilesystemId: fsID,
		InodeId:      reportFile.InodeId,
	})
...
func TestResearchAgent_HandlerCrashRecovery(t *testing.T) {
	...
	_, err = h.CreateFile(ctx, &temporalzfspb.CreateFileRequest{
		NamespaceId: nsID, FilesystemId: fsID,
