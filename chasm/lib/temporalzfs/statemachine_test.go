...
func TestTransitionCreate(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Verify status.
			require.Equal(t, temporalzfspb.FILESYSTEM_STATUS_RUNNING, fs.Status)
...
func TestTransitionCreate_InvalidSourceState(t *testing.T) {
	for _, status := range []temporalzfspb.FilesystemStatus{
		...
		t.Run(status.String(), func(t *testing.T) {
			ctx := newMockMutableContext()
			fs := &Filesystem{
				FilesystemState: &temporalzfspb.FilesystemState{Status: status},
			}
			err := TransitionCreate.Apply(fs, ctx, CreateEvent{})
...
func TestTransitionDelete_FromArchived(t *testing.T) {
	...
	fs := &Filesystem{
		FilesystemState: &temporalzfspb.FilesystemState{
			Status: temporalzfspb.FILESYSTEM_STATUS_ARCHIVED,
		},
	}
