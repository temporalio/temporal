...
func TestChunkGCValidate(t *testing.T) {
	...
	testCases := []struct {
		...
		{temporalzfspb.FILESYSTEM_STATUS_RUNNING, true},
		{temporalzfspb.FILESYSTEM_STATUS_UNSPECIFIED, false},
		{temporalzfspb.FILESYSTEM_STATUS_ARCHIVED, false},
		{temporalzfspb.FILESYSTEM_STATUS_DELETED, false},
	}
...
func TestManifestCompactValidate(t *testing.T) {
	...
	fs.Status = temporalzfspb.FILESYSTEM_STATUS_ARCHIVED
	ok, err = executor.Validate(nil, fs, chasm.TaskAttributes{}, nil)
	require.NoError(t, err)
	require.False(t, ok)
}
...
func TestQuotaCheckValidate(t *testing.T) {
	...
	fs.Status = temporalzfspb.FILESYSTEM_STATUS_DELETED
	ok, err = executor.Validate(nil, fs, chasm.TaskAttributes{}, nil)
	require.NoError(t, err)
	require.False(t, ok)
}
...
func TestChunkGCExecute(t *testing.T) {
	...
	err := executor.Execute(ctx, fs, chasm.TaskAttributes{}, &temporalzfspb.ChunkGCTask{})
	require.NoError(t, err)
	...
	task := ctx.Tasks[0]
	require.IsType(t, &temporalzfspb.ChunkGCTask{}, task.Payload)
...
func TestQuotaCheckExecute(t *testing.T) {
	...
	err := executor.Execute(ctx, fs, chasm.TaskAttributes{}, &temporalzfspb.QuotaCheckTask{})
	require.NoError(t, err)
