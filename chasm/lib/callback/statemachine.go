...
var TransitionAttemptFailed = chasm.NewTransition(
	...
	func(ctx chasm.MutableContext, cb *Callback, event EventAttemptFailed) error {
...
var TransitionFailed = chasm.NewTransition(
	...
	func(ctx chasm.MutableContext, cb *Callback, event EventFailed) error {
		cb.recordAttempt(event.Time)
		cb.LastAttemptFailure = &failurepb.Failure{
			Message: event.Err.Error(),
			FailureInfo: &failurepb.Failure_ApplicationFailureInfo{
...
var TransitionSucceeded = chasm.NewTransition(
	...
	func(ctx chasm.MutableContext, cb *Callback, event EventSucceeded) error {
		cb.recordAttempt(event.Time)
