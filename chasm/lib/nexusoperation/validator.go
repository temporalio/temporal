...
// permitted for the namespace and use a variant that standalone Nexus operations can persist, then
// delegates to callback.Validator for the callback contents (count, URL length, endpoint allowlist,
// header size). The delegate also lower-cases the callback headers in place.
