...
func TestNewStandaloneOperationAttachesLinks(t *testing.T) {
	...
	t.Run("WithLinks", func(t *testing.T) {
		...
		op, err := newStandaloneOperation(ctx, newStartReq(link), defaultCallbackValidator(t), newTestLinkValidator(10, 10))
	...
	t.Run("RejectsAnInvalidLink", func(t *testing.T) {
		...
		_, err := newStandaloneOperation(ctx, newStartReq(invalid), defaultCallbackValidator(t), newTestLinkValidator(10, 10))
