...
func TestTransitionTimedout(t *testing.T) {
	testCases := []struct {
		...
		{
			name:         "schedule to close timeout from paused, never started",
			startStatus:  activitypb.ACTIVITY_EXECUTION_STATUS_PAUSED,
			timeoutType:  enumspb.TIMEOUT_TYPE_SCHEDULE_TO_CLOSE,
			attemptCount: 1,
		},
		{
			name:           "schedule to close timeout from paused during retry backoff",
			startStatus:    activitypb.ACTIVITY_EXECUTION_STATUS_PAUSED,
			timeoutType:    enumspb.TIMEOUT_TYPE_SCHEDULE_TO_CLOSE,
			attemptCount:   2,
			hasStartedTime: true,
		...
		{
			name:                      "schedule to close timeout from cancel requested",
		...
		{
			name:                      "schedule to close timeout from pause requested",
			startStatus:               activitypb.ACTIVITY_EXECUTION_STATUS_PAUSE_REQUESTED,
			timeoutType:               enumspb.TIMEOUT_TYPE_SCHEDULE_TO_CLOSE,
			attemptCount:              1,
			hasStartedTime:            true,
			expectStartToCloseLatency: true,
		...
		{
			name:                      "schedule to close timeout from reset requested",
			startStatus:               activitypb.ACTIVITY_EXECUTION_STATUS_RESET_REQUESTED,
			timeoutType:               enumspb.TIMEOUT_TYPE_SCHEDULE_TO_CLOSE,
			attemptCount:              1,
			hasStartedTime:            true,
			expectStartToCloseLatency: true,
...
func TestTransitionCanceled(t *testing.T) {
	testCases := []struct {
		...
		{
			name:       "worker-confirmed cancellation without StartedTime",
			fromStatus: activitypb.ACTIVITY_EXECUTION_STATUS_CANCEL_REQUESTED,
		},
		{
