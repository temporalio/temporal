	...
	result, err := chasm.StartExecution(
		...
		func(mutableContext chasm.MutableContext, request *workflowservice.StartActivityExecutionRequest) (*Activity, error) {
			...
			if cbs := request.GetCompletionCallbacks(); len(cbs) > 0 {
				if err := newActivity.addCompletionCallbacks(mutableContext, request.GetRequestId(), cbs, h.callbackValidator, frontendReq.GetNamespace()); err != nil {
					return nil, err
				}
			}
	...
	if !result.Created && (attachCallbacks || attachLinks) {
		...
		_, _, err := chasm.UpdateComponent(
			...
			func(a *Activity, ctx chasm.MutableContext, _ any) (any, error) {
				if attachCallbacks {
					if err := a.addCompletionCallbacks(ctx, requestID, cbs, h.callbackValidator, frontendReq.GetNamespace()); err != nil {
						return nil, err
					}
				}
				if attachLinks {
