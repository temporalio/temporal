...
// Marshal an object of type ActivityListMemo to the protobuf v3 wire format
func (val *ActivityListMemo) Marshal() ([]byte, error) {
	return proto.Marshal(val)
}

// Unmarshal an object of type ActivityListMemo from the protobuf v3 wire format
func (val *ActivityListMemo) Unmarshal(buf []byte) error {
	return proto.Unmarshal(buf, val)
}

// Size returns the size of the object, in bytes, once serialized
func (val *ActivityListMemo) Size() int {
	return proto.Size(val)
}

// Equal returns whether two ActivityListMemo values are equivalent by recursively
// comparing the message's fields.
// For more information see the documentation for
// https://pkg.go.dev/google.golang.org/protobuf/proto#Equal
func (this *ActivityListMemo) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	var that1 *ActivityListMemo
	switch t := that.(type) {
	case *ActivityListMemo:
		that1 = t
	case ActivityListMemo:
		that1 = &t
	default:
