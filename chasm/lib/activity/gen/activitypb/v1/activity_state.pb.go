...
type ActivityState struct {
	...
	// Sum of the serialized size of every completion callback attached to this activity.
	// Counts only the callback specification, never the delivery bookkeeping (attempt,
	// failures, timestamps), which mutates after attach and would otherwise let an activity
	// drift over its budget on its own.
	//
	// Zero alongside a non-empty callback set means the activity was persisted before this
