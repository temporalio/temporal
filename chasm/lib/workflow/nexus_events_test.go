...
// TestNexusEventDefinitionsReportMissingOperation verifies that Apply is called with a scheduled event id that cannot be
// found in the chasm tree returns ErrNexusOperationNotFound.
func TestNexusEventDefinitionsReportMissingOperation(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.ErrorIs(t, err, ErrNexusOperationNotFound,
				"callers must be able to tell this apart from a genuine failure")
			require.NotErrorIs(t, err, ErrEventNotCherryPickable,
				"a missing operation is not the same as an event that is merely not cherry-pickable")
		})
	}
}

// TestNexusEventDefinitionsReportInvalidTransition tests that NexusEventDefinitions can report
// chasm.ErrInvalidTransition in addition to ErrNexusOperationNotFound and ErrEventNotCherryPickable.
func TestNexusEventDefinitionsReportInvalidTransition(t *testing.T) {
	...
	require.NotErrorIs(t, err, ErrNexusOperationNotFound, "the operation is present; this is not a lookup miss")
	require.NotErrorIs(t, err, ErrEventNotCherryPickable)
}
