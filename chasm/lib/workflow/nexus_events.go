...
func (d CancelRequestedEventDefinition) Apply(ctx chasm.MutableContext, wf *Workflow, event *historypb.HistoryEvent) error {
	...
	return op.RequestCancel(ctx, &nexusoperationpb.CancellationState{
		...
		// System principal ⇒ auto-close: skip the clamp to the operation's remaining timeout. Reading
		// it from the event keeps it correct across reset replay.
		AutoClose: isSystemPrincipal(event.GetPrincipal()),
	})
}

// isSystemPrincipal reports whether an event was stamped with Temporal's internal principal — i.e.
// generated by the system (e.g. the Nexus auto-close policy) rather than by a user request.
