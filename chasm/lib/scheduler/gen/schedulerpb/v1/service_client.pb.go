...
func (c *SchedulerServiceLayeredClient) callMigrateScheduleV1NoRetry(
	ctx context.Context,
	request *MigrateScheduleV1Request,
	opts ...grpc.CallOption,
) (*MigrateScheduleV1Response, error) {
	var response *MigrateScheduleV1Response
	var err error
	startTime := time.Now().UTC()
	// the caller is a namespace, hence the tag below.
	caller := headers.GetCallerInfo(ctx).CallerName
	metricsHandler := c.metricsHandler.WithTags(
		metrics.OperationTag("SchedulerService.MigrateScheduleV1"),
		metrics.NamespaceTag(caller),
		metrics.ServiceRoleTag(metrics.HistoryRoleTagValue),
	)
	metrics.ClientRequests.With(metricsHandler).Record(1)
	...
	op := func(ctx context.Context, client SchedulerServiceClient) error {
		...
		ctx, cancel := context.WithTimeout(ctx, history.DefaultTimeout)
		defer cancel()
		response, err = client.MigrateScheduleV1(ctx, request, opts...)
		return err
	}
	err = c.redirector.Execute(ctx, shardID, op)
	return response, err
}
func (c *SchedulerServiceLayeredClient) MigrateScheduleV1(
	ctx context.Context,
	request *MigrateScheduleV1Request,
	opts ...grpc.CallOption,
) (*MigrateScheduleV1Response, error) {
	call := func(ctx context.Context) (*MigrateScheduleV1Response, error) {
		return c.callMigrateScheduleV1NoRetry(ctx, request, opts...)
	}
	return backoff.ThrottleRetryContextWithReturn(ctx, call, c.retryPolicy, common.IsServiceClientTransientError)
}
