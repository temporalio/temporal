...
func (s *SpecProcessorImpl) ProcessTimeRange(
	...
	if !scheduler.useScheduledAction(false) && !manual {
		...
		wakeup, err := s.checkNextScheduleResult(scheduler, next, err)
		if err != nil {
			return nil, err
		}
	...
	nextWakeup, err := s.checkNextScheduleResult(scheduler, next, err)
	if err != nil {
		return nil, err
	}
...
func (s *SpecProcessorImpl) checkNextScheduleResult(
	...
	if err == nil {
		...
		if next.ComputeLimitWarning {
			s.metricsHandler.Counter(metrics.ScheduleComputeLimitWarning.Name()).Record(1)
	...
	if errors.Is(err, legacyscheduler.ErrComputeLimitExceeded) {
		s.metricsHandler.Counter(metrics.ScheduleComputeLimitExceeded.Name()).Record(1)
		newTaggedLogger(s.logger, scheduler).Warn(
