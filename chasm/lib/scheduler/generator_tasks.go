...
func (g *GeneratorTaskHandler) Execute(
	...
	if generator.LastProcessedTime == nil {
		...
		g.logSchedule(ctx, logger, "starting schedule", scheduler)
	}
...
func (g *GeneratorTaskHandler) logSchedule(ctx chasm.MutableContext, logger log.Logger, msg string, sched *Scheduler) {
	spec := jsonStringer{sched.Schedule.Spec}
	policies := jsonStringer{sched.Schedule.Policies}

	sched.logEvent(ctx, fmt.Sprintf("%s:\nSpec: %s\nPolicies: %s\n", msg, spec, policies))
	logger.Info(msg,
