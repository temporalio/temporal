...
[`STAMP_IMPORT.md`](./STAMP_IMPORT.md) and [`UMPIRE_DRIVER.md`](./UMPIRE_DRIVER.md).
...
| **Active side** | deferred to Driver (unbuilt) | **built**: BFS traversal + random walk generate the scenarios |
...
what `UMPIRE_PLAN.md` wants from **Driver** (the generator) *and* the **Scenario/Coverage**
...
against the *same* model. That collapses Umpire's rulebook, the deferred Driver generator, and
