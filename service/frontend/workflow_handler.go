...
func (wh *WorkflowHandler) StartWorkflowExecution(
	...
	if wh.config.EnableExperimentalAPIExample(namespaceName.String()) {
		// temporal:allow-experimental-api example
		if fooText := request.GetFooText(); fooText != "" {
			wh.logger.Debug(
				"Received experimental API example field.",
				tag.WorkflowNamespace(namespaceName.String()),
				tag.Int("foo-text-length", len(fooText)),
			)
		}
	}
