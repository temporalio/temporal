...
func (wh *WorkflowHandler) CreateSchedule(
	...
	searchattribute.AddSearchAttribute(&request.SearchAttributes, defs.TemporalNamespaceDivision, payload.EncodeString(scheduler.NamespaceDivision))
...
func (wh *WorkflowHandler) StartBatchOperation(
	...
	case *workflowservice.StartBatchOperationRequest_UnpauseActivitiesOperation:
		...
		case *batchpb.BatchOperationUnpauseActivities_Type:
			...
			input.Request.VisibilityQuery = fmt.Sprintf("%s = %s", defs.TemporalPauseInfo, escapedSearchValue)
		case *batchpb.BatchOperationUnpauseActivities_MatchAll:
			wildCardUnpause := fmt.Sprintf("%s STARTS_WITH 'property:activityType='", defs.TemporalPauseInfo)
	...
	searchattribute.AddSearchAttribute(&searchAttributes, defs.BatcherUser, payload.EncodeString(identity))
	searchattribute.AddSearchAttribute(&searchAttributes, defs.TemporalNamespaceDivision, payload.EncodeString(batcher.NamespaceDivision))
...
func (wh *WorkflowHandler) DescribeBatchOperation(
	...
	var identity string
	encodedBatcherIdentity := executionInfo.GetSearchAttributes().GetIndexedFields()[defs.BatcherUser]
...
func (wh *WorkflowHandler) cleanScheduleSearchAttributes(searchAttributes *commonpb.SearchAttributes) *commonpb.SearchAttributes {
	...
	delete(fields, defs.TemporalSchedulePaused)
	...
	delete(fields, defs.BinaryChecksums)
	delete(fields, defs.BuildIds)
	// all schedule workflows should be in this namespace division so there's no need to include it
	delete(fields, defs.TemporalNamespaceDivision)
