...
// --- driving a real simplePartitionScaler on the fake clock ---
//
// The tests above use MockPartitionScaler, which is right for checking what the manager
// does with a decision. The window-sizing tests below need the opposite: the manager's real
// cooldown and batching behavior feeding a real simplePartitionScaler, so that the tracker
// arithmetic in task_tracker_test.go can be checked against what the loop actually does.

// scalerCall records one OnTasks call that got past the cooldown gate.
type scalerCall struct {
	at        time.Duration // fake time since the run started
	numTasks  int           // tasks handed over in this call
	newTarget int           // decision, or -1 for NoChange
}

// recordingScaler wraps a real PartitionScaler and records every call that reaches it,
// which is only the calls the manager's cooldown gate lets through.
type recordingScaler struct {
	inner PartitionScaler
	ts    clock.TimeSource
	start time.Time
	calls []scalerCall
}

func (r *recordingScaler) OnTasks(in PartitionScalerInput) PartitionScalerDecision {
	dec := r.inner.OnTasks(in)
	target := dec.NewTarget
	if dec.NoChange {
		target = -1
	}
	r.calls = append(r.calls, scalerCall{
		at:        r.ts.Now().Sub(r.start),
...
func (r *recordingScaler) Stop() { r.inner.Stop() }

// targets returns the sequence of distinct applied targets, i.e. the trajectory the queue's
// partition count actually followed.
func (r *recordingScaler) targets() []int {
	var out []int
	for _, c := range r.calls {
		if c.newTarget < 0 {
			continue
		}
		if len(out) == 0 || out[len(out)-1] != c.newTarget {
			out = append(out, c.newTarget)
		}
	}
	return out
}

// changes is the number of applied target changes, i.e. one persistence write plus one
// ephemeral push to every client of the queue each.
func (r *recordingScaler) changes() int {
	return len(r.targets())
}

// lastChangeAt is when the target last moved. A run that has converged stops moving early;
// one that oscillates is still moving at the end.
func (r *recordingScaler) lastChangeAt() time.Duration {
	var at time.Duration
	prev := -1
	for _, c := range r.calls {
		if c.newTarget >= 0 && c.newTarget != prev {
			at, prev = c.at, c.newTarget
		}
	}
	return at
}

// decisionRangeAfter is the min and max target decided from t onwards, counting every
// decision and not just the ones that changed the applied target -- i.e. the band the
// measured rate implies once the first window has warmed up.
func (r *recordingScaler) decisionRangeAfter(t time.Duration) (lo, hi int) {
	lo = math.MaxInt
	for _, c := range r.calls {
		if c.at < t || c.newTarget < 0 {
			continue
		}
		lo, hi = min(lo, c.newTarget), max(hi, c.newTarget)
	}
	return lo, hi
}

// driveRealScaler runs a real simplePartitionScaler through a real scaleManager under a
// steady whole-queue arrival rate for duration of fake time, and returns everything the
// scaler saw.
//
// The manager's worker goroutine is deliberately not started. Instead the loop below is a
// synchronous stand-in for backgroundWork's wakeup branch: it feeds the real AddedTasks,
// takes the real wakeup signal off the real channel, and calls the real callScaler. So the
// batch threshold, the cooldown gate, and the fact that the batch keeps accumulating while
// the gate is closed are all the production code paths -- only the scheduling is
// deterministic.
//
// Arrivals are modelled as one AddedTasks(1) per task, i.e. a single write partition. With
// P partitions the caller passes numTasks=P per task (see signalPartitionScaler), so both
// the batch threshold and the reported counts scale by P; that changes the call cadence but
// not the mechanism these tests are about.
func (s *ScaleManagerSuite) driveRealScaler(
	scalerSettings dynamicconfig.SimplePartitionScalerSettings,
	tasksPerSecond int,
	duration time.Duration,
) *recordingScaler {
	s.T().Helper()

	s.timeSource.Update(time.Unix(0, 0))
	start := s.timeSource.Now()

	rec := &recordingScaler{
		inner: newSimplePartitionScaler(dynamicconfig.GetTypedPropertyFn(scalerSettings), s.timeSource),
		ts:    s.timeSource,
		start: start,
	}

	s.scaleDB.EXPECT().UpdateScaleState(gomock.Any(), gomock.Any()).Return(nil).AnyTimes()
	s.userData.EXPECT().SetPartitionScale(gomock.Any()).AnyTimes()
	...
	// Stand in for Start without launching backgroundWork, so the loop below is the only
	// thing that ever calls callScaler.
	sm.scaleDB = s.scaleDB
	sm.setState(nil, s.settings)

	// Step finer than the tracker's bucket size so arrivals are smooth at bucket
	// granularity, and carry the fractional remainder so the rate is exact over the run.
	const step = 10 * time.Millisecond
	pending := 0.0
	for elapsed := time.Duration(0); elapsed < duration; elapsed += step {
		s.timeSource.Advance(step)

		pending += float64(tasksPerSecond) * step.Seconds()
		for ; pending >= 1; pending-- {
			sm.AddedTasks(1)
		}

		// backgroundWork's wakeup branch, minus the goroutine.
		select {
		case <-sm.wakeup:
			sm.callScaler()
		default:
...
// windowSizingSettings is the scaler config the two window-sizing tests share: one Up and
// one Down window of the same length, with the Down TargetRate half the Up one to leave the
// deadband the SimplePartitionScalerSettings doc asks for.
func windowSizingSettings(window time.Duration) dynamicconfig.SimplePartitionScalerSettings {
	return dynamicconfig.SimplePartitionScalerSettings{
		Enabled: true,
		Downs:   []dynamicconfig.SimplePartitionScalerThreshold{{Window: window, TargetRate: 50}},
		Ups:     []dynamicconfig.SimplePartitionScalerThreshold{{Window: window, TargetRate: 100}},
	}
}

// logTrajectory logs every point at which the applied target moved.
func (s *ScaleManagerSuite) logTrajectory(window time.Duration, rec *recordingScaler) {
	s.T().Logf("window=%v: %d calls past the cooldown gate, %d applied changes, last at %v",
		window, len(rec.calls), rec.changes(), rec.lastChangeAt())
	prev := -1
	for _, c := range rec.calls {
		if c.newTarget >= 0 && c.newTarget != prev {
			s.T().Logf("  at=%-9v tasks=%-6d -> target=%d", c.at, c.numTasks, c.newTarget)
			prev = c.newTarget
		}
	}
}

// TestShortWindowOscillatesAgainstCooldown confirms the failure mode that the tracker
// arithmetic in TestTaskTrackerWindowSizingForProdSettings predicts: a rate window shorter
// than the MaxRate cooldown does not just read high by a fixed factor, it never converges.
//
// The coupling is in callScaler: while now < nextDecision it returns before swapping the
// batch, so during a cooldown no tasks are handed to the tracker at all while arrivals keep
// piling into the batch counter. The next call then hands over the whole cooldown's worth at
// once, and a window shorter than the cooldown divides that lump by the window.
//
// With MaxRate 0.33 (a 3.03s cooldown), a 1s window -- the window that goes with today's
// 100ms buckets -- and a steady, perfectly stable 1000 tasks/s against an Up TargetRate of
// 100, the correct answer is 10 partitions for the whole run. What actually happens is a
// sawtooth with a ~36s period, between 2x and 4x too high:
//
//	t=1.1s   first full window: 11 calls x 100 tasks over 1s reads 1100/s -> target 11
//	         (the +10% is the 1 + interval/window bias), and arms the cooldown
//	t=4.14s  gate reopens carrying 3040 tasks; the 1s window holds nothing else, so it
//	         reads ~2900/s -> target 29, and arms the cooldown again
//	...      the lump stays in the 1s window for another second, so the calls right after
//	         each cooldown read higher still and ratchet the target up one at a time, each
//	         change re-arming the cooldown. ~3.3s per +1.
//	t=44.1s  target has reached 39. This is the first call where the lump has aged out
//	         before the target was exceeded, so the window finally reads the true 1000/s --
//	         and Downs fire, round(1000/50) = 20, collapsing the target
//	t=47.1s  the collapse armed the cooldown, so the next call gets another 3040-task lump
//	         and the whole cycle starts over
//
// Nothing about the load changed across any of that. Every one of those steps is a
// persistence write plus an ephemeral push that reconfigures every client of the queue.
func (s *ScaleManagerSuite) TestShortWindowOscillatesAgainstCooldown() {
	// MatchingPartitionScaleManager defaults, except that the background timer is disabled
	// so the batch threshold is the only thing driving calls.
	s.settings.MaxRate = 0.33
	s.settings.BatchSize = 100
	s.settings.BackgroundInterval = time.Hour

	const window = time.Second
	const tasksPerSecond = 1000
	const trueTarget = tasksPerSecond / 100 // TargetRate 100 => 10 partitions
	const duration = 90 * time.Second

	rec := s.driveRealScaler(windowSizingSettings(window), tasksPerSecond, duration)
	s.logTrajectory(window, rec)

	// Still moving at the very end of the run: this does not converge, it cycles.
	s.Greater(rec.lastChangeAt(), 4*duration/5,
		"target should still be moving at the end of the run")
	s.GreaterOrEqual(rec.changes(), 20,
		"a stable rate should not produce this many partition changes in %v", duration)

	// Past warm-up, the target never comes back within 2x of correct and reaches ~4x.
	lo, hi := rec.decisionRangeAfter(window + 5*time.Second)
	s.GreaterOrEqual(lo, 2*trueTarget,
		"the low end of the sawtooth is still twice the correct target of %d", trueTarget)
	s.GreaterOrEqual(hi, 3*trueTarget,
		"the high end of the sawtooth reaches at least three times the correct target")
	s.GreaterOrEqual(hi-lo, 15, "the sawtooth should swing across a wide band")
}

// TestWindowLongerThanCooldownConverges is the control for the test above: the same load and
// the same manager settings, with only the window changed. Once the window is at least ~10x
// the cooldown the lump is a small perturbation instead of the whole measurement, and the
// target converges and stays put.
//
// The settled targets match the ratios pinned in TestTaskTrackerWindowSizingReference for a
// 3s call interval: 1.20 for a 10s window and 1.10 for a 30s window, i.e. readings of
// ~1200/s and ~1100/s against a true 1000/s, so round(rate/100) gives 12 and 11. The 10s
// case then sits one higher than that, because Ups only raise and the Down threshold
// (round(rate/50) = 20) is far above it, so the deadband holds whatever the highest reading
// reached.
func (s *ScaleManagerSuite) TestWindowLongerThanCooldownConverges() {
	s.settings.MaxRate = 0.33 // 3.03s cooldown, same as above
	s.settings.BatchSize = 100
	s.settings.BackgroundInterval = time.Hour
	...
	for _, tc := range []struct {
		...
		{window: 10 * time.Second, wantSettled: 13}, // ~3.3x the cooldown
		{window: 30 * time.Second, wantSettled: 11}, // ~10x the cooldown
	} {
		s.Run(tc.window.String(), func() {
			rec := s.driveRealScaler(windowSizingSettings(tc.window), tasksPerSecond, duration)
			s.logTrajectory(tc.window, rec)

			targets := rec.targets()
			s.Require().NotEmpty(targets, "no decision was ever applied")
			s.Equal(tc.wantSettled, targets[len(targets)-1], "settled target")

			// Converged: it stops moving well before the run ends, and it gets there in a
			// handful of steps rather than cycling. Compare the 20+ changes, still going at
			// the end of the run, in the 1s case.
			s.Less(rec.lastChangeAt(), 2*duration/3, "target should have stopped moving")
			s.LessOrEqual(rec.changes(), 4, "should converge in a few steps, not cycle")

			// The settled value is a slow one-way ratchet up from the correct 10, not a
			// swing: every decision after warm-up sits in [10, settled].
			lo, hi := rec.decisionRangeAfter(tc.window + 5*time.Second)
			s.Equal(tasksPerSecond/100, lo, "the correct target should still be among the readings")
			s.Equal(tc.wantSettled, hi, "no reading should exceed the settled target")
		})
	}
}
