...
func TestRegistryImpl_RecordStorageDriverMetric(t *testing.T) {
	t.Run("disabled when ExternalPayloadsEnabled is false", func(t *testing.T) {
		...
		params.ExternalPayloadsEnabled = dynamicconfig.GetBoolPropertyFnFilteredByNamespace(false)
		params.StorageDriverMetricsAllowedTypes = dynamicconfig.GetTypedPropertyFn([]string{"s3"})
		r := newRegistryImpl(params)

		r.recordStorageDriverMetric("test-ns", []*workerpb.WorkerHeartbeat{
			{
				WorkerInstanceKey: "worker1",
	...
	t.Run("allowed driver types pass through", func(t *testing.T) {
		captureHandler := metricstest.NewCaptureHandler()
		capture := captureHandler.StartCapture()
		defer captureHandler.StopCapture(capture)

		params := testDefaultRegistryParams(captureHandler)
		params.ExternalPayloadsEnabled = dynamicconfig.GetBoolPropertyFnFilteredByNamespace(true)
		params.StorageDriverMetricsAllowedTypes = dynamicconfig.GetTypedPropertyFn([]string{"s3"})
		r := newRegistryImpl(params)
	...
	t.Run("unknown driver types sanitized to other", func(t *testing.T) {
		captureHandler := metricstest.NewCaptureHandler()
		capture := captureHandler.StartCapture()
		defer captureHandler.StopCapture(capture)
		...
		r.recordStorageDriverMetric("test-ns", []*workerpb.WorkerHeartbeat{
			{
				WorkerInstanceKey: "worker1",
				Drivers:           []*workerpb.StorageDriverInfo{{Type: "gcs"}},
			},
		})
		...
		recordings := snap["worker_storage_driver_type"]
		require.Len(t, recordings, 1)
		assert.Equal(t, "other", recordings[0].Tags["worker_storage_driver_type"])
	})
