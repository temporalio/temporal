...
func TestUpdateAndListNamespace(t *testing.T) {
	...
	list := m.ListNamespace("ns1", alwaysTrue)
...
func TestListNamespacePredicate(t *testing.T) {
	m := NewRegistry(1, time.Hour, 0, 10, time.Hour)
	defer m.Stop()
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			list := m.ListNamespace("ns", tc.pred)
...
func TestEvictByTTL(t *testing.T) {
	m := NewRegistry(1, 1*time.Second, 0, 10, time.Hour)
	defer m.Stop()
	...
	list := m.ListNamespace("ns", alwaysTrue)
...
func TestEvictByCapacity(t *testing.T) {
	maxItems := int64(3)
	m := NewRegistry(1, time.Hour, 0, maxItems, time.Hour)
	defer m.Stop()
	...
	remaining := m.ListNamespace("ns", alwaysTrue)
...
func BenchmarkUpdate(b *testing.B) {
	m := NewRegistry(16, time.Hour, time.Minute, int64(b.N), time.Hour)
	defer m.Stop()
...
func BenchmarkListNamespace(b *testing.B) {
	m := NewRegistry(16, time.Hour, time.Minute, 1000, time.Hour)
	defer m.Stop()
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = m.ListNamespace("benchNs", alwaysTrue)
	}
}
...
func BenchmarkRandomUpdate(b *testing.B) {
	...
	m := NewRegistry(len(namespaces), time.Hour, time.Minute, int64(b.N), time.Hour)
	defer m.Stop()
