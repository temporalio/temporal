...
func TestRegistryImpl_RecordWorkerHeartbeat(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			setup: func(r *registryImpl) {
				r.upsertHeartbeats("namespace1", "namespace1", nil /* principal */, []*workerpb.WorkerHeartbeat{{
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(r *registryImpl) {
				...
				// Setup namespace2
				r.upsertHeartbeats("namespace2", "namespace2", nil /* principal */, []*workerpb.WorkerHeartbeat{{
					WorkerInstanceKey: "worker2",
				}})
			},
			nsID:            "namespace1",
...
func TestRegistryImpl_ListWorkersPagination(t *testing.T) {
	...
	r.upsertHeartbeats("ns1", "ns1", nil /* principal */, []*workerpb.WorkerHeartbeat{
