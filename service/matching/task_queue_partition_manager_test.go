...
// setupPartitionManagerWithTaskAddHooks creates a partition manager with the given task match hooks.
func (s *PartitionManagerTestSuite) setupPartitionManagerWithTaskAddHooks(taskAddHooks []hooks.TaskAddHook) (*taskQueuePartitionManagerImpl, func()) {
	f, err := tqid.NewTaskQueueFamily(namespaceID, taskQueueName)
	s.Require().NoError(err)
	partition := f.TaskQueue(enumspb.TASK_QUEUE_TYPE_WORKFLOW).RootPartition()
	tqConfig := newTaskQueueConfig(partition.TaskQueue(), s.partitionMgr.engine.config, s.partitionMgr.ns.Name())

	pm, err := newTaskQueuePartitionManager(s.partitionMgr.engine, s.partitionMgr.ns, partition, tqConfig, s.partitionMgr.logger, s.partitionMgr.throttledLogger, metrics.NoopMetricsHandler, s.userDataMgr, taskAddHooks)
	s.Require().NoError(err)
	pm.Start()
...
func (s *PartitionManagerTestSuite) TestTaskAddHooks_AddHookSyncMatch() {
	...
	pm, cleanup := s.setupPartitionManagerWithTaskAddHooks([]hooks.TaskAddHook{hook})
	defer cleanup()
...
func (s *PartitionManagerTestSuite) TestTaskAddHooks_MultipleHooksInvoked() {
	...
	pm, cleanup := s.setupPartitionManagerWithTaskAddHooks([]hooks.TaskAddHook{hook1, hook2})
	defer cleanup()
