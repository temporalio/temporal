...
func TestLiveness(t *testing.T) {
	...
	systemClock := clock.NewEventTimeSource()
	liveness := newLiveness(systemClock, ttl, func() { idleCalled.Store(1) })
	liveness.Start()
	systemClock.Advance(1 * time.Second)
...
func TestLivenessStop(t *testing.T) {
	...
	liveness := newLiveness(systemClock, ttl, func() { idleCalled.Store(1) })
	liveness.Start()
	systemClock.Advance(500 * time.Millisecond)
	liveness.Stop()
	systemClock.Advance(1 * time.Second)
	assert.Equal(t, int32(0), idleCalled.Load())
