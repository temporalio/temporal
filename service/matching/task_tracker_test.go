...
func TestAddTasksRate(t *testing.T) {
	...
	systemClock.Update(currentTime)

	tr := newTaskTracker(systemClock)
	...
	systemClock.Advance(1 * time.Second)                 // time: 1 second
	...
	systemClock.Advance(5 * time.Second)
	...
	systemClock.Advance(24 * time.Second) // time: 30 second
	...
	systemClock.Advance(5 * time.Second) // time: 35 second
	...
	systemClock.Advance(15 * time.Second) // time: 50 second
	trackTasksHelper(tr, 10)
	...
	systemClock.Advance(60 * time.Second)
