...
func (s *activityReplicatorStateSuite) TestVersionHistory_GetCurrentError() {
	namespaceID := tests.NamespaceID
	workflowID := tests.WorkflowID
	runID := uuid.NewString()

	// CurrentVersionHistoryIndex out of range -> GetCurrentVersionHistory errors.
	badVersionHistories := &historyspb.VersionHistories{
		CurrentVersionHistoryIndex: 5,
		Histories:                  []*historyspb.VersionHistory{},
	}
	incomingVersionHistory := &historyspb.VersionHistory{
		BranchToken: []byte{},
	...
	s.mockMutableState.EXPECT().GetExecutionInfo().Return(&persistencespb.WorkflowExecutionInfo{
		VersionHistories: badVersionHistories,
	}).AnyTimes()
...
func (s *activityReplicatorStateSuite) TestVersionHistory_LastIncomingItemError() {
	namespaceID := tests.NamespaceID
	workflowID := tests.WorkflowID
	runID := uuid.NewString()
	...
	// Empty incoming version history -> GetLastVersionHistoryItem errors.
	incomingVersionHistory := &historyspb.VersionHistory{
		BranchToken: []byte{},
		Items:       []*historyspb.VersionHistoryItem{},
	}
...
func (s *activityReplicatorStateSuite) TestSyncActivity_LoadMutableStateError() {
	...
	actReplLoadErr := serviceerror.NewInternal("load mutable state failed")
	weContext := historyi.NewMockWorkflowContext(s.controller)
	weContext.EXPECT().LoadMutableState(gomock.Any(), s.mockShard).Return(nil, actReplLoadErr)
	...
	err = s.nDCActivityStateReplicator.SyncActivityState(context.Background(), request)
	s.Error(err)
	s.Equal(actReplLoadErr, err)
}

func (s *activityReplicatorStateSuite) TestSyncActivities_LoadMutableStateError() {
	namespaceName := tests.Namespace
	namespaceID := tests.NamespaceID
	workflowID := tests.WorkflowID
	runID := uuid.NewString()
...
func (s *activityReplicatorStateSuite) TestSyncActivity_ActivityFound_NonZombie_LegacyUpdate() {
	// EnableUpdateWorkflowModeIgnoreCurrent == false exercises the legacy
	// UpdateWorkflowExecutionWithNew path with UpdateWorkflowModeUpdateCurrent.
	s.mockShard.GetConfig().EnableUpdateWorkflowModeIgnoreCurrent = func() bool { return false }
	...
	localVersionHistories := &historyspb.VersionHistories{
		...
		Histories: []*historyspb.VersionHistory{{
			BranchToken: []byte{},
			Items: []*historyspb.VersionHistoryItem{
				{EventId: scheduledEventID + 10, Version: version},
			},
		}},
	}
	incomingVersionHistory := &historyspb.VersionHistory{
		BranchToken: []byte{},
		Items: []*historyspb.VersionHistoryItem{
			{EventId: scheduledEventID, Version: version},
		},
	}
	...
	s.mockMutableState.EXPECT().UpdateActivityInfo(gomock.Any(), false).Return(nil)
	s.mockMutableState.EXPECT().ShouldResetActivityTimerTaskMask(gomock.Any(), gomock.Any()).Return(false)
	s.mockMutableState.EXPECT().GetPendingActivityInfos().Return(map[int64]*persistencespb.ActivityInfo{})
...
func (s *activityReplicatorStateSuite) TestSyncActivities_ActivityFound_Zombie_LegacyUpdate() {
	// EnableUpdateWorkflowModeIgnoreCurrent == false + zombie state exercises the
	// legacy UpdateWorkflowExecutionWithNew path with UpdateWorkflowModeBypassCurrent.
	s.mockShard.GetConfig().EnableUpdateWorkflowModeIgnoreCurrent = func() bool { return false }
...
func (s *activityReplicatorStateSuite) TestSyncActivity_ActivityRetry() {
	// StartedEventId == EmptyEventID and incoming Attempt > local attempt triggers
	// AddTasks(ActivityRetryTimerTask).
	namespaceName := tests.Namespace
	namespaceID := tests.NamespaceID
	workflowID := tests.WorkflowID
	runID := uuid.NewString()
	...
	s.mockMutableState.EXPECT().AddTasks(gomock.Any())
	s.mockMutableState.EXPECT().UpdateActivityInfo(gomock.Any(), false).Return(nil)
	s.mockMutableState.EXPECT().ShouldResetActivityTimerTaskMask(gomock.Any(), gomock.Any()).Return(false)
	s.mockMutableState.EXPECT().GetPendingActivityInfos().Return(map[int64]*persistencespb.ActivityInfo{})
...
func (s *activityReplicatorStateSuite) TestSyncActivity_UpdateActivityInfoError() {
	...
	actReplUpdateErr := serviceerror.NewInternal("update activity info failed")
	s.mockMutableState.EXPECT().UpdateActivityInfo(gomock.Any(), false).Return(actReplUpdateErr)
	s.mockMutableState.EXPECT().ShouldResetActivityTimerTaskMask(gomock.Any(), gomock.Any()).Return(false)
