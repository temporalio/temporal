...
func (s *stateRebuilderSuite) stateRbExpectReadHistory(branchToken []byte, lastEventID int64, version int64) {
	firstEventID := common.FirstEventID
	nextEventID := lastEventID + 1
	events1 := []*historypb.HistoryEvent{{
		EventId:   1,
	...
	s.mockExecutionManager.EXPECT().ReadHistoryBranchByBatch(gomock.Any(), &persistence.ReadHistoryBranchRequest{
		...
		History:        []*historypb.History{{Events: events1}, {Events: events2}},
		TransactionIDs: []int64{10, 20},
		NextPageToken:  nil,
		Size:           100,
	}, nil)
}

func (s *stateRebuilderSuite) stateRbExpectNamespace(targetNamespaceID namespace.ID) {
	s.mockNamespaceCache.EXPECT().GetNamespaceByID(targetNamespaceID).Return(namespace.NewGlobalNamespaceForTest(
		&persistencespb.NamespaceInfo{Id: targetNamespaceID.String(), Name: "stateRb namespace"},
		&persistencespb.NamespaceConfig{},
		&persistencespb.NamespaceReplicationConfig{
			ActiveClusterName: cluster.TestCurrentClusterName,
			Clusters: []string{
...
func (s *stateRebuilderSuite) TestRebuild_NamespaceLookupError() {
	targetNamespaceID := namespace.ID(uuid.NewString())
	s.mockNamespaceCache.EXPECT().GetNamespaceByID(targetNamespaceID).Return(nil, serviceerror.NewNotFound("namespace not found"))

	_, _, err := s.nDCStateRebuilder.Rebuild(
		context.Background(),
		s.now,
		definition.NewWorkflowKey(s.namespaceID.String(), s.workflowID, s.runID),
...
func (s *stateRebuilderSuite) TestRebuild_DataLossError() {
	targetNamespaceID := namespace.ID(uuid.NewString())
	branchToken := []byte("stateRb branch token")
	s.stateRbExpectNamespace(targetNamespaceID)
...
func (s *stateRebuilderSuite) TestRebuild_ReadHistoryError() {
	targetNamespaceID := namespace.ID(uuid.NewString())
	branchToken := []byte("stateRb branch token")
	s.stateRbExpectNamespace(targetNamespaceID)
	s.mockExecutionManager.EXPECT().ReadHistoryBranchByBatch(gomock.Any(), gomock.Any()).Return(
		nil, serviceerror.NewInternal("read failed"))
...
func (s *stateRebuilderSuite) TestRebuild_VersionMismatchError() {
	targetNamespaceID := namespace.ID(uuid.NewString())
	version := int64(12)
	lastEventID := int64(2)
	branchToken := []byte("stateRb branch token")
	s.stateRbExpectNamespace(targetNamespaceID)
	s.stateRbExpectReadHistory(branchToken, lastEventID, version)

	// pass a baseLastEventVersion that does not match the rebuilt last item version
	_, _, err := s.nDCStateRebuilder.Rebuild(
		context.Background(),
		s.now,
		definition.NewWorkflowKey(s.namespaceID.String(), s.workflowID, s.runID),
...
func (s *stateRebuilderSuite) TestRebuild_TaskRefresherError() {
	targetNamespaceID := namespace.ID(uuid.NewString())
	version := int64(12)
	lastEventID := int64(2)
	branchToken := []byte("stateRb branch token")
	s.stateRbExpectNamespace(targetNamespaceID)
	s.stateRbExpectReadHistory(branchToken, lastEventID, version)
	s.mockTaskRefresher.EXPECT().Refresh(gomock.Any(), gomock.Any(), gomock.Any()).Return(serviceerror.NewInternal("refresh failed"))
...
func (s *stateRebuilderSuite) TestRebuildWithCurrentMutableState_NamespaceLookupError() {
	targetNamespaceID := namespace.ID(uuid.NewString())
	s.mockNamespaceCache.EXPECT().GetNamespaceByID(targetNamespaceID).Return(nil, serviceerror.NewNotFound("namespace not found"))
	...
	_, _, err := s.nDCStateRebuilder.RebuildWithCurrentMutableState(
		context.Background(),
		s.now,
		definition.NewWorkflowKey(s.namespaceID.String(), s.workflowID, s.runID),
		[]byte("stateRb branch token"),
...
func (s *stateRebuilderSuite) TestRebuildWithCurrentMutableState_TaskRefresherError() {
	targetNamespaceID := namespace.ID(uuid.NewString())
	version := int64(12)
	lastEventID := int64(2)
	branchToken := []byte("stateRb branch token")
	...
	currentMutableState := &persistencespb.WorkflowMutableState{
		ExecutionState: &persistencespb.WorkflowExecutionState{
			RequestIds: map[string]*persistencespb.RequestIDInfo{
				uuid.NewString(): {EventType: enumspb.EVENT_TYPE_WORKFLOW_EXECUTION_STARTED},
			},
		},
		ExecutionInfo: &persistencespb.WorkflowExecutionInfo{
			TransitionHistory: []*persistencespb.VersionedTransition{
				{TransitionCount: 10, NamespaceFailoverVersion: 12},
			},
		},
	}
	_, _, err := s.nDCStateRebuilder.RebuildWithCurrentMutableState(
