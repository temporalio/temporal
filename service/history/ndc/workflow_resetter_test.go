...
func (s *workflowResetterSuite) TestCherryPickChasmEvent() {
	...
	// Wrapped on purpose -- chasm/lib/workflow returns the sentinel bare, which the reapply tests below cover, so
	// this case pins that the match is errors.Is and not identity.
	notFoundErr := fmt.Errorf("wrapped: %w", chasmworkflow.ErrNexusOperationNotFound)
	...
	testCases := []struct {
		...
		{
			// A workflow with no CHASM tree cannot own the operation. Erroring here would abort the whole reapply
			// batch over an event that simply belongs to a tree this workflow doesn't have.
			name:        "chasm disabled is skipped, not an error",
...
func (s *workflowResetterSuite) TestReapplyEventsHSMNotFoundFallsBackToChasm() {
	...
	s.Run("neither tree owns the operation, so the event is skipped", func() {
		...
		applied, err := reapplyEvents(
			...
			s.newChasmRegistryWithEvent(eventType, chasmworkflow.ErrNexusOperationNotFound),
			[]*historypb.HistoryEvent{event}, nil, "", false, s.logger,
		)
...
func (s *workflowResetterSuite) TestReapplyEventsOrphanedOperationDoesNotDiscardBatch() {
	...
	errByEventID := map[int64]error{
		ownedEventID:  nil,
		orphanEventID: chasmworkflow.ErrNexusOperationNotFound,
	}
