...
func (t *transferQueueActiveTaskExecutor) Execute(
	...
	case *tasks.ActivityCancelControlTask:
		err = t.processActivityCancelControl(ctx, task)
	default:
...
func (t *transferQueueActiveTaskExecutor) processActivityCancelControl(
	ctx context.Context,
	task *tasks.ActivityCancelControlTask,
) (retError error) {
	if !t.config.EnableActivityCancellationViaControlQueue() {
		return nil
	}
