...
func (ms *MutableStateImpl) reschedulePendingWorkflowTask(increaseWorkflowTaskStamp bool) error {
	...
	//
	// Note that this only happens if the caller requested it. This is because
	// in the versioning context,
	if increaseWorkflowTaskStamp {
		ms.executionInfo.WorkflowTaskStamp += 1
	}

	// Reset the attempt as well; forcing a non-transient workflow task to be scheduled.
	ms.executionInfo.Attempt = 1

	// For a non-speculative WFT, reschedule it.
