...
// TestTaskGeneratorImpl_RegenerateTimerTasksForTimeSkipping_BoundTimer verifies the
// elapsed-duration bound timer is regenerated when BoundTargetTime is set, and
// skipped when it is not.
func TestTaskGeneratorImpl_RegenerateTimerTasksForTimeSkipping_BoundTimer(t *testing.T) {
	t.Parallel()

	skippedDuration := time.Hour
	boundTarget := time.Now().UTC().Add(2 * time.Hour)

	for _, tc := range []struct {
		name            string
		boundTargetTime *timestamppb.Timestamp
		wantBoundTimer  bool
	}{
		{
			name:            "BoundTargetTime set: bound timer is regenerated",
			boundTargetTime: timestamppb.New(boundTarget),
			wantBoundTimer:  true,
		},
		{
			name:            "BoundTargetTime nil: bound timer is not regenerated",
			boundTargetTime: nil,
			wantBoundTimer:  false,
		},
	} {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			...
			mutableState.EXPECT().GetExecutionInfo().Return(&persistencespb.WorkflowExecutionInfo{
				TimeSkippingInfo: &persistencespb.TimeSkippingInfo{
					AccumulatedSkippedDuration: durationpb.New(skippedDuration),
					BoundTargetTime:            tc.boundTargetTime,
				},
			}).AnyTimes()
			...
			for _, task := range captured {
				if bt, ok := task.(*tasks.TimeSkippingTimerTask); ok {
					require.Nil(t, bound, "bound timer task emitted more than once")
					bound = bt
				}
			}
			if tc.wantBoundTimer {
				require.NotNil(t, bound, "expected TimeSkippingTimerTask")
				require.Equal(t, tests.WorkflowKey, bound.WorkflowKey)
				require.Equal(t, boundTarget, bound.VisibilityTimestamp)
				require.Equal(t, int64(0), bound.TaskID, "TaskID must be zero (set by shard)")
			} else {
				require.Nil(t, bound, "no bound timer task expected when BoundTargetTime is nil")
			}
		})
	}
}
