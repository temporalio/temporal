...
// TestAddCompletedWorkflowEvent_RetentionConvertsVirtualToWall is the archival test's
// sibling for the non-archival branch of GenerateWorkflowCloseTasks. Retention is a
// wall-clock storage lifetime: it must run from *real* close time, so accumulated skip
// has to cancel out of the DeleteHistoryEventTask's VisibilityTimestamp.
func (s *mutableStateSuite) TestAddCompletedWorkflowEvent_RetentionConvertsVirtualToWall() {
	const skipped = 60 * 24 * time.Hour

	// Archival stays disabled so GenerateWorkflowCloseTasks takes the else branch.
	s.mockConfig.RetentionTimerJitterDuration = func() time.Duration { return 0 }
	retention := s.namespaceEntry.Retention()
	...
	s.Empty(s.mutableState.InsertTasks[tasks.CategoryArchival],
		"archival must be disabled for this test to exercise the DeleteHistoryEventTask branch")

	var deleteTask *tasks.DeleteHistoryEventTask
	for _, t := range s.mutableState.InsertTasks[tasks.CategoryTimer] {
		if dt, ok := t.(*tasks.DeleteHistoryEventTask); ok {
			s.Require().Nil(deleteTask, "DeleteHistoryEventTask emitted more than once")
			deleteTask = dt
		}
	}
	s.Require().NotNil(deleteTask, "no DeleteHistoryEventTask in CategoryTimer")

	s.False(deleteTask.VisibilityTimestamp.Before(realLowerBound.Add(retention)),
		"retention deadline fires before real close + retention: %v < %v",
		deleteTask.VisibilityTimestamp, realLowerBound.Add(retention))
	s.False(deleteTask.VisibilityTimestamp.After(realUpperBound.Add(retention)),
		"retention deadline too far out (virtual time leaked into VisibilityTimestamp): %v > %v (skip=%v, retention=%v)",
		deleteTask.VisibilityTimestamp, realUpperBound.Add(retention), skipped, retention)
}
