...
// replicates the bound-reached transition. If the bound this task was generated
// for is still pending (same source event and not yet reached), the task is
// retried until the discard delay elapses; otherwise it is acked.
func (t *timerQueueStandbyTaskExecutor) executeTimeSkippingTimerTask(
	...
	actionFn := func(_ context.Context, wfContext historyi.WorkflowContext, mutableState historyi.MutableState, _ historyi.ReleaseWorkflowContextFunc) (any, error) {
		...
		// the bound this timer task is associated with is still valid and has not been reached so keep waiting
		if cb != nil && cb.GetSourceEventId() == timerTask.EventID && !cb.GetHasReached() {
			return &struct{}{}, nil
		}
		return nil, nil
	}
