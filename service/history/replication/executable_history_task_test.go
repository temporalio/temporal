...
func (s *executableHistoryTaskSuite) TestQueueID() {
	s.Equal(s.task.WorkflowKey, s.task.QueueID())
}

func (s *executableHistoryTaskSuite) TestMarkUnbatchable() {
	s.True(s.task.CanBatch())
	s.task.MarkUnbatchable()
	s.False(s.task.CanBatch())
}

func (s *executableHistoryTaskSuite) TestCancel() {
	s.executableTask.EXPECT().Cancel().Times(1)
	s.True(s.task.CanBatch())
	s.task.Cancel()
	s.False(s.task.CanBatch())
}

func (s *executableHistoryTaskSuite) TestMarkPoisonPill_GetShardError() {
	histTaskErr := errors.New("histTask-shard-err")
	s.shardController.EXPECT().GetShardByNamespaceWorkflow(
		namespace.ID(s.task.NamespaceID),
		s.task.WorkflowID,
	).Return(nil, histTaskErr)

	err := s.task.MarkPoisonPill()
	s.Equal(histTaskErr, err)
}

func (s *executableHistoryTaskSuite) TestMarkPoisonPill_RawTaskInfoAlreadySet() {
	shardID := rand.Int31()
	shardContext := historyi.NewMockShardContext(s.controller)
	s.shardController.EXPECT().GetShardByNamespaceWorkflow(
		namespace.ID(s.task.NamespaceID),
...
func (s *executableHistoryTaskSuite) TestMarkPoisonPill_DeserializeError() {
	shardID := rand.Int31()
	shardContext := historyi.NewMockShardContext(s.controller)
	s.shardController.EXPECT().GetShardByNamespaceWorkflow(
		namespace.ID(s.task.NamespaceID),
	...
	s.task.eventsBlobs = []*commonpb.DataBlob{histTaskCorruptedBlob()}
	...
	// MarkPoisonPill returns nil (swallowed) when deserialization fails.
	err := s.task.MarkPoisonPill()
	s.NoError(err)
}

func (s *executableHistoryTaskSuite) TestMarkPoisonPill_NoEvents() {
	shardID := rand.Int31()
	shardContext := historyi.NewMockShardContext(s.controller)
	s.shardController.EXPECT().GetShardByNamespaceWorkflow(
		namespace.ID(s.task.NamespaceID),
	...
	emptyBlob, err := s.eventSerializer.SerializeEvents([]*historypb.HistoryEvent{})
	s.NoError(err)
	s.task.eventsBlobs = []*commonpb.DataBlob{emptyBlob}
	...
	// MarkPoisonPill returns nil (swallowed) when there are no events.
	err = s.task.MarkPoisonPill()
	s.NoError(err)
}

func (s *executableHistoryTaskSuite) TestGetDeserializedEvents_CachedResponse() {
	cached := &eventsDeserializeResponse{
		events:       [][]*historypb.HistoryEvent{{{EventId: 1, Version: 1}}},
		newRunEvents: []*historypb.HistoryEvent{{EventId: 2, Version: 1}},
		err:          nil,
	}
	s.task.eventsDesResponse = cached

	events, newRunEvents, err := s.task.getDeserializedEvents()
	s.NoError(err)
	s.Equal(cached.events, events)
	s.Equal(cached.newRunEvents, newRunEvents)
}

func (s *executableHistoryTaskSuite) TestGetDeserializedEvents_EventsBlobError() {
	s.task.eventsBlobs = []*commonpb.DataBlob{histTaskCorruptedBlob()}

	_, _, err := s.task.getDeserializedEvents()
	s.Error(err)
	// Error is cached.
	s.NotNil(s.task.eventsDesResponse)
	s.Equal(err, s.task.eventsDesResponse.err)
}

func (s *executableHistoryTaskSuite) TestGetDeserializedEvents_NewRunEventsBlobError() {
	s.task.newRunEventsBlob = histTaskCorruptedBlob()
...
func (s *executableHistoryTaskSuite) TestBatchWith_NotBatchable_Failed() {
	currentTask, incomingTask := s.generateTwoBatchableTasks()
	currentTask.MarkUnbatchable()
	_, success := currentTask.BatchWith(incomingTask)
	s.False(success)
}

func (s *executableHistoryTaskSuite) TestBatchWith_IncomingCannotBatch_Failed() {
	currentTask, incomingTask := s.generateTwoBatchableTasks()
	incomingTask.MarkUnbatchable()
	_, success := currentTask.BatchWith(incomingTask)
	s.False(success)
}

func (s *executableHistoryTaskSuite) TestBatchWith_SourceClusterDoesNotMatch_Failed() {
	currentTask, incomingTask := s.generateTwoBatchableTasks()
	// Replace incoming task's ExecutableTask with one reporting a different source cluster.
	otherExecutableTask := NewMockExecutableTask(s.controller)
	otherExecutableTask.EXPECT().TaskID().Return(int64(999)).AnyTimes()
	otherExecutableTask.EXPECT().SourceClusterName().Return("histTask-other-cluster").AnyTimes()
	incomingTask.ExecutableTask = otherExecutableTask
	_, success := currentTask.BatchWith(incomingTask)
	s.False(success)
}

func (s *executableHistoryTaskSuite) TestBatchWith_CurrentEventsDeserializeError_Failed() {
	currentTask, incomingTask := s.generateTwoBatchableTasks()
	// Force current task to re-deserialize from a corrupted blob by clearing the cache.
	currentTask.eventsDesResponse = nil
	currentTask.eventsBlobs = []*commonpb.DataBlob{histTaskCorruptedBlob()}
	_, success := currentTask.BatchWith(incomingTask)
	s.False(success)
}

func (s *executableHistoryTaskSuite) TestBatchWith_IncomingEventsDeserializeError_Failed() {
	currentTask, incomingTask := s.generateTwoBatchableTasks()
	// validateIncomingBatchTask succeeds (it caches incoming events via its own
	// getDeserializedEvents using the in-memory response), but to exercise the
	// incoming deserialize error path in BatchWith we corrupt after validation by
	// making incoming's deserialize fail. Clear cache and corrupt blob.
	incomingTask.eventsDesResponse = nil
	incomingTask.eventsBlobs = []*commonpb.DataBlob{histTaskCorruptedBlob()}
	_, success := currentTask.BatchWith(incomingTask)
	s.False(success)
}

func (s *executableHistoryTaskSuite) TestValidateIncomingBatchTask_UnsupportedType() {
	unsupported := NewMockBatchableTask(s.controller)
	unsupported.EXPECT().CanBatch().Return(true).AnyTimes()
	_, success := s.task.BatchWith(unsupported)
	s.False(success)
}

func (s *executableHistoryTaskSuite) TestCheckSourceCluster_Match() {
	s.NoError(s.task.checkSourceCluster(s.sourceClusterName))
}
...
func (s *executableHistoryTaskSuite) TestCheckBaseExecutionInfo_BothNil() {
	...
	s.NoError(s.task.checkBaseExecutionInfo(nil))
}

func (s *executableHistoryTaskSuite) TestCheckBaseExecutionInfo_CurrentNil() {
	s.task.baseExecutionInfo = nil
...
func (s *executableHistoryTaskSuite) TestCheckBaseExecutionInfo_Equal() {
	s.task.baseExecutionInfo = &workflowspb.BaseExecutionInfo{RunId: "histTask-a"}
	s.NoError(s.task.checkBaseExecutionInfo(&workflowspb.BaseExecutionInfo{RunId: "histTask-a"}))
}
...
func (s *executableHistoryTaskSuite) TestCheckEvents_GetDeserializedEventsError() {
	s.task.eventsBlobs = []*commonpb.DataBlob{histTaskCorruptedBlob()}
	err := s.task.checkEvents([][]*historypb.HistoryEvent{{{EventId: 1, Version: 1}}})
	s.Error(err)
}

func (s *executableHistoryTaskSuite) TestHandleErr_Resend_NamespaceError() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	nsErr := errors.New("histTask-ns-err")
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		"", false, nsErr,
	).AnyTimes()
	err := serviceerrors.NewRetryReplication(
...
func (s *executableHistoryTaskSuite) TestHandleErr_Resend_NotContinue() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
	).AnyTimes()
...
// histTaskCorruptedBlob returns a DataBlob that fails event deserialization.
func histTaskCorruptedBlob() *commonpb.DataBlob {
	return &commonpb.DataBlob{
		EncodingType: enumspb.ENCODING_TYPE_PROTO3,
		Data:         []byte("histTask-not-a-valid-proto"),
	}
}
