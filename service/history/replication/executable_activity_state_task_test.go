...
func (s *executableActivityStateTaskSuite) TestQueueID() {
	s.Equal(s.task.WorkflowKey, s.task.QueueID())
}

func (s *executableActivityStateTaskSuite) TestMarkUnbatchable() {
	s.True(s.task.CanBatch())
	s.task.MarkUnbatchable()
	s.False(s.task.CanBatch())
}

func (s *executableActivityStateTaskSuite) TestCancel() {
...
func (s *executableActivityStateTaskSuite) TestBatchWith_NotBatchable_ShouldNotBatch() {
	s.task.MarkUnbatchable()
	actStateOther := NewExecutableActivityStateTask(
		ProcessToolBox{
			ClusterMetadata: s.clusterMetadata,
			ClientBean:      s.clientBean,
			ShardController: s.shardController,
		...
		2,
		time.Unix(0, rand.Int63()),
		s.generateReplicationAttribute(s.task.NamespaceID, s.task.WorkflowID, s.task.RunID),
		s.sourceClusterName,
		s.sourceShardKey,
		&replicationspb.ReplicationTask{Priority: enumsspb.TASK_PRIORITY_HIGH},
	)
	batchResult, batched := s.task.BatchWith(actStateOther)
	s.False(batched)
	s.Nil(batchResult)
}

func (s *executableActivityStateTaskSuite) TestBatchWith_IncomingCannotBatch_ShouldNotBatch() {
	actStateIncoming := NewExecutableActivityStateTask(
		ProcessToolBox{
			ClusterMetadata: s.clusterMetadata,
			ClientBean:      s.clientBean,
			ShardController: s.shardController,
	...
	actStateIncoming.MarkUnbatchable()
	batchResult, batched := s.task.BatchWith(actStateIncoming)
	s.False(batched)
	s.Nil(batchResult)
}

func (s *executableActivityStateTaskSuite) TestValidateIncomingBatchTask_Nil() {
...
func (s *executableActivityStateTaskSuite) TestValidateIncomingBatchTask_UnsupportedType() {
	actStateWrong := NewMockBatchableTask(s.controller)
	actStateWrong.EXPECT().CanBatch().Return(true).AnyTimes()
...
func (s *executableActivityStateTaskSuite) TestHandleErr_Resend_NamespaceError() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		"", false, errors.New("namespace-error"),
	).AnyTimes()
	err := serviceerrors.NewRetryReplication(
		"",
		s.task.NamespaceID,
...
func (s *executableActivityStateTaskSuite) TestHandleErr_Resend_DoNotContinue() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
	).AnyTimes()
...
func (s *executableActivityStateTaskSuite) TestHandleErr_Duplicate() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	s.executableTask.EXPECT().MarkTaskDuplicated().Times(1)
	s.NoError(s.task.HandleErr(consts.ErrDuplicate))
}

func (s *executableActivityStateTaskSuite) TestExecute_GetShardError() {
	s.executableTask.EXPECT().TerminalState().Return(false)
	s.executableTask.EXPECT().MarkExecutionStart()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
	).AnyTimes()
	shardErr := errors.New("get-shard-error")
	s.shardController.EXPECT().GetShardByNamespaceWorkflow(
		namespace.ID(s.task.NamespaceID),
		s.task.WorkflowID,
	).Return(nil, shardErr)
	s.Equal(shardErr, s.task.Execute())
}

func (s *executableActivityStateTaskSuite) TestExecute_GetEngineError() {
	s.executableTask.EXPECT().TerminalState().Return(false)
	s.executableTask.EXPECT().MarkExecutionStart()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
	...
	engineErr := errors.New("get-engine-error")
	shardContext.EXPECT().GetEngine(gomock.Any()).Return(nil, engineErr)
	s.Equal(engineErr, s.task.Execute())
}
