...
func (s *executableBackfillHistoryEventsTaskSuite) TestQueueID() {
	s.Equal(s.task.WorkflowKey, s.task.QueueID())
}

func (s *executableBackfillHistoryEventsTaskSuite) TestExecute_GetShardError() {
	s.executableTask.EXPECT().TerminalState().Return(false)
	s.executableTask.EXPECT().MarkExecutionStart()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
	).AnyTimes()
	backfillShardErr := errors.New("backfill-shard-error")
	s.shardController.EXPECT().GetShardByNamespaceWorkflow(
		namespace.ID(s.task.NamespaceID),
		s.task.WorkflowID,
	).Return(nil, backfillShardErr)

	s.Equal(backfillShardErr, s.task.Execute())
}

func (s *executableBackfillHistoryEventsTaskSuite) TestExecute_GetEngineError() {
	s.executableTask.EXPECT().TerminalState().Return(false)
	s.executableTask.EXPECT().MarkExecutionStart()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
	...
	backfillEngineErr := errors.New("backfill-engine-error")
	shardContext.EXPECT().GetEngine(gomock.Any()).Return(nil, backfillEngineErr)

	s.Equal(backfillEngineErr, s.task.Execute())
}

func (s *executableBackfillHistoryEventsTaskSuite) TestExecute_DeserializeError() {
	s.executableTask.EXPECT().TerminalState().Return(false)
	s.executableTask.EXPECT().MarkExecutionStart()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
	...
	// Corrupt the event batch so deserialization fails.
	s.task.taskAttr.EventBatches = []*commonpb.DataBlob{{
...
func (s *executableBackfillHistoryEventsTaskSuite) TestGetDeserializedEvents_NewRunError() {
	// EventBatches deserialize fine, but the new run event batch is corrupt.
	s.task.taskAttr.NewRunInfo.EventBatch = &commonpb.DataBlob{
		EncodingType: enumspb.ENCODING_TYPE_PROTO3,
		Data:         []byte("not-a-valid-proto"),
	}
...
func (s *executableBackfillHistoryEventsTaskSuite) TestHandleErr_Duplicate() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
...
func (s *executableBackfillHistoryEventsTaskSuite) TestHandleErr_NotFound() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()

	s.NoError(s.task.HandleErr(serviceerror.NewNotFound("")))
}

func (s *executableBackfillHistoryEventsTaskSuite) TestHandleErr_Default() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	backfillDefaultErr := serviceerror.NewUnavailable("backfill-default")

	s.Equal(backfillDefaultErr, s.task.HandleErr(backfillDefaultErr))
}

func (s *executableBackfillHistoryEventsTaskSuite) backfillNewSyncStateErr() *serviceerrors.SyncState {
	return serviceerrors.NewSyncState(
		...
		s.task.RunID,
		0,
		s.replicationTask.VersionedTransition,
		nil,
	).(*serviceerrors.SyncState)
}

func (s *executableBackfillHistoryEventsTaskSuite) TestHandleErr_SyncState_NamespaceError() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	syncStateErr := s.backfillNewSyncStateErr()
	nsErr := errors.New("backfill-ns-error")
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		"", false, nsErr,
	)
...
func (s *executableBackfillHistoryEventsTaskSuite) TestHandleErr_SyncState_Error() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	syncStateErr := s.backfillNewSyncStateErr()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
	).AnyTimes()
	s.executableTask.EXPECT().SyncState(gomock.Any(), syncStateErr, ResendAttempt).Return(
		false, errors.New("backfill-sync-state-error"),
	)

	s.Equal(syncStateErr, s.task.HandleErr(syncStateErr))
}

func (s *executableBackfillHistoryEventsTaskSuite) TestHandleErr_SyncState_DoNotContinue() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	syncStateErr := s.backfillNewSyncStateErr()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
...
func (s *executableBackfillHistoryEventsTaskSuite) TestHandleErr_SyncState_Continue() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	syncStateErr := s.backfillNewSyncStateErr()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
	...
	// SyncState returning true triggers a re-Execute.
	s.executableTask.EXPECT().TerminalState().Return(false)
	s.executableTask.EXPECT().MarkExecutionStart()
	s.executableTask.EXPECT().ReplicationTask().Times(1).Return(s.replicationTask)
	shardContext := historyi.NewMockShardContext(s.controller)
...
func (s *executableBackfillHistoryEventsTaskSuite) TestHandleErr_Resend_NamespaceError() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	nsErr := errors.New("backfill-resend-ns-error")
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		"", false, nsErr,
	)
	err := serviceerrors.NewRetryReplication(
...
func (s *executableBackfillHistoryEventsTaskSuite) TestHandleErr_Resend_StartVersionError() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
	).AnyTimes()
	// Empty version history so GetVersionHistoryEventVersion fails for the start event.
	s.task.taskAttr.EventVersionHistory = []*historyspb.VersionHistoryItem{}
	err := serviceerrors.NewRetryReplication(
		"",
		s.task.NamespaceID,
		s.task.WorkflowID,
	...
	s.Error(s.task.HandleErr(err))
}

func (s *executableBackfillHistoryEventsTaskSuite) TestHandleErr_Resend_EndVersionError() {
	s.executableTask.EXPECT().NamespaceName().Return("test-namespace").AnyTimes()
	s.executableTask.EXPECT().GetNamespaceInfo(gomock.Any(), s.task.NamespaceID, gomock.Any()).Return(
		uuid.NewString(), true, nil,
	).AnyTimes()
	// Version history covers the start event (firstEventID+1) but not the end event
	// (nextEventID-2), so GetVersionHistoryEventVersion fails for the end event only.
	s.task.taskAttr.EventVersionHistory = []*historyspb.VersionHistoryItem{{
		EventId: s.firstEventID + 1,
		Version: s.version,
	}}
	err := serviceerrors.NewRetryReplication(
		"",
