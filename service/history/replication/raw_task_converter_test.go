...
func (s *rawTaskConverterSuite) TestRawConvNewSourceTaskConverter() {
	...
	s.NotNil(converter)
	s.Equal(s.mockEngine, converter.historyEngine)
	s.Equal(s.shardContext.Resource.NamespaceCache, converter.namespaceCache)
	s.Equal(s.shardContext.GetConfig(), converter.config)
}

func (s *rawTaskConverterSuite) TestRawConvConvert_Success() {
	serializer := serialization.NewSerializer()
	converter := NewSourceTaskConverter(
		s.mockEngine,
		s.shardContext.Resource.NamespaceCache,
	...
	s.mockEngine.EXPECT().ConvertReplicationTask(gomock.Any(), task, targetClusterID).Return(replicationTask, nil)

	result, err := converter.Convert(task, targetClusterID, enumsspb.TASK_PRIORITY_HIGH)
	s.NoError(err)
	s.NotNil(result)
	s.NotNil(result.RawTaskInfo)
}

func (s *rawTaskConverterSuite) TestRawConvConvert_NilReplicationTask() {
	serializer := serialization.NewSerializer()
	converter := NewSourceTaskConverter(
		s.mockEngine,
		s.shardContext.Resource.NamespaceCache,
	...
	// nil replication task: should not attempt serialization, returns nil, nil
	s.mockEngine.EXPECT().ConvertReplicationTask(gomock.Any(), task, targetClusterID).Return(nil, nil)

	result, err := converter.Convert(task, targetClusterID, enumsspb.TASK_PRIORITY_LOW)
	s.NoError(err)
	s.Nil(result)
}

func (s *rawTaskConverterSuite) TestRawConvConvert_EngineError() {
	serializer := serialization.NewSerializer()
	converter := NewSourceTaskConverter(
		s.mockEngine,
		s.shardContext.Resource.NamespaceCache,
	...
	convertErr := serviceerror.NewInternal("convert failed")
	s.mockEngine.EXPECT().ConvertReplicationTask(gomock.Any(), task, targetClusterID).Return(nil, convertErr)

	result, err := converter.Convert(task, targetClusterID, enumsspb.TASK_PRIORITY_UNSPECIFIED)
	s.Error(err)
	s.Equal(convertErr, err)
	s.Nil(result)
}

func (s *rawTaskConverterSuite) TestRawConvConvert_NamespaceLookupError() {
	// Use a namespace ID that the registry does not know about, exercising the
	// "blindly send the task" branch where nsName is set to EmptyName.
	unknownNamespaceID := namespace.ID(uuid.NewString())
	rawConvWorkflowID := uuid.NewString()
	rawConvRunID := uuid.NewString()
	s.shardContext.Resource.NamespaceCache.EXPECT().GetNamespaceByID(unknownNamespaceID).Return(
		nil, serviceerror.NewNamespaceNotFound(unknownNamespaceID.String()),
	)
...
func (s *rawTaskConverterSuite) TestRawConvConvertGetHistoryError_NotFound() {
	workflowKey := definition.NewWorkflowKey(s.namespaceID, s.workflowID, s.runID)
	// NotFound should be swallowed (return nil) to unblock the replication queue.
	result := convertGetHistoryError(workflowKey, s.logger, serviceerror.NewNotFound("missing"))
	s.NoError(result)
}

func (s *rawTaskConverterSuite) TestRawConvConvertGetHistoryError_DataLoss() {
	workflowKey := definition.NewWorkflowKey(s.namespaceID, s.workflowID, s.runID)
	// DataLoss should be swallowed (return nil) to unblock the replication queue.
	result := convertGetHistoryError(workflowKey, s.logger, serviceerror.NewDataLoss("corrupted"))
	s.NoError(result)
}

func (s *rawTaskConverterSuite) TestRawConvConvertGetHistoryError_Other() {
	workflowKey := definition.NewWorkflowKey(s.namespaceID, s.workflowID, s.runID)
	otherErr := serviceerror.NewInternal("some other error")
	result := convertGetHistoryError(workflowKey, s.logger, otherErr)
	s.Error(result)
	s.Equal(otherErr, result)
}

func (s *rawTaskConverterSuite) TestRawConvConvertHistoryReplicationTask_GetEventsBlobError() {
	ctx := context.Background()
	shardID := int32(12)
	firstEventID := int64(999)
	nextEventID := int64(1911)
	...
	s.mutableState.EXPECT().GetBaseWorkflowInfo().Return(nil).AnyTimes()
	// DataLoss is swallowed by convertGetHistoryError so the overall result is nil, nil.
	s.executionManager.EXPECT().ReadRawHistoryBranch(gomock.Any(), gomock.Any()).Return(
		nil, serviceerror.NewDataLoss("corrupted history"),
	)
...
func (s *rawTaskConverterSuite) TestRawConvConvertHistoryReplicationTask_GetEventsBlobError_Propagated() {
	ctx := context.Background()
	shardID := int32(12)
	firstEventID := int64(999)
	nextEventID := int64(1911)
	...
	s.mutableState.EXPECT().GetBaseWorkflowInfo().Return(nil).AnyTimes()
	otherErr := serviceerror.NewUnavailable("db unavailable")
	s.executionManager.EXPECT().ReadRawHistoryBranch(gomock.Any(), gomock.Any()).Return(nil, otherErr)
...
func (s *rawTaskConverterSuite) TestRawConvGenerateVerifyVersionedTransitionTask() {
	ctx := context.Background()
	targetClusterID := int32(3)
	...
	versionHistoryItems := []*historyspb.VersionHistoryItem{
		{EventId: nextEventID, Version: version},
	}
	versionHistory := &historyspb.VersionHistory{
		BranchToken: []byte("branch token"),
		Items:       versionHistoryItems,
	...
	transitionHistory := []*persistencespb.VersionedTransition{
		{NamespaceFailoverVersion: 1, TransitionCount: 3},
		{NamespaceFailoverVersion: 1, TransitionCount: 6},
	}
	...
	// progress already has the versioned transition sent (TransitionCount 6 in [0,10]),
	// so the verify path is taken.
	progress := &ReplicationProgress{
		versionedTransitions: [][]*persistencespb.VersionedTransition{
			{{NamespaceFailoverVersion: 1, TransitionCount: 10}},
		},
	}
	s.progressCache.EXPECT().Get(s.runID, targetClusterID).Return(progress)

	converter := newSyncVersionedTransitionTaskConverter(s.shardContext, s.workflowCache, nil, s.progressCache, s.executionManager, s.syncStateRetriever, s.logger)
	result, err := convertSyncVersionedTransitionTask(ctx, task, targetClusterID, converter)
	s.NoError(err)
	s.NotNil(result)
	s.Equal(enumsspb.REPLICATION_TASK_TYPE_VERIFY_VERSIONED_TRANSITION_TASK, result.TaskType)
	attr := result.GetVerifyVersionedTransitionTaskAttributes()
	s.NotNil(attr)
	s.Equal(s.namespaceID, attr.NamespaceId)
	s.Equal(s.workflowID, attr.WorkflowId)
	s.Equal(s.runID, attr.RunId)
	s.Equal(nextEventID, attr.NextEventId)
	// EventVersionHistory is capped at the LCA item for nextEventID-1.
	s.ProtoEqual(&historyspb.VersionHistoryItem{EventId: nextEventID - 1, Version: version}, attr.EventVersionHistory[len(attr.EventVersionHistory)-1])
	s.True(s.lockReleased)
}

func (s *rawTaskConverterSuite) TestRawConvGenerateVerifyVersionedTransitionTask_FromLastVersionHistoryItem() {
	ctx := context.Background()
	targetClusterID := int32(3)
	version := int64(1)
	...
	task := &tasks.SyncVersionedTransitionTask{
		...
		TaskID:              taskID,
		ArchetypeID:         chasm.WorkflowArchetypeID,
		// NextEventID is empty so nextEventId is derived from LastVersionHistoryItem.
		NextEventID:            common.EmptyEventID,
		LastVersionHistoryItem: &historyspb.VersionHistoryItem{EventId: lastEventID, Version: version},
		VersionedTransition: &persistencespb.VersionedTransition{
			NamespaceFailoverVersion: version,
			TransitionCount:          6,
		},
	...
	versionHistoryItems := []*historyspb.VersionHistoryItem{
		{EventId: lastEventID + 5, Version: version},
	}
	versionHistory := &historyspb.VersionHistory{
		BranchToken: []byte("branch token"),
		Items:       versionHistoryItems,
	...
	s.Equal(lastEventID+1, attr.NextEventId)
	s.True(s.lockReleased)
}

func (s *rawTaskConverterSuite) TestRawConvConvertSyncVersionedTransitionTask_ChasmNotOnVersionedTransition() {
	ctx := context.Background()
	targetClusterID := int32(3)
	taskID := int64(1444)
	task := &tasks.SyncVersionedTransitionTask{
		...
		VisibilityTimestamp: time.Now().UTC(),
		TaskID:              taskID,
		// UnspecifiedArchetypeID exercises the default-archetype assignment branch.
	...
	s.workflowContext.EXPECT().LoadMutableState(gomock.Any(), s.shardContext).Return(s.mutableState, nil)
	// Empty transition history and a non-workflow chasm execution triggers the error path.
	s.mutableState.EXPECT().GetExecutionInfo().Return(&persistencespb.WorkflowExecutionInfo{
		TransitionHistory: nil,
	}).Times(1)
	s.mutableState.EXPECT().IsWorkflow().Return(false).Times(1)
...
func (s *rawTaskConverterSuite) TestRawConvConvertTaskEquivalents_NoEquivalents() {
	ctx := context.Background()
	targetClusterID := int32(3)
	taskID := int64(1444)
	task := &tasks.SyncVersionedTransitionTask{
	...
	s.mutableState.EXPECT().IsWorkflow().Return(true).Times(1)

	converter := newSyncVersionedTransitionTaskConverter(s.shardContext, s.workflowCache, nil, s.progressCache, s.executionManager, s.syncStateRetriever, s.logger)
	// No task equivalents: task is dropped, returns nil, nil.
	result, err := convertSyncVersionedTransitionTask(ctx, task, targetClusterID, converter)
	s.NoError(err)
	s.Nil(result)
	s.True(s.lockReleased)
...
func (s *rawTaskConverterSuite) TestRawConvGetVersionHistoryAndEvents_XDCCacheHit() {
	ctx := context.Background()
	shardID := int32(12)
	firstEventID := int64(999)
	nextEventID := int64(1911)
	...
	cachedItems := []*historyspb.VersionHistoryItem{
		{EventId: nextEventID - 1, Version: version},
	}
	events := &commonpb.DataBlob{
		EncodingType: enumspb.ENCODING_TYPE_PROTO3,
	...
	// Pre-seed the XDC cache so the cache-hit branch in getVersionHistoryAndEvents
	// is exercised and no workflow load / history read happens.
	...
	eventBlobCache.Put(
		...
		persistence.NewXDCCacheValue(
			baseWorkflowInfo,
			cachedItems,
			[]*commonpb.DataBlob{events},
			nextEventID,
		),
	)

	result, err := convertHistoryReplicationTask(ctx, s.shardContext, task, shardID, s.workflowCache, eventBlobCache, s.executionManager, s.logger, s.shardContext.GetConfig())
	s.NoError(err)
	s.NotNil(result)
	attr := result.GetHistoryTaskAttributes()
	s.NotNil(attr)
	s.ProtoEqual(baseWorkflowInfo, attr.BaseExecutionInfo)
	s.Equal(cachedItems, attr.VersionHistoryItems)
}
