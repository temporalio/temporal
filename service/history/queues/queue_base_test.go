...
func (s *queueBaseSuite) TestEmitImmediateQueueBacklogAge_InMemory() {
	...
	mockShard.Resource.ClusterMetadata.EXPECT().GetCurrentClusterName().Return(cluster.TestCurrentClusterName).AnyTimes()
	mockShard.SetMetricsHandler(captureHandler)

	base := s.newQueueBase(mockShard, tasks.CategoryTransfer, nil)
	now := base.timeSource.Now()

	// Post-shrink shape: scope starts at the oldest loaded task, iterator covers the tail above it.
	scope := NewScope(NewRange(tasks.NewImmediateKey(200), tasks.NewImmediateKey(1000)), predicates.Universal[tasks.Task]())
	...
	slice.iterators = []Iterator{NewIterator(nil, NewRange(tasks.NewImmediateKey(301), tasks.NewImmediateKey(1000)))}
	...
	// Labels must match shardinfo_immediate_queue_lag.
...
// unloadedImmediateSlice builds a slice with a backlog in persistence but nothing loaded.
func (s *queueBaseSuite) unloadedImmediateSlice(base *queueBase, h metrics.Handler, minID, maxID int64, pred tasks.Predicate) Slice {
	return NewSlice(nil, base.executableFactory, base.monitor, NewScope(NewRange(tasks.NewImmediateKey(minID), tasks.NewImmediateKey(maxID)), pred), GrouperNamespaceID{}, noPredicateSizeLimit, defaultMaxPendingKeys, h)
}

func (s *queueBaseSuite) TestEmitImmediateQueueBacklogAge_OldestUnloaded_NoEmit() {
	captureHandler := metricstest.NewCaptureHandler()
	_, base := s.newImmediateBase(captureHandler)
	now := base.timeSource.Now()

	// An unloaded slice sits below everything in memory, so the oldest task's age is unknown.
	loaded := s.loadedImmediateSlice(base, captureHandler, 1000, 2000, now.Add(-10*time.Minute))
	unloaded := s.unloadedImmediateSlice(base, captureHandler, 100, 1000, predicates.Universal[tasks.Task]())
	base.readerGroup.NewReader(DefaultReaderId, unloaded, loaded)
...
func (s *queueBaseSuite) TestEmitImmediateQueueBacklogAge_LoadedSliceWithUnloadedHead_NoEmit() {
	captureHandler := metricstest.NewCaptureHandler()
	_, base := s.newImmediateBase(captureHandler)
	now := base.timeSource.Now()

	// A cleared slice merged into a loaded one: loaded tasks start at 500 but iterators still cover
	// [100,500), so the scope lower bound (100) is not the oldest loaded task.
	sl := NewSlice(nil, base.executableFactory, base.monitor, NewScope(NewRange(tasks.NewImmediateKey(100), tasks.NewImmediateKey(2000)), predicates.Universal[tasks.Task]()), GrouperNamespaceID{}, noPredicateSizeLimit, defaultMaxPendingKeys, captureHandler)
	sl.add(s.mockPendingExecutable(tasks.NewImmediateKey(500), now.Add(-time.Minute)))
	sl.iterators = []Iterator{NewIterator(nil, NewRange(tasks.NewImmediateKey(100), tasks.NewImmediateKey(500)))}
	base.readerGroup.NewReader(DefaultReaderId, sl)
...
func (s *queueBaseSuite) TestEmitImmediateQueueBacklogAge_MultiReader_LowerReaderUnloaded_NoEmit() {
	captureHandler := metricstest.NewCaptureHandler()
	_, base := s.newImmediateBase(captureHandler)
	now := base.timeSource.Now()

	// Reader 1 holds a lower unloaded range, so the oldest task is not in memory.
	base.readerGroup.NewReader(DefaultReaderId, s.loadedImmediateSlice(base, captureHandler, 1000, 2000, now.Add(-time.Minute)))
	base.readerGroup.NewReader(DefaultReaderId+1, s.unloadedImmediateSlice(base, captureHandler, 100, 1000, predicates.Universal[tasks.Task]()))

	capture := captureHandler.StartCapture()
	// Repeated because readers are visited in map order, so one call only samples one ordering.
	for range 30 {
		base.emitImmediateQueueBacklogAge()
	}
	snap := capture.Snapshot()
...
func (s *queueBaseSuite) TestEmitImmediateQueueBacklogAge_EqualMinKeys_Emits() {
	captureHandler := metricstest.NewCaptureHandler()
	_, base := s.newImmediateBase(captureHandler)
	now := base.timeSource.Now()

	// SplitByPredicate leaves both halves on the same range and moves one to the next reader, the
	// normal post-move-group state. The half holding the frontier task supplies the age.
	base.readerGroup.NewReader(DefaultReaderId, s.loadedImmediateSlice(base, captureHandler, 100, 1000, now.Add(-time.Hour)))
	base.readerGroup.NewReader(DefaultReaderId+1, s.unloadedImmediateSlice(base, captureHandler, 100, 1000, predicates.Universal[tasks.Task]()))
	...
	recordings := snap[metrics.ShardInfoImmediateQueueBacklogAge.Name()]
	s.Require().Len(recordings, 30)
	for _, recording := range recordings {
		age := recording.Value.(time.Duration)
		s.GreaterOrEqual(age, time.Hour)
		s.Less(age, time.Hour+time.Minute)
	}
}

func (s *queueBaseSuite) TestEmitImmediateQueueBacklogAge_EmptyBacklog_EmitsZero() {
	captureHandler := metricstest.NewCaptureHandler()
	_, base := s.newImmediateBase(captureHandler)

	// No slices at all: nothing pending and nothing to load, so the age is zero rather than unknown.
	base.readerGroup.NewReader(DefaultReaderId)
...
func (s *queueBaseSuite) TestCheckPoint_EmitsImmediateQueueBacklogAge() {
	captureHandler := metricstest.NewCaptureHandler()
	mockShard, base := s.newImmediateBase(captureHandler)
	mockShard.Resource.ClusterMetadata.EXPECT().GetAllClusterInfo().Return(cluster.TestAllClusterInfo).AnyTimes()
	...
	// The frontier task is acked but not yet shrunk away, so the age is only knowable once
	// ShrinkSlices has advanced the slice past it.
	sl := NewSlice(nil, base.executableFactory, base.monitor, NewScope(NewRange(tasks.NewImmediateKey(100), tasks.NewImmediateKey(1000)), predicates.Universal[tasks.Task]()), GrouperNamespaceID{}, noPredicateSizeLimit, defaultMaxPendingKeys, captureHandler)
	sl.add(s.mockAckedExecutable(tasks.NewImmediateKey(100)))
	sl.add(s.mockPendingExecutable(tasks.NewImmediateKey(500), now.Add(-45*time.Minute)))
	sl.iterators = nil
	base.readerGroup.NewReader(DefaultReaderId+1, sl)

	capture := captureHandler.StartCapture()
	base.emitImmediateQueueBacklogAge()
	s.Empty(capture.Snapshot()[metrics.ShardInfoImmediateQueueBacklogAge.Name()], "acked frontier is not a known age")

	// checkpoint() shrinks first, so it emits; this also pins the emit call site itself.
	base.checkpoint()
	snap := capture.Snapshot()
	captureHandler.StopCapture(capture)
	...
	s.Require().Len(recordings, 1)
	age := recordings[0].Value.(time.Duration)
	s.GreaterOrEqual(age, 45*time.Minute)
	s.Less(age, 46*time.Minute)
}

func (s *queueBaseSuite) TestEmitImmediateQueueBacklogAge_UsesShardMetricsHandler() {
	captureHandler := metricstest.NewCaptureHandler()
	mockShard := shard.NewTestContext(
		s.controller,
		&persistencespb.ShardInfo{ShardId: 0, RangeId: 10},
	...
	// Only the shard's handler captures, so the metric must be emitted through it.
	s.metricsHandler = metrics.NoopMetricsHandler
	base := s.newQueueBase(mockShard, tasks.CategoryTransfer, nil)
	now := base.timeSource.Now()
	base.readerGroup.NewReader(DefaultReaderId, s.loadedImmediateSlice(base, metrics.NoopMetricsHandler, 100, 1000, now.Add(-time.Hour)))
	...
	s.Require().Len(snap[metrics.ShardInfoImmediateQueueBacklogAge.Name()], 1)
}

func (s *queueBaseSuite) mockAckedExecutable(key tasks.Key) *MockExecutable {
	e := NewMockExecutable(s.controller)
	e.EXPECT().GetKey().Return(key).AnyTimes()
	e.EXPECT().GetVisibilityTime().Return(time.Unix(0, 0)).AnyTimes()
	e.EXPECT().State().Return(ctasks.TaskStateAcked).AnyTimes()
	e.EXPECT().GetNamespaceID().Return("ns").AnyTimes()
	e.EXPECT().GetTask().Return(e).AnyTimes()
	return e
}

func (s *queueBaseSuite) TestEmitImmediateQueueBacklogAge_NewerSliceUnloaded_Emits() {
	captureHandler := metricstest.NewCaptureHandler()
	_, base := s.newImmediateBase(captureHandler)
	now := base.timeSource.Now()

	// Only newer tasks are unloaded, so the oldest is still known.
	loaded := s.loadedImmediateSlice(base, captureHandler, 100, 1000, now.Add(-time.Hour))
	unloaded := s.unloadedImmediateSlice(base, captureHandler, 1000, 2000, predicates.Universal[tasks.Task]())
	base.readerGroup.NewReader(DefaultReaderId, loaded, unloaded)
