...
func (s *workflowSuite) TestVersionCeilingDefersMigration() {
	...
	s.runWithCeiling(true, oldPeerCeiling, pausedHourlySchedule(), 2)
	...
	s.env.ExecuteWorkflow(func(ctx workflow.Context, args *schedulespb.StartScheduleArgs) error {
		// ceiling unset (clamp lifted) and migration disabled: the deferred migration from the
		// prior run's PendingMigration is what executes here.
		return schedulerWorkflowWithSpecBuilder(ctx, args, newSpecBuilderForTest(0, 0), func() bool { return false }, func() bool { return true }, func() int { return -1 })
	}, canArgs)
...
func (s *workflowSuite) runWithCeiling(enableCHASM bool, ceiling int, sched *schedulepb.Schedule, iterations int) {
	s.runWorkflowFn(func(ctx workflow.Context, args *schedulespb.StartScheduleArgs) error {
		return schedulerWorkflowWithSpecBuilder(ctx, args, newSpecBuilderForTest(0, 0), func() bool { return enableCHASM }, func() bool { return true }, func() int { return ceiling })
	}, sched, iterations)
}
