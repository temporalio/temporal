...
func (s *workflowSuite) TestVersionCeilingDefersMigration() {
	...
	s.runWithCeiling(true, func() int { return oldPeerCeiling }, pausedHourlySchedule(), 2)
	...
	s.env.ExecuteWorkflow(func(ctx workflow.Context, args *schedulespb.StartScheduleArgs) error {
		return schedulerWorkflowWithSpecBuilder(ctx, args, NewSpecBuilder(), false, nil) // ceiling unset: clamp lifted
	}, canArgs)
...
func (s *workflowSuite) runWithCeiling(enableCHASMMigration bool, versionCeiling func() int, sched *schedulepb.Schedule, iterations int) {
	s.runWorkflowFn(func(ctx workflow.Context, args *schedulespb.StartScheduleArgs) error {
		return schedulerWorkflowWithSpecBuilder(ctx, args, NewSpecBuilder(), enableCHASMMigration, versionCeiling)
	}, sched, iterations)
}
