...
const (
	...
	tryDeleteForAddingVersionIdentity = "try-delete-for-add-version"       // identity of the worker-deployment workflow when it tries to delete a version on the event that the addition
	// of a version shall exceed the max number of versions allowed in a worker-deployment (defaultMaxVersions)
